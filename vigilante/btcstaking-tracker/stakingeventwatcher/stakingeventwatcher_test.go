...
func TestHandlingDelegations(t *testing.T) {
	...
	mockAnonNodeAdapter := NewMockAnonNodeAdapter(ctrl)

	mockAnonNodeAdapter.EXPECT().BtcClientTipHeight().Return(uint32(0), nil).AnyTimes()
	...
	sew := StakingEventWatcher{
		...
		cfg:                             &cfg,
		anonNodeAdapter:              mockAnonNodeAdapter,
	...
	mockAnonNodeAdapter.EXPECT().QueryHeaderDepth(gomock.Any()).Return(uint32(2), nil).AnyTimes()
	mockAnonNodeAdapter.EXPECT().IsDelegationVerified(gomock.Any()).Return(true, nil).AnyTimes()
	mockAnonNodeAdapter.EXPECT().BTCDelegation(gomock.Any()).Return(&Delegation{
		Status: btcstakingtypes.BTCDelegationStatus_VERIFIED.String(),
	}, nil).AnyTimes()

	params := AnonParams{ConfirmationTimeBlocks: 1}
	mockAnonNodeAdapter.EXPECT().Params().Return(&params, nil).AnyTimes()
...
func TestHandlingDelegationsByEvents(t *testing.T) {
	...
	mockAnonNodeAdapter.EXPECT().CometBFTTipHeight(gomock.Any()).DoAndReturn(func(_ context.Context) (int64, error) {
	...
	firstCall := mockAnonNodeAdapter.EXPECT().
