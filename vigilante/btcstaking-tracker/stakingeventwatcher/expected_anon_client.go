...
var (
	ErrHeaderNotKnownToAnon       = errors.New("btc header not known to anon")
	ErrHeaderOnAnonLCFork         = errors.New("btc header is on anon btc light client fork")
	ErrAnonBtcLightClientNotReady = errors.New("anon btc light client is not ready to receive delegation")
)
...
type AnonParams struct {
	ConfirmationTimeBlocks uint32 // K-deep
}

type AnonNodeAdapter interface {
...
type AnonClientAdapter struct {
	anonClient *ancclient.Client
	cfg           *config.BTCStakingTrackerConfig
}

var _ AnonNodeAdapter = (*AnonClientAdapter)(nil)

func NewAnonClientAdapter(anonClient *ancclient.Client, cfg *config.BTCStakingTrackerConfig) *AnonClientAdapter {
	return &AnonClientAdapter{
...
func (bca *AnonClientAdapter) DelegationsByStatus(status btcstakingtypes.BTCDelegationStatus, cursor []byte, limit uint64) ([]Delegation, []byte, error) {
	resp, err := bca.anonClient.BTCDelegations(
		status,
		&query.PageRequest{
			Key:   cursor,
			Limit: limit,
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed to retrieve delegations from anon: %w", err)
	}

	delegations := make([]Delegation, len(resp.BtcDelegations))

	for i, delegation := range resp.BtcDelegations {
		stakingTx, _, err := anc.NewBTCTxFromHex(delegation.StakingTxHex)
		if err != nil {
			return nil, nil, err
		}

		unbondingTx, _, err := anc.NewBTCTxFromHex(delegation.UndelegationResponse.UnbondingTxHex)
		if err != nil {
			return nil, nil, err
		}
...
// IsDelegationActive method for AnonClientAdapter
func (bca *AnonClientAdapter) IsDelegationActive(stakingTxHash chainhash.Hash) (bool, error) {
...
// IsDelegationVerified method for AnonClientAdapter checks if delegation is in Status verified
func (bca *AnonClientAdapter) IsDelegationVerified(stakingTxHash chainhash.Hash) (bool, error) {
	resp, err := bca.anonClient.BTCDelegation(stakingTxHash.String())
...
// ReportUnbonding method for AnonClientAdapter
func (bca *AnonClientAdapter) ReportUnbonding(
	ctx context.Context,
	stakingTxHash chainhash.Hash,
	stakeSpendingTx *wire.MsgTx,
	inclusionProof *btcstakingtypes.InclusionProof,
	fundingTxs [][]byte) error {
	signer := bca.anonClient.MustGetAddr()

	stakeSpendingBytes, err := anc.SerializeBTCTx(stakeSpendingTx)
	if err != nil {
		return err
	}
	...
	resp, err := bca.anonClient.ReliablySendMsg(ctx, &msg, []*sdkerrors.Error{}, []*sdkerrors.Error{})
	if err != nil && resp != nil {
...
// BtcClientTipHeight method for AnonClientAdapter
func (bca *AnonClientAdapter) BtcClientTipHeight() (uint32, error) {
...
func (bca *AnonClientAdapter) ActivateDelegation(
...
func (bca *AnonClientAdapter) QueryHeaderDepth(headerHash *chainhash.Hash) (uint32, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	clientCtx := client.Context{Client: bca.anonClient.RPCClient}
	...
	if err := retry.Do(func() error {
		...
		if strings.Contains(err.Error(), btclctypes.ErrHeaderDoesNotExist.Error()) {
			return 0, fmt.Errorf("%s: %w", err.Error(), ErrHeaderNotKnownToAnon)
		}
...
func (bca *AnonClientAdapter) Params() (*AnonParams, error) {
	var bccParams *btcctypes.Params
	if err := retry.Do(func() error {
		response, err := bca.anonClient.BTCCheckpointParams()
		if err != nil {
			return err
		}
	...
	return &AnonParams{ConfirmationTimeBlocks: bccParams.BtcConfirmationDepth}, nil
}

func (bca *AnonClientAdapter) CometBFTTipHeight(ctx context.Context) (int64, error) {
	res, err := bca.anonClient.RPCClient.Status(ctx)
	if err != nil {
...
func (bca *AnonClientAdapter) StakingTxHashesByEvent(ctx context.Context, eventType string, criteria string, page, count *int) ([]string, error) {
	res, err := bca.anonClient.RPCClient.TxSearch(ctx, criteria, false, page, count, "asc")
	if err != nil {
...
func (bca *AnonClientAdapter) DelegationsModifiedInBlock(
	ctx context.Context,
	height int64,
	eventTypes []string,
) ([]string, error) {
	...
	res, err := bca.anonClient.RPCClient.BlockResults(ctx, &height)
	if err != nil {
...
// BTCDelegation method for AnonClientAdapter to get BTC delegation
func (bca *AnonClientAdapter) BTCDelegation(stakingTxHash string) (*Delegation, error) {
	resp, err := bca.anonClient.BTCDelegation(stakingTxHash)

	if err != nil {
		return nil, fmt.Errorf("failed to retrieve delegation from anon: %w", err)
	}
