...
BTC staking tracker is a daemon program that relays information between Anon
...
- [Unbonding watcher routine](./unbondingwatcher/): upon observing an unbonding
  ...
  transaction's signature from the staker to Anon, such that Anon will
...
- A Anon client for submitting transactions to and querying Anon.

## Routines

### Unbonding watcher routine

The unbonding watcher routine aims to notify Anon about early unbonding
...
- `fetchDelegations` routine: Periodically, fetch all active BTC delegations
  from Anon, and send them to the `handleDelegations` routine.
...
- Upon seeing an unbonding transaction of a BTC delegation on Bitcoin, notify
  Anon that the corresponding BTC delegation is unbonded via a
...
- a routine that subscribes to Anon and forwards all events about finality
...
to Bitcoin, without affecting other BTC delegations. Anon circumvents the
...
- `selectiveSlashingReporter` routine: upon a slashing transaction,
  2. Retrieve the BTC delegation and its finality provider from Anon.
  ...
  4. Try to extract the finality provider's secret key by comparing the covenant
     ...
     signatures.
  5. If successful, then report the selective slashing offence to Anon, and
