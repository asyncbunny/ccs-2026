...
import (
	...
	eotscmd "github.com/anon-org/finality-provider/eotsmanager/cmd/eotsd/daemon"
...
// in this case, the BTC private key should be extracted by Anon
...
func TestCatchingUp(t *testing.T) {
	...
	currentBlock, err := tm.ANCConsumerClient.QueryLatestBlock(ctx)
	require.NoError(t, err)
...
func TestFinalityProviderEditCmd(t *testing.T) {
	...
	gotFp, err := tm.AnonController.QueryFinalityProvider(ctx, fpIns.GetBtcPk())
	require.NoError(t, err)
	...
	updatedFp, err := tm.AnonController.QueryFinalityProvider(ctx, fpIns.GetBtcPk())
	require.NoError(t, err)
...
func TestFinalityProviderCreateCmd(t *testing.T) {
	// todo(lazar955): breaking change on anc does not allow to create multiple fps with the same key, skipping for now
	t.Skip()
	t.Parallel()
	ctx, cancel := context.WithCancel(t.Context())
	tm, fps := StartManagerWithFinalityProvider(t, 1, ctx)
	...
	data := struct {
		...
		KeyName:                 fpIns.GetConfig().AnonConfig.Key,
		ChainID:                 testChainID,
	...
	fp, err := tm.AnonController.QueryFinalityProvider(ctx, eotsPk.MustToBTCPK())
	require.NoError(t, err)
	require.NotNil(t, fp)
}
...
func TestPrintEotsCmd(t *testing.T) {
	...
	for i := 0; i < r.Intn(10); i++ {
		...
		require.NoError(t, err)
		expected[eotsKeyName] = anctypes.NewBIP340PubKeyFromBTCPK(pk).MarshalHex()
	}
...
func TestEotsdRollbackCmd(t *testing.T) {
	...
	eotsPK, err := anctypes.NewBIP340PubKey(key)
	require.NoError(t, err)
...
func TestBatchSubmissionWithDuplicates(t *testing.T) {
	...
	currentHeight, err := tm.ANCConsumerClient.QueryLatestBlock(t.Context())
	require.NoError(t, err)
	...
	for h := lastVotedHeight; h <= currentHeight.GetHeight(); h++ {
		votes, err := tm.ANCConsumerClient.Client().VotesAtHeight(h)
		require.NoError(t, err)

		block, err := tm.ANCConsumerClient.QueryBlock(t.Context(), h)
		require.NoError(t, err)
	...
	require.Eventually(t, func() bool {
		votes, err := tm.ANCConsumerClient.Client().VotesAtHeight(unvotedBlocks[0].GetHeight())
		if err != nil {
	...
	require.Eventually(t, func() bool {
		votes, err := tm.ANCConsumerClient.Client().VotesAtHeight(unvotedBlocks[1].GetHeight())
		if err != nil {
