...
func DefaultANCConfig() ANCConfig {
	dc := anccfg.DefaultAnonConfig()
	...
	return ANCConfig{
		...
		Debug:          dc.Debug,
		Timeout:        dc.Timeout,
		// Setting this to relatively low value, out current anon client (lens) will
...
func (cfg *ANCConfig) Validate() error {
...
func (cfg *ANCConfig) ToAnonConfig() anccfg.AnonConfig {
	return anccfg.AnonConfig{
		Key:              cfg.Key,
		ChainID:          cfg.ChainID,
		RPCAddr:          cfg.RPCAddr,
