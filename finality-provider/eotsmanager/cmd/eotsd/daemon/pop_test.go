...
var popsToVerify = []daemon.PoPExport{
	daemon.PoPExport{
		...
		NtkSignEotsPk: "GO7xlC+BIypdcQdnIDsM+Ts75X9JKTOkDpXt5t4TSOIt/P1puAHVNhaYbweStVs25J9uRK+4XfrjD0M+t0Qy4g==",
	...
	daemon.PoPExport{
		...
		NtkPublicKey: "AyAU94yfGIa+MPq60oR3QnNn1DJ+9cZZrHDCu4Nx1Uo3",

		NtkSignEotsPk: "kNcxCeqmQFQO//LvqhbzUQwbD3+/FfSbrvfyEa4Xf1MP5YDZp0XKYlOh+wA6dqFsb5IA7Wciz0WRbkGxRwxHVg==",
		EotsSignNtk:   "L9wYf+rg2fQRzy2rau2MTO9V5sEBCSUWtWcxJxweSrvkDpR6tJTwKz3Ba0c/q7yNCT91Ag7H4rKMVhKmyN4tkQ==",

		NtkAddress: "anc1ayrme3m73xv294t50k7v5pfj6pauyps03atepn",
	},
}

func TestPoPValidEotsSignNtk(t *testing.T) {
	t.Parallel()

	for _, pop := range popsToVerify {
		valid, err := daemon.ValidEotsSignNtk(
			pop.EotsPublicKey,
			pop.NtkAddress,
			pop.EotsSignNtk,
		)
...
func TestPoPValidNtkSignEotsPk(t *testing.T) {
	t.Parallel()
	for _, pop := range popsToVerify {
		valid, err := daemon.ValidNtkSignEots(
			pop.NtkPublicKey,
			pop.NtkAddress,
			pop.EotsPublicKey,
			pop.NtkSignEotsPk,
