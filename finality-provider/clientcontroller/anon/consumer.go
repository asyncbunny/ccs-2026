...
var _ api.ConsumerController = &AnonConsumerController{}
var messageIndexRegex = regexp.MustCompile(`message index:\s*(\d+)`)

//nolint:revive
type AnonConsumerController struct {
	ancClient *ancclient.Client
	cfg       *fpcfg.ANCConfig
	logger    *zap.Logger
}

func NewAnonConsumerController(
	cfg *fpcfg.ANCConfig,
	logger *zap.Logger,
) (*AnonConsumerController, error) {
	ancConfig := cfg.ToAnonConfig()
	if err := ancConfig.Validate(); err != nil {
		return nil, fmt.Errorf("invalid config for Anon client: %w", err)
	}

	bc, err := ancclient.New(
		&ancConfig,
		logger,
	)
	if err != nil {
...
func (bc *AnonConsumerController) MustGetTxSigner() string {
...
func (bc *AnonConsumerController) Client() *ancclient.Client {
	return bc.ancClient
}

func (bc *AnonConsumerController) GetKeyAddress() sdk.AccAddress {
	// get key address, retrieves address based on key name which is configured in
	// cfg *stakercfg.ANCConfig. If this fails, it means we have misconfiguration problem
	// and we should panic.
	// This is checked at the start of AnonConsumerController, so if it fails something is really wrong
...
func (bc *AnonConsumerController) reliablySendMsg(ctx context.Context, msg sdk.Msg, expectedErrs []*sdkErr.Error, unrecoverableErrs []*sdkErr.Error) (*anonclient.RelayerTxResponse, error) {
	return bc.reliablySendMsgs(ctx, []sdk.Msg{msg}, expectedErrs, unrecoverableErrs)
}

func (bc *AnonConsumerController) reliablySendMsgs(ctx context.Context, msgs []sdk.Msg, expectedErrs []*sdkErr.Error, unrecoverableErrs []*sdkErr.Error) (*anonclient.RelayerTxResponse, error) {
...
// CommitPubRandList commits a list of Schnorr public randomness via a MsgCommitPubRand to Anon
// it returns tx hash and error
func (bc *AnonConsumerController) CommitPubRandList(
	ctx context.Context,
	req *api.CommitPubRandListRequest,
) (*types.TxResponse, error) {
	msg := &finalitytypes.MsgCommitPubRandList{
		Signer:      bc.MustGetTxSigner(),
		FpBtcPk:     anctypes.NewBIP340PubKeyFromBTCPK(req.FpPk),
		StartHeight: req.StartHeight,
		NumPubRand:  req.NumPubRand,
		Commitment:  req.Commitment,
		Sig:         anctypes.NewBIP340SignatureFromBTCSig(req.Sig),
	}
...
// SubmitBatchFinalitySigs submits a batch of finality signatures to Anon
func (bc *AnonConsumerController) SubmitBatchFinalitySigs(
	ctx context.Context,
	...
	for i, b := range req.Blocks {
		...
		msg := &finalitytypes.MsgAddFinalitySig{
			...
			PubRand:      anctypes.NewSchnorrPubRandFromFieldVal(req.PubRandList[i]),
			Proof:        &cmtProof,
			BlockAppHash: b.GetHash(),
			FinalitySig:  anctypes.NewSchnorrEOTSSigFromModNScalar(req.Sigs[i]),
		}
...
func (bc *AnonConsumerController) QueryFinalityProviderHasPower(
	_ context.Context,
	req *api.QueryFinalityProviderHasPowerRequest,
) (bool, error) {
	res, err := bc.ancClient.FinalityProviderPowerAtHeight(
		anctypes.NewBIP340PubKeyFromBTCPK(req.FpPk).MarshalHex(),
		req.BlockHeight,
	)
	if err != nil {
...
func (bc *AnonConsumerController) QueryLatestFinalizedBlock(_ context.Context) (types.BlockDescription, error) {
...
func (bc *AnonConsumerController) QueryBlocks(_ context.Context, req *api.QueryBlocksRequest) ([]types.BlockDescription, error) {
...
func (bc *AnonConsumerController) queryLatestBlocks(startKey []byte, count uint64, status finalitytypes.QueriedBlockStatus, reverse bool) ([]types.BlockDescription, error) {
	...
	res, err := bc.ancClient.ListBlocks(status, pagination)
	if err != nil {
...
func (bc *AnonConsumerController) QueryBlock(_ context.Context, height uint64) (types.BlockDescription, error) {
	res, err := bc.ancClient.Block(height)
	if err != nil {
...
func (bc *AnonConsumerController) QueryLastPubRandCommit(_ context.Context, fpPk *btcec.PublicKey) (types.PubRandCommit, error) {
	fpBtcPk := anctypes.NewBIP340PubKeyFromBTCPK(fpPk)
	...
	res, err := bc.ancClient.ListPubRandCommit(fpBtcPk.MarshalHex(), pagination)
	if err != nil {
	...
	var commit *AnonPubRandCommit
	for height, commitRes := range res.PubRandCommitMap {
		commit = &AnonPubRandCommit{
			StartHeight: height,
...
func (bc *AnonConsumerController) QueryIsBlockFinalized(_ context.Context, height uint64) (bool, error) {
	res, err := bc.ancClient.Block(height)
	if err != nil {
...
func (bc *AnonConsumerController) QueryFinalityActivationBlockHeight(_ context.Context) (uint64, error) {
	res, err := bc.ancClient.FinalityParams()
	if err != nil {
...
func (bc *AnonConsumerController) QueryFinalityProviderHighestVotedHeight(_ context.Context, fpPk *btcec.PublicKey) (uint64, error) {
...
func (bc *AnonConsumerController) QueryLatestBlock(ctx context.Context) (types.BlockDescription, error) {
...
func (bc *AnonConsumerController) queryCometBestBlock(ctx context.Context) (*types.BlockInfo, error) {
	ctx, cancel := context.WithTimeout(ctx, bc.cfg.Timeout)
	// this will return 20 items at max in the descending order (highest first)
	chainInfo, err := bc.ancClient.RPCClient.BlockchainInfo(ctx, 0, 0)
	defer cancel()
...
func (bc *AnonConsumerController) QueryFinalityProviderStatus(_ context.Context, fpPk *btcec.PublicKey) (*api.FinalityProviderStatusResponse, error) {
	fpPubKey := anctypes.NewBIP340PubKeyFromBTCPK(fpPk)
	res, err := bc.ancClient.FinalityProvider(fpPubKey.MarshalHex())
	if err != nil {
...
func (bc *AnonConsumerController) Close() error {
...
func (bc *AnonConsumerController) UnjailFinalityProvider(ctx context.Context, fpPk *btcec.PublicKey) (*types.TxResponse, error) {
...
func (bc *AnonConsumerController) QueryPubRandCommitList(_ context.Context, fpPk *btcec.PublicKey, startHeight uint64) ([]types.PubRandCommit, error) {
	fpBtcPk := anctypes.NewBIP340PubKeyFromBTCPK(fpPk)
	...
	commitList := make([]*AnonPubRandCommit, 0)
...
func (bc *AnonConsumerController) reliablySendMsgsResendingOnMsgErr(
	ctx context.Context,
	msgs []sdk.Msg,
	expectedErrs []*sdkErr.Error,
	unrecoverableErrs []*sdkErr.Error,
	...
	for i := uint64(0); i < maxRetries; i++ {
		...
		res, errSendMsg := bc.ancClient.ReliablySendMsgs(ctx, msgs, nil, allUnrecoverable)
		if errSendMsg != nil {
