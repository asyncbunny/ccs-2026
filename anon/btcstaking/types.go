...
// Package responsible for different kinds of btc scripts used by anon
...
// anonScriptPaths contains all possible anon script paths
// not every anon output will contain all of those paths
type anonScriptPaths struct {
...
func newAnonScriptPaths(
	stakerKey *btcec.PublicKey,
	fpKeys []*btcec.PublicKey,
	covenantKeys []*btcec.PublicKey,
	covenantQuorum uint32,
	lockTime uint16,
) (*anonScriptPaths, error) {
	if stakerKey == nil {
...
// BuildStakingInfo builds all Anon specific BTC scripts that must
...
// obey parameters expected by Anon chain.
func BuildStakingInfo(
	...
	anonScripts, err := newAnonScriptPaths(
		stakerKey,
		fpKeys,
		covenantKeys,
		covenantQuorum,
	...
	unbondingPaths = append(unbondingPaths, anonScripts.timeLockPathScript)
	unbondingPaths = append(unbondingPaths, anonScripts.unbondingPathScript)
	unbondingPaths = append(unbondingPaths, anonScripts.slashingPathScript)

	timeLockLeafHash := txscript.NewBaseTapLeaf(anonScripts.timeLockPathScript).TapHash()
	unbondingPathLeafHash := txscript.NewBaseTapLeaf(anonScripts.unbondingPathScript).TapHash()
	slashingLeafHash := txscript.NewBaseTapLeaf(anonScripts.slashingPathScript).TapHash()
...
// BuildUnbondingInfo builds all Anon specific BTC scripts that must
...
func ParseBlkHeightAndPubKeyFromStoreKey(key []byte) (blkHeight uint64, fpBTCPK *anc.BIP340PubKey, err error) {
	sizeBigEndian := 8
	if len(key) < sizeBigEndian+1 {
		return 0, nil, fmt.Errorf("key not long enough to parse block height and BIP340PubKey: %s", key)
	}

	fpBTCPK, err = anc.NewBIP340PubKey(key[sizeBigEndian:])
	if err != nil {
		return 0, nil, fmt.Errorf("failed to parse pub key from key %w: %w", anc.ErrUnmarshal, err)
	}
