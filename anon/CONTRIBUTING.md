...
* Fork the repo (core developers must create a branch directly in the Anon
repo), branch from the HEAD of `main`, make some commits, and submit a PR to
`main`.
* For developers who are core contributors and are working within the Anon
...
Tests can be executed by running `make test` at the top level of the Anon
...
If you open a PR in Anon, it is mandatory to update the relevant
...
The main branch of every Anon repository should just build with `go get`,
...
When dependencies in Anon `go.mod` are changed, it is generally accepted
...
Anon follows [semantic versioning](https://semver.org), but with the some
