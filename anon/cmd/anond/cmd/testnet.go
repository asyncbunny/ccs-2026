...
import (
	...
	"github.com/anon-org/anon/v4/cmd/anond/cmd/genhelpers"
...
func TestnetCmd(mbm module.BasicManager, genBalIterator banktypes.GenesisBalancesIterator) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "testnet",
		Short: "Initialize files for a anon testnet",
...
Example:
	anond testnet --v 4 --output-dir ./output --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "anond", "Home directory of the node's daemon configuration")
	cmd.Flags().String(flagStartingIPAddress, "192.168.0.1", "Starting IP address (192.168.0.1 results in persistent peers list ID0@192.168.0.1:46656, ID1@192.168.0.2:46656, ...)")
	cmd.Flags().String(server.FlagMinGasPrices, fmt.Sprintf("0.000006%s", appparams.BaseCoinUnit), "Minimum gas prices to accept for transactions; All fees in a tx must meet this minimum (e.g. 0.001anc)")
	cmd.Flags().String(flags.FlagKeyringBackend, flags.DefaultKeyringBackend, "Select keyring's backend (os|file|test)")
	cmd.Flags().String(flags.FlagKeyType, string(hd.Secp256k1Type), "Key signing algorithm to generate keys for")
	cmd.Flags().String(flagBtcNetwork, string(anc.BtcSimnet), "Bitcoin network to use. Available networks: simnet, testnet, regtest, mainnet")
...
func InitTestnet(
	...
	anonConfig := DefaultAnonAppConfig()
	anonConfig.MinGasPrices = minGasPrices
	anonConfig.API.Enable = true
	anonConfig.API.Address = "tcp://0.0.0.0:1317"
	anonConfig.Telemetry.Enabled = true
	anonConfig.Telemetry.PrometheusRetentionTime = 60
	anonConfig.Telemetry.EnableHostnameLabel = false
	anonConfig.Telemetry.GlobalLabels = [][]string{{"chain_id", chainID}}
	// BTC related config. Default values "simnet" and "BBT1"
	anonConfig.BtcConfig.Network = btcNetwork
	// Explorer related config. Allow CORS connections.
	anonConfig.API.EnableUnsafeCORS = true
	anonConfig.GRPC.Enable = true
	anonConfig.GRPC.Address = "0.0.0.0:9090"

	// Disable IAVL cache by default as Anon leaf nodes can be large, and in case
	// of big cache values, Anon node can run out of memory.
	anonConfig.IAVLCacheSize = 0
	anonConfig.IAVLDisableFastNode = true
	...
	for i := 0; i < numValidators; i++ {
		...
		customTemplate := DefaultAnonTemplate()
		srvconfig.SetConfigTemplate(customTemplate)
		srvconfig.WriteConfigFile(filepath.Join(nodeDir, "config/app.toml"), anonConfig)
