...
func initAppConfig() (string, interface{}) {
	...
	anonTemplate := DefaultAnonTemplate()
	return anonTemplate, anonConfig
}
...
func appExport(
	...
	if height != -1 {
		anonApp = app.NewAnonApp(logger, db, traceStore, false, map[int64]bool{}, uint(1), &blsSigner, appOpts, app.EmptyWasmOpts)

		if err = anonApp.LoadHeight(height); err != nil {
			return servertypes.ExportedApp{}, fmt.Errorf("failed to load height: %w", err)
		}
	} else {
		anonApp = app.NewAnonApp(logger, db, traceStore, true, map[int64]bool{}, uint(1), &blsSigner, appOpts, app.EmptyWasmOpts)
	}

	return anonApp.ExportAppStateAndValidators(forZeroHeight, jailAllowedAddrs, modulesToExport)
}
