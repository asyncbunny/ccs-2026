...
func Test_CmdGenTx(t *testing.T) {
	...
	anc := app.NewAnonAppWithCustomOptions(t, false, signer, app.SetupOptions{
		Logger:             logger,
		DB:                 dbm.NewMemDB(),
		InvCheckPeriod:     0,
	...
	err = genutiltest.ExecInitCmd(anc.BasicModuleManager, home, anc.AppCodec())
	require.NoError(t, err)

	serverCtx := server.NewContext(viper.New(), cfg, logger)
	clientCtx := client.Context{}.
		WithCodec(anc.AppCodec()).
		WithHomeDir(home).
		WithTxConfig(anc.TxConfig())
	...
	genTxCmd := genhelpers.GenTxCmd(anc.BasicModuleManager, anc.TxConfig(), banktypes.GenesisBalancesIterator{}, app.DefaultNodeHome, authcodec.NewBech32Codec(params.Bech32PrefixValAddr))
