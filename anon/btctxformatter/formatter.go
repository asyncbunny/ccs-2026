...
func encodeHeader(tag AnonTag, version FormatVersion, halfNumber uint8) []byte {
	var data = []byte(tag)
...
func GetCheckpointData(
	...
	// At this point this is probable anon data, strip the header and return data
	// to the caller
...
// IsAnonCheckpointData Checks if given bytearray is potential anon data,
// if it is then returns index of data along side with data itself
func IsAnonCheckpointData(
	tag AnonTag,
	version FormatVersion,
	data []byte,
) (*AnonData, error) {
	...
	for idx < NumberOfParts {
		...
		if err == nil {
			bd := AnonData{Data: data, Index: idx}
			return &bd, nil
		}
	...
	return nil, errors.New("not valid anon data")
}
