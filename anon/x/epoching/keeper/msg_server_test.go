...
func TestMsgWrappedDelegate(t *testing.T) {
	...
	// enter 1st epoch, in which ANC starts handling validator-related msgs
...
func TestExponentiallyEventsEndEpochQueuedMessages(t *testing.T) {
	...
	initialBalance := h.App.BankKeeper.GetBalance(ctx, userAddr, "uanc")
	totalDiff := sdkmath.ZeroInt()
	for i := 0; i < numDelMessages; i++ {
		balanceBefore := h.App.BankKeeper.GetBalance(ctx, userAddr, "uanc")
		msgDel := types.NewMsgWrappedDelegate(
			stakingtypes.NewMsgDelegate(
				h.GenAccs[0].GetAddress().String(),
				valAddr.String(),
				sdk.NewCoin("uanc", sdkmath.NewInt(100000)),
			),
		)
		...
		h.NoError(err)
		require.NotNil(t, res)
		balanceAfter := h.App.BankKeeper.GetBalance(ctx, userAddr, "uanc")
		diff := balanceBefore.Amount.Sub(balanceAfter.Amount)
		totalDiff = totalDiff.Add(diff)
	}

	finalBalance := h.App.BankKeeper.GetBalance(ctx, userAddr, "uanc")
