...
Anon. This reduces the frequency of Anon sending checkpoints to Bitcoin,
thereby reducing Anon's operation cost and its footprint on Bitcoin.
...
- [Concepts](#concepts)
  ...
  - [Anon's Epoching module design](#anons-epoching-module-design)
- [States](#states)
...
1. Anon's Bitcoin Timestamping protocol requires checkpointing the validator
...
implemented. Anon has implemented the Epoching module, catering to specific
design goals such as checkpointing epochs. In addition, Anon implements
...
### Anon's Epoching module design

Anon implements the Epoching module in order to reduce the frequency of
...
**Disabling functionalities of the Staking module.** Anon disables two
...
set updates to once per epoch, Anon disables the validator set update
...
In order to disable the two functionalities, Anon disables Staking module's
...
by the Staking module, all of commands under `anond tx staking` were
removed.
...
**Bitcoin-assisted Unbonding.** Anon implements the Bitcoin-assisted
...
that have been unbonding for 21 days, and is invoked upon every block. Anon
...
[object](../../proto/anon/epoching/v1/params.proto) defined as follows:
...
[object](../../proto/anon/epoching/v1/epoching.proto) representing the epoch
metadata.
...
message Epoch {
  ...
  // Anon needs to remember the last header's time of each epoch to complete
...
[object](../../proto/anon/epoching/v1/epoching.proto) representing this
queued message.
...
message QueuedMessage {
  ...
  bytes msg_id = 2;
  // block_height is the height when this msg is submitted to Anon
  uint64 block_height = 3;
  // block_time is the timestamp when this msg is submitted to Anon
...
[proto/anon/epoching/v1/tx.proto](../../proto/anon/epoching/v1/tx.proto).
...
Anon disables the Staking module's EndBlocker to avoid validator set updates
...
Anon disables the Staking module's `EndBlocker` [as
follows](../../app/app.go).
...
Upon `BeginBlocker`, the Epoching module of each Anon node will [execute the
following](./abci.go):
...
Upon `EndBlocker`, the Epoching module of each Anon node will [execute the
...
[parameter](../../proto/anon/btccheckpoint/v1/params.proto) in the
