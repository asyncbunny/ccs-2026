...
func (k Keeper) CreateBTCDelegation(ctx sdk.Context, parsedMsg *types.ParsedCreateDelegationMessage) error {
	...
	// - are known to Anon
	// - there's only 1 Anon finality provider
	// - is not slashed
	err := k.validateStakedFPs(ctx, parsedMsg.FinalityProviderKeys.PublicKeysAncFormat)
	if err != nil {
		return err
	}
	...
	newBTCDel := &types.BTCDelegation{
		...
		FpBtcPkList:      parsedMsg.FinalityProviderKeys.PublicKeysAncFormat,
...
// - are not slashed
// - are not deleted
func (k Keeper) validateStakedFPs(ctx sdk.Context, fpBTCPKs []anc.BIP340PubKey) error {
