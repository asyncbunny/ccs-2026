...
func TestExportGenesis(t *testing.T) {
	...
	for i := range gs.FinalityProviders {
		...
		err := k.SetFpAncAddr(ctx, fp.Address())
		require.NoError(t, err)
	...
	for _, ch := range chainsHeight {
		...
		header.Height = int64(ch.BlockHeightAnc)
	...
	for _, deletedFpBtcPkHex := range gs.DeletedFpsBtcPkHex {
		fpBtcPk, err := anc.NewBIP340PubKeyFromHex(deletedFpBtcPkHex)
		require.NoError(t, err)
...
func setupTest(t *testing.T) (sdk.Context, *helper.Helper, *types.GenesisState) {
	...
	chainsHeight := make([]*types.BlockHeightAncToBtc, 0)
	// creates the first as it starts already with an chain height from the helper.
	chainsHeight = append(chainsHeight, &types.BlockHeightAncToBtc{
		BlockHeightAnc: 1,
		BlockHeightBtc: 0,
	})
	...
	for i := range fps {
		...
		fpsAncAddr = append(fpsAncAddr, fp.Addr)
