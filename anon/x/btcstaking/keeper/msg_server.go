...
func (ms msgServer) EditFinalityProvider(goCtx context.Context, req *types.MsgEditFinalityProvider) (*types.MsgEditFinalityProviderResponse, error) {
	...
	// ensure the signer corresponds to the finality provider's Anon address
	if !strings.EqualFold(fpAddr.String(), fp.Addr) {
		return nil, status.Errorf(codes.PermissionDenied, "the signer does not correspond to the finality provider's Anon address")
	}
...
func (ms msgServer) AddCovenantSigs(goCtx context.Context, req *types.MsgAddCovenantSigs) (*types.MsgAddCovenantSigsResponse, error) {
	...
	*/
	unbondingMsgTx, err := anc.NewBTCTxFromBytes(btcDel.BtcUndelegation.UnbondingTx)
	if err != nil {
...
func getFundingTxTransactions(txs [][]byte) ([]*wire.MsgTx, error) {
	...
	for i, tx := range txs {
		fundingTx, err := anc.NewBTCTxFromBytes(tx)
...
// this effectively proves that the BTC delegator wants to unbond and Anon
...
func (ms msgServer) BTCUndelegate(goCtx context.Context, req *types.MsgBTCUndelegate) (*types.MsgBTCUndelegateResponse, error) {
	...
	stakeSpendingTx, err := anc.NewBTCTxFromBytes(req.StakeSpendingTx)
	if err != nil {
...
// CheckDuplicatedFpAncAddr verifies if there isn't another FP registered with the same anon address
func (k Keeper) CheckDuplicatedFpAncAddr(ctx context.Context, fpAddr sdk.AccAddress) error {
	found, err := k.HasFpRegistered(ctx, fpAddr)
	if err != nil {
		return status.Errorf(codes.InvalidArgument, "invalid address %s: %v", fpAddr.String(), err)
	}
	if found {
		return types.ErrFpRegistered.Wrapf("there is already an finality provider registered with the same anon address: %s", fpAddr.String())
	}
	return nil
}
...
func hasSufficientCovenantOverlap(
	covCommittee1,
	covCommittee2 []anc.BIP340PubKey,
