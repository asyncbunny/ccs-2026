...
func (k Keeper) AddFinalityProvider(goCtx context.Context, msg *types.MsgCreateFinalityProvider) error {
	...
	if err := k.CheckDuplicatedFpAncAddr(ctx, fpAddr); err != nil {
		return err
	}
	...
	if err := k.SetFpAncAddr(goCtx, fp.Address()); err != nil {
		return err
	}
...
func (k Keeper) SetFpAncAddr(ctx context.Context, fpAddr sdk.AccAddress) error {
	return k.fpAncAddr.Set(ctx, fpAddr)
}
...
func (k Keeper) IsFinalityProviderDeleted(ctx context.Context, fpBtcPk *anc.BIP340PubKey) bool {
...
func (k Keeper) SoftDeleteFinalityProvider(ctx context.Context, fpBtcPk *anc.BIP340PubKey) error {
	return k.finalityProvidersDeleted.Set(ctx, fpBtcPk.MustMarshal())
}

// HasFpRegistered returns if there is already an finality provider registered with the anon address
func (k Keeper) HasFpRegistered(ctx context.Context, fpAncAddr sdk.AccAddress) (bool, error) {
	return k.fpAncAddr.Has(ctx, fpAncAddr)
}
...
func (k Keeper) SlashFinalityProvider(ctx context.Context, fpBTCPK []byte) error {
	...
	fp.SlashedAnonHeight = uint64(sdk.UnwrapSDKContext(ctx).HeaderInfo().Height)
