...
import (
	...
	testutilevents "github.com/anon-org/anon/v4/testutil/events"
	testhelper "github.com/anon-org/anon/v4/testutil/helper"
...
func FuzzMsgCreateFinalityProvider(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		// tries to create another fp with same anc address as an registered one
		fp, err := datagen.GenRandomFinalityProvider(r)
		require.NoError(t, err)
		...
		_, dupFpAncAddrErr := h.MsgServer.CreateFinalityProvider(h.Ctx, msg)
		require.EqualError(t, dupFpAncAddrErr, types.ErrFpRegistered.Wrapf("there is already an finality provider registered with the same anon address: %s", dupFpAddr).Error())
	})
}
...
func FuzzMsgEditFinalityProvider(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		_, err = h.MsgServer.EditFinalityProvider(h.Ctx, msg)
		require.Equal(h.T(), err, status.Errorf(codes.PermissionDenied, "the signer does not correspond to the finality provider's Anon address"))
...
func TestCreateBTCDelegation_DeletedFinalityProvider(t *testing.T) {
	...
	// generate and insert new finality provider
	_, fpPK, _ := h.CreateFinalityProvider(r)
	fpBIP340PK := anc.NewBIP340PubKeyFromBTCPK(fpPK)
	...
	serializedStakingTx, err := anc.SerializeBTCTx(testStakingInfo.StakingTx)
	h.NoError(err)
	...
	h.NoError(err)
	serializedUnbondingTx, err := anc.SerializeBTCTx(testUnbondingInfo.UnbondingTx)
	h.NoError(err)

	// construct message
	stakerPk := delSK.PubKey()
	stPk := anc.NewBIP340PubKeyFromBTCPK(stakerPk)
	...
	msgCreateBTCDel := &types.MsgCreateBTCDelegation{
		...
		BtcPk:                         stPk,
		FpBtcPkList:                   []anc.BIP340PubKey{*fpBIP340PK},
		Pop:                           pop,
...
func createActiveBtcDel(t *testing.T, btcLightclientTipHeight uint32) (*testutil.Helper, *rand.Rand, btcctypes.Params, string) {
	...
	decodeStakingTxHash, err := anc.NewBTCTxFromBytes(decodeStakingTxHashBz)
	h.NoError(err)
...
func TestDoNotAllowDelegationWithoutFinalityProvider(t *testing.T) {
	...
	*/
	...
	msgCreateBTCDel := &types.MsgCreateBTCDelegation{
		...
		FpBtcPkList:                   []anc.BIP340PubKey{*fp.BtcPk},
...
func createNDelegationsForFinalityProvider(
	...
	for i := 0; i < numDelegations; i++ {
		...
		del, err := datagen.GenRandomBTCDelegation(
			...
			t,
			net,
			[]anc.BIP340PubKey{*anc.NewBIP340PubKeyFromBTCPK(fpPK)},
			delSK,
			covenatnSks,
			covenantPks,
			quorum,
...
func TestBtcStakeExpansion(t *testing.T) {
	testCases := []struct {
		...
		setupOriginalDelegation func(t *testing.T, h *testutil.Helper, r *rand.Rand, covenantSKs []*btcec.PrivateKey, anonFPPK *btcec.PublicKey, delSK *btcec.PrivateKey, stakingValue int64) (*types.BTCDelegation, string, uint32)
	}{
		{
		...
		},
		{
			...
			setupOriginalDelegation: func(t *testing.T, h *testutil.Helper, r *rand.Rand, covenantSKs []*btcec.PrivateKey, anonFPPK *btcec.PublicKey, delSK *btcec.PrivateKey, stakingValue int64) (*types.BTCDelegation, string, uint32) {
				...
				originalStkTx, err := anc.NewBTCTxFromBytes(originalDel.GetStakingTx())
				require.NoError(t, err)
				...
				fundingTxBz, err := anc.SerializeBTCTx(firstExpansionFundingTx)
				h.NoError(err)
				msg := &types.MsgBTCUndelegate{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			_, anonFPPK, _ := h.CreateFinalityProvider(r)
			...
			if tc.diffDelKey {
				...
				h.NoError(err)
				prevDel, prevDelStakingTxHash, lcTip = tc.setupOriginalDelegation(t, h, r, covenantSKs, anonFPPK, ogDelSK, stakingValue)
			} else {
				prevDel, prevDelStakingTxHash, lcTip = tc.setupOriginalDelegation(t, h, r, covenantSKs, anonFPPK, delSK, stakingValue)
			}
			...
			if len(tc.expStkExpErr) != 0 {
				require.Error(t, err)
				require.Contains(t, err.Error(), tc.expStkExpErr)
				return
			} else {
				require.NoError(t, err)
			}
			...
			prevStkTx, err := anc.NewBTCTxFromBytes(prevDel.GetStakingTx())
			require.NoError(t, err)
