...
func (k Keeper) ExportGenesis(ctx context.Context) (*types.GenesisState, error) {
	...
	fpAncAddr, err := k.fpAncAddrs(ctx)
	if err != nil {
		return nil, err
	}
...
func (k Keeper) blockHeightChains(ctx context.Context) []*types.BlockHeightAncToBtc {
	iter := k.btcHeightStore(ctx).Iterator(nil, nil)
	defer iter.Close()

	blocks := make([]*types.BlockHeightAncToBtc, 0)
	for ; iter.Valid(); iter.Next() {
		...
		blocks = append(blocks, &types.BlockHeightAncToBtc{
			BlockHeightAnc: sdk.BigEndianToUint64(iter.Key()),
...
func (k Keeper) setBlockHeightChains(ctx context.Context, blocks *types.BlockHeightAncToBtc) {
	store := k.btcHeightStore(ctx)
	store.Set(sdk.Uint64ToBigEndian(blocks.BlockHeightAnc), sdk.Uint64ToBigEndian(uint64(blocks.BlockHeightBtc)))
}
...
func (k Keeper) deletedFps(ctx context.Context) ([]string, error) {
	...
	err := k.finalityProvidersDeleted.Walk(ctx, nil, func(key []byte) (stop bool, err error) {
		fpBtcPk, err := anc.NewBIP340PubKey(key)
		if err != nil {
			return true, err
		}
...
func (k Keeper) fpAncAddrs(ctx context.Context) ([]string, error) {
	entries := make([]string, 0)

	iterator, err := k.fpAncAddr.Iterate(ctx, nil)
	if err != nil {
		return nil, err
	}
	defer iterator.Close()
...
func parseBIP340PubKeysFromStoreKey(key []byte) (fpBTCPK, delBTCPK *anc.BIP340PubKey, err error) {
	if len(key) < anc.BIP340PubKeyLen*2 {
		return nil, nil, fmt.Errorf("key not long enough to parse two BIP340PubKey: %s", key)
	}

	fpBTCPK, err = anc.NewBIP340PubKey(key[:anc.BIP340PubKeyLen])
	if err != nil {
		return nil, nil, fmt.Errorf("failed to parse pub key from key %w: %w", anc.ErrUnmarshal, err)
	}

	delBTCPK, err = anc.NewBIP340PubKey(key[anc.BIP340PubKeyLen:])
	if err != nil {
		return nil, nil, fmt.Errorf("failed to parse pub key from key %w: %w", anc.ErrUnmarshal, err)
	}
