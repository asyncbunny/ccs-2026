...
func FuzzVerifyInclusionProofAndGetHeight(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		headerBytes, err := anctypes.NewBTCHeaderBytesFromBytes(btcHeaderWithProof.HeaderBytes)
		...
		t.Run("reject coinbase transaction", func(t *testing.T) {
			...
			headerBytes := anctypes.NewBTCHeaderBytesFromBlockHeader(&inclusionHeader)
			...
			inclusionHeaderHashBytes := anctypes.NewBTCHeaderHashBytesFromChainhash(&inclusionHeaderHash)
