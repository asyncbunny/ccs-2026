...
func NewCreateFinalityProviderCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			// get BTC PK
			btcPK, err := anc.NewBIP340PubKeyFromHex(args[0])
			if err != nil {
				return err
			}
...
func NewAddBTCDelegationInclusionProofCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: strings.TrimSpace(
			`Add a signature on the unbonding tx of a BTC delegation identified by a given staking tx hash signed by the delegator. The signature proves that delegator wants to unbond, and Anon will consider the BTC delegation unbonded.`, // TODO: example
		),
		RunE: func(cmd *cobra.Command, args []string) error {
			clientCtx, err := client.GetClientTxContext(cmd)
			if err != nil {
...
func NewAddCovenantSigsCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			covPK, err := anc.NewBIP340PubKeyFromHex(args[0])
			if err != nil {
			...
			unbondingTxSig, err := anc.NewBIP340SignatureFromHex(args[3])
			if err != nil {
				return err
			}
			...
			// stake expansion
			if len(args) == 6 {
				stkExpSig, err := anc.NewBIP340SignatureFromHex(args[5])
				if err != nil {
					return err
				}
...
func NewBTCUndelegateCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, bytes, err := anc.NewBTCTxFromHex(args[1])
			if err != nil {
				return err
			}
			...
			for _, fundingTxHex := range strings.Split(args[3], ",") {
				_, fundingTxBytes, err := anc.NewBTCTxFromHex(fundingTxHex)
				if err != nil {
...
func parseArgsIntoMsgCreateBTCDelegation(args []string) (*types.MsgCreateBTCDelegation, error) {
	...
	fpPKs := make([]anc.BIP340PubKey, len(fpPKStrs))
	for i := range fpPKStrs {
		fpPK, err := anc.NewBIP340PubKeyFromHex(fpPKStrs[i])
		if err != nil {
			return nil, err
		}
		fpPKs[i] = *fpPK
	...
	delegatorSlashingSig, err := anc.NewBIP340SignatureFromHex(args[8])
	if err != nil {
		return nil, err
	}

	// get unbonding tx
	_, unbondingTxBytes, err := anc.NewBTCTxFromHex(args[9])
	if err != nil {
		return nil, err
	}
	...
	delegatorUnbondingSlashingSig, err := anc.NewBIP340SignatureFromHex(args[13])
	if err != nil {
		return nil, err
	}
