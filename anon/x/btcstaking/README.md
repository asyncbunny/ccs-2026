...
Anon's Bitcoin Staking protocol allows bitcoin holders to _trustlessly_ stake
their bitcoin for providing economic security to the Anon chain and other
...
3. Once the staking transaction is confirmed on Bitcoin, the BTC staker sends
   ...
   unbonding transaction, and unbonding slashing transaction to Anon.
...
and then request the Anon blockchain to activate the stake.
...
1. The BTC staker constructs the following transactions (whose specifications
   can be found [here](../../docs/staking-script.md)) and sends them on Anon
...
Anon's [Finality module](../finality) will make use of the voting power table
...
[object](../../proto/anon/btcstaking/v1/params.proto) defined as follows:
...
message Params {
  ...
  repeated bytes covenant_pks = 1 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
...
[object](../../proto/anon/btcstaking/v1/btcstaking.proto) representing a
finality provider.
...
message FinalityProvider {
    ...
    bytes btc_pk = 4 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
    // pop is the proof of possession of the btc_pk, where the BTC
    // private key signs the bech32 anc addr of the finality provider.
    ProofOfPossessionBTC pop = 5;
    // slashed_anon_height indicates the Anon height when
    // the finality provider is slashed.
    // if it's 0 then the finality provider is not slashed
    uint64 slashed_anon_height = 6;
...
[structure](../../proto/anon/btcstaking/v1/btcstaking.proto) includes
...
message BTCDelegation {
    ...
    // the PK follows encoding in BIP-340 spec
    bytes btc_pk = 2 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
    // pop is the proof of possession of anon_pk and btc_pk
    ...
    repeated bytes fp_btc_pk_list = 4 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
    ...
    bytes delegator_sig = 12 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
...
message BTCUndelegation {
    ...
    bytes delegator_slashing_sig = 3 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
    ...
    // by covenant members
    // It must be provided after processing undelegate message by Anon
...
[object](../../proto/anon/btcstaking/v1/btcstaking.proto) that contains
...
defined at
[proto/anon/btcstaking/v1/tx.proto](../../proto/anon/btcstaking/v1/tx.proto).
...
Upon `MsgCreateFinalityProvider`, a Anon node will execute as follows:

1. Verify a [proof of
   possession](https://rist.tech.cornell.edu/papers/pkreg.pdf) indicating the
   ownership of the Bitcoin secret keys over the Anon address.
...
needs to be submitted by using the Anon account registered in the finality
provider.
...
Upon `MsgEditFinalityProvider`, a Anon node will execute as follows:
...
message MsgCreateBTCDelegation {
  ...
  bytes btc_pk = 3 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
  ...
  bytes delegator_slashing_sig = 10 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
  ...
  bytes delegator_unbonding_slashing_sig = 15 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
}
```

Upon `MsgCreateBTCDelegation`, a Anon node will execute as follows:
...
4. Verify a [proof of
   possession](https://rist.tech.cornell.edu/papers/pkreg.pdf) indicating the
   ownership of the Bitcoin secret key over the Anon staker address.
5. Ensure the staking transaction is not duplicated with an existing BTC
   delegation known to Anon.
...
Bitcoin blockchain.
This message is utilised for notifying the Anon blockchain
...
Upon `MsgAddBTCDelegationInclusionProof`, a Anon
node will execute as follows:
1. Ensure that the staking transaction is tracked by Anon.
...
message MsgAddCovenantSigs {
  ...
  bytes pk = 2  [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340PubKey" ];
  // staking_tx_hash is the hash of the staking tx.
  // It uniquely identifies a BTC delegation
  ...
  // unbonding_tx_sig is the signature of the covenant on the unbonding tx submitted to anon
  // the signature follows encoding in BIP-340 spec
  bytes unbonding_tx_sig = 5 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
  // slashing_unbonding_tx_sigs is a list of adaptor signatures of the covenant
  // on slashing tx corresponding to unbonding tx submitted to anon
  // the order of sigs should respect the order of finality providers
  // of the corresponding delegation
...
Upon `AddCovenantSigs`, a Anon node will execute as follows:

1. Ensure the given BTC delegation is known to Anon.
...
message MsgBTCUndelegate {
  ...
  // It uniquely identifies a BTC delegation
  string staking_tx_hash = 2;
  // unbonding_tx_sig is the signature of the staker on the unbonding tx submitted to anon
  // the signature follows encoding in BIP-340 spec
  bytes unbonding_tx_sig = 3 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
}
```

Upon `BTCUndelegate`, a Anon node will execute as follows:
...
2. Verify the Schnorr signature on the unbonding transaction from the BTC
   ...
   wants to unbond this BTC delegation from Anon.
3. Add the Schnorr signature to the `BTCDelegation` in the BTC delegation
   storage. Anon will consider this BTC delegation to be unbonded from now
   on.
...
Upon `MsgSelectiveSlashingEvidence`, a Anon node will execute as follows:
...
at `proto/anon/btcstaking/v1/events.proto`.
...
message EventFinalityProviderCreated {
  ...
  string btc_pk_hex = 1;
  // addr is the anon address to receive commission from delegations.
  string addr = 2;
...
Available Queries:
Parameters
Endpoint: `/anon/btcstaking/v1/params`
Description: Queries the current parameters of the BTC Staking module.

Params by Version
Endpoint: `/anon/btcstaking/v1/params/{version}`
Description: Queries the parameters of the module for a specific past version.

Finality Providers
Endpoint: `/anon/btcstaking/v1/finality_providers`
Description: Retrieves all finality providers in the Anon staking module.

Finality Provider by Public Key
Endpoint: `/anon/btcstaking/v1/finality_providers/{fp_btc_pk_hex}/finality_provider`
...
Endpoint: `/anon/btcstaking/v1/btc_delegations/{status}`
Description: Queries all BTC delegations under a given status.

Active Finality Providers at Height
Endpoint: `/anon/btcstaking/v1/finality_providers/{height}`
Description: Retrieves finality providers with non-zero voting power at a specific Anon block height.

Finality Provider Power at Height
Endpoint: `/anon/btcstaking/v1/finality_providers/{fp_btc_pk_hex}/power/`{height}
Description: Queries the voting power of a specific finality provider at a given Anon block height.

Finality Provider Current Power
Endpoint: `/anon/btcstaking/v1/finality_providers/{fp_btc_pk_hex}/power`
Description: Queries the current voting power of a specific finality provider.

Activated Height
Endpoint: `/anon/btcstaking/v1/activated_height`
...
Endpoint: `/anon/btcstaking/v1/finality_providers/{fp_btc_pk_hex}/delegations`
Description: Queries all BTC delegations under a specific finality provider.

BTC Delegation by Staking Transaction Hash
Endpoint: `/anon/btcstaking/v1/btc_delegation/{staking_tx_hash_hex}`
