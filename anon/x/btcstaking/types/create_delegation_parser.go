...
func NewParsedPublicKey(key *anc.BIP340PubKey) (*ParsedPublicKey, error) {
	if key == nil {
		return nil, fmt.Errorf("cannot parse nil *anc.BIP340PubKey")
	}
	pk, err := key.ToBTCPK()

	if err != nil {
		return nil, fmt.Errorf("failed to parse *anc.BIP340PubKey: %w", err)
	}
...
func NewParsedBIP340Signature(sig *anc.BIP340Signature) (*ParsedBIP340Signature, error) {
	if sig == nil {
		return nil, fmt.Errorf("cannot parse nil *anc.BIP340Signature")
	}

	signature, err := sig.ToBTCSig()

	if err != nil {
		return nil, fmt.Errorf("failed to parse *anc.BIP340Signature: %w", err)
	}
...
func NewBtcTransaction(transactionBytes []byte) (*ParsedBtcTransaction, error) {
	tx, err := anc.NewBTCTxFromBytes(transactionBytes)
...
type ParsedPublicKeyList struct {
	...
	PublicKeysAncFormat []anc.BIP340PubKey
}

func NewParsedPublicKeyList(pks []anc.BIP340PubKey) (*ParsedPublicKeyList, error) {
	if len(pks) == 0 {
		return nil, fmt.Errorf("cannot parse empty list of *anc.BIP340PubKey")
	}

	parsedKeys, err := anc.NewBTCPKsFromBIP340PKs(pks)

	if err != nil {
		return nil, fmt.Errorf("failed to parse list of *anc.BIP340PubKey: %w", err)
	}

	return &ParsedPublicKeyList{
		PublicKeys:          parsedKeys,
		PublicKeysAncFormat: pks,
	}, nil
}

type ParsedProofOfInclusion struct {
	HeaderHash *anc.BTCHeaderHashBytes
	Proof      []byte
	Index      uint32
}
...
func parseCreateDelegationMessage(msg *MsgCreateBTCDelegation) (*ParsedCreateDelegationMessage, error) {
	...
	duplicate, err := ExistsDup(fpPKs.PublicKeysAncFormat)
	if err != nil {
	...
	if len(fpPKs.PublicKeysAncFormat) != 1 {
