...
func TestBTCDelegatorValidate(t *testing.T) {
	validLen := anctypes.BIP340PubKeyLen
	validKey := anctypes.BIP340PubKey(make([]byte, validLen))
	invalidKey := anctypes.BIP340PubKey(make([]byte, validLen-1))
...
func TestGenesisStateValidateBTCDelegationAndDelegator(t *testing.T) {
	...
	require.NoError(t, invalidIdx.Add(datagen.GenRandomAnonTx(r).TxHash())) // different tx
