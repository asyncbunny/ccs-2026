...
func (m *MsgEditFinalityProvider) ValidateBasic() error {
	...
	if len(m.BtcPk) != anc.BIP340PubKeyLen {
		return fmt.Errorf("malformed BTC PK")
	}
	if _, err := anc.NewBIP340PubKey(m.BtcPk); err != nil {
		return err
	}
...
func (m *MsgBtcStakeExpand) GetDelegatorSlashingSig() *anc.BIP340Signature {
	return m.DelegatorSlashingSig
}

// GetDelegatorUnbondingSlashingSig returns the unbonding slashing signature
func (m *MsgBtcStakeExpand) GetDelegatorUnbondingSlashingSig() *anc.BIP340Signature {
	return m.DelegatorUnbondingSlashingSig
}

// GetFpBtcPkList returns the list of FP BTC PK
func (m *MsgBtcStakeExpand) GetFpBtcPkList() []anc.BIP340PubKey {
	return m.FpBtcPkList
}

// GetBtcPk returns the btc delegator BTC PK
func (m *MsgBtcStakeExpand) GetBtcPk() *anc.BIP340PubKey {
	return m.BtcPk
}
...
func (m *MsgBtcStakeExpand) GetStakeExpansion() (*ParsedCreateDelStkExp, error) {
	...
	stkExpandTx, err := anc.NewBTCTxFromBytes(m.StakingTx)
	if err != nil {
		return nil, err
	}

	fundingTx, err := anc.NewBTCTxFromBytes(m.FundingTx)
	if err != nil {
		return nil, err
	}
...
func (m *MsgBTCUndelegate) ValidateBasic() error {
	...
	tx, err := anc.NewBTCTxFromBytes(m.StakeSpendingTx)
