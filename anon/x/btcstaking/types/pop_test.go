...
func FuzzPop_ValidBip322SigNotMatchingBip340PubKey(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		require.NoError(t, err)
		bip340PK1 := anc.NewBIP340PubKeyFromBTCPK(btcPK1)
...
func TestPoPBTCVerify(t *testing.T) {
	...
	tcs := []struct {
		...
		},
		{
			...
			bip340PK,
			popBip322,
			fmt.Errorf("failed to verify possession of anon sig by the BTC key: signature not empty on failed checksig"),
		},
		{
		...
		},
		{
			...
			fmt.Errorf("failed to verify possession of anon sig by the BTC key: cannot verify bip322 signature. One of the required parameters is empty"),
		},
		{
