...
import (
	...
	"github.com/anon-org/anon/v4/crypto/bip322"
	"github.com/anon-org/anon/v4/crypto/ecdsa"
...
type checkStakerKey func(stakerKey *anc.BIP340PubKey) error
...
func (pop *ProofOfPossessionBTC) Verify(staker sdk.AccAddress, bip340PK *anc.BIP340PubKey, net *chaincfg.Params) error {
...
func VerifyBIP340(sigType BTCSigType, btcSigRaw []byte, bip340PK *anc.BIP340PubKey, msg []byte) error {
	...
	bip340Sig, err := anc.NewBIP340Signature(btcSigRaw)
	if err != nil {
		return err
	}
	...
	// So we have to hash anonSig before verifying the signature
	hash := tmhash.Sum(msg)
...
func (pop *ProofOfPossessionBTC) VerifyBIP340(stakerAddr sdk.AccAddress, bip340PK *anc.BIP340PubKey) error {
...
func isSupportedAddressAndWitness(
	...
	if txscript.IsPayToTaproot(script) && len(witness) == 1 {
		return func(stakerKey *anc.BIP340PubKey) error {
...
func VerifyBIP322SigPop(
	...
	key, err := anc.NewBIP340PubKey(pubKeyNoCoord)
	if err != nil {
		return err
	}
...
func VerifyBIP322(sigType BTCSigType, btcSigRaw []byte, bip340PK *anc.BIP340PubKey, signedMsg []byte, net *chaincfg.Params) error {
...
// VerifyBIP322 verifies the validity of PoP where Bitcoin signature is in BIP-322
// after decoding pop.BtcSig to bip322Sig which contains sig and address,
// 1. verify whether bip322 pop signature where msg=pop.AnonSig
// 2. verify(sig=pop.AnonSig, pubkey=anonPK, msg=bip340PK)?
func (pop *ProofOfPossessionBTC) VerifyBIP322(addr sdk.AccAddress, bip340PK *anc.BIP340PubKey, net *chaincfg.Params) error {
	msgToSign := MsgToSignBIP322(addr)
	if err := VerifyBIP322(pop.BtcSigType, pop.BtcSig, bip340PK, msgToSign, net); err != nil {
		return fmt.Errorf("failed to verify possession of anon sig by the BTC key: %w", err)
	}
	return nil
}
...
func VerifyECDSA(sigType BTCSigType, btcSigRaw []byte, bip340PK *anc.BIP340PubKey, msg string) error {
...
func (pop *ProofOfPossessionBTC) VerifyECDSA(addr sdk.AccAddress, bip340PK *anc.BIP340PubKey) error {
...
func (pop *ProofOfPossessionBTC) ValidateBasic() error {
	...
	switch pop.BtcSigType {
	case BTCSigType_BIP340:
		_, err := anc.NewBIP340Signature(pop.BtcSig)
		if err != nil {
