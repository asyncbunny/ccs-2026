...
import (
	...
	types "github.com/anon-org/anon/v4/types"
...
func (gs GenesisState) Validate() error {
	...
	if err := types.ValidateEntries(gs.BlockHeightChains, func(bh *BlockHeightAncToBtc) uint64 {
		return bh.BlockHeightAnc
	}); err != nil {
		return err
	}
	...
	if err := gs.validateFpAncAddr(gs.FinalityProviders); err != nil {
		return err
	}
...
func (gs GenesisState) validateDeletedFps(fps []*FinalityProvider) error {
	...
	for _, deletedFpBtcPkHex := range gs.DeletedFpsBtcPkHex {
		...
		_, err := anc.NewBIP340PubKeyFromHex(deletedFpBtcPkHex)
		if err != nil {
...
// validateFpAncAddr validates there is no duplicate fp anc addr
func (gs GenesisState) validateFpAncAddr(fps []*FinalityProvider) error {
	mapFpAddr := make(map[string]struct{})
	for _, fp := range fps {
		mapFpAddr[fp.Addr] = struct{}{}
	}

	for _, fpAddr := range gs.FpAncAddr {
...
func SortData(gs *GenesisState) {
	...
	sort.Slice(gs.BlockHeightChains, func(i, j int) bool {
		return gs.BlockHeightChains[i].BlockHeightAnc < gs.BlockHeightChains[j].BlockHeightAnc
	})
	...
	sort.Slice(gs.FpAncAddr, func(i, j int) bool {
		return gs.FpAncAddr[i] < gs.FpAncAddr[j]
	})
