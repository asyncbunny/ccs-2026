...
func ValidateParsedMessageAgainstTheParams(
	...
	stakingOutputIdx, err := anc.GetOutputIdxInBTCTx(pm.StakingTx.Transaction, stakingInfo.StakingOutput)
	...
	if pm.UnbondingTx.Transaction.TxOut[0].Value >= pm.StakingTx.Transaction.TxOut[stakingOutputIdx].Value {
		...
		// Unbonding tx should not be replaceable at anon level (and by extension on btc level), as this would
