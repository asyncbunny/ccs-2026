...
func (d *BTCDelegation) GetFpIdx(fpBTCPK *anc.BIP340PubKey) int {
...
// Anon will consider this BTC delegation unbonded directly
...
func (d *BTCDelegation) GetStakingTxHash() (chainhash.Hash, error) {
	parsed, err := anc.NewBTCTxFromBytes(d.StakingTx)
...
func (d *BTCDelegation) MustGetStakingTx() *wire.MsgTx {
	stakingTx, err := anc.NewBTCTxFromBytes(d.StakingTx)
...
func (d *BTCDelegation) MustGetUnbondingTx() *wire.MsgTx {
	unbondingTx, err := anc.NewBTCTxFromBytes(d.BtcUndelegation.UnbondingTx)
...
func (d *BTCDelegation) ValidateBasic() error {
	...
	if _, err := anc.NewBTCTxFromBytes(d.StakingTx); err != nil {
		return fmt.Errorf("failed to deserialize staking tx: %v", err)
	}

	// ensure slashing tx is correctly formatted
	if _, err := anc.NewBTCTxFromBytes(d.SlashingTx.MustMarshal()); err != nil {
...
func (d *BTCDelegation) IsSignedByCovMember(covPk *anc.BIP340PubKey) bool {
...
func (d *BTCDelegation) GetStakingInfo(bsParams *Params, btcNet *chaincfg.Params) (*btcstaking.StakingInfo, error) {
	fpBtcPkList, err := anc.NewBTCPKsFromBIP340PKs(d.FpBtcPkList)
	if err != nil {
		return nil, fmt.Errorf("failed to convert finality provider pks to BTC pks %v", err)
	}
	covenantBtcPkList, err := anc.NewBTCPKsFromBIP340PKs(bsParams.CovenantPks)
	if err != nil {
...
func (d *BTCDelegation) findFPIdx(fpBTCPK *anc.BIP340PubKey) (int, error) {
...
func (d *BTCDelegation) BuildSlashingTxWithWitness(bsParams *Params, btcNet *chaincfg.Params, fpSK *btcec.PrivateKey) (*wire.MsgTx, error) {
	stakingMsgTx, err := anc.NewBTCTxFromBytes(d.StakingTx)
	if err != nil {
		return nil, fmt.Errorf("failed to convert a Anon staking tx to wire.MsgTx: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf(
			...
			d.BtcPk.MarshalHex(),
			anc.NewBIP340PubKeyFromBTCPK(fpSK.PubKey()).MarshalHex(),
			err,
		)
	}
...
func (d *BTCDelegation) BuildUnbondingSlashingTxWithWitness(bsParams *Params, btcNet *chaincfg.Params, fpSK *btcec.PrivateKey) (*wire.MsgTx, error) {
	unbondingMsgTx, err := anc.NewBTCTxFromBytes(d.BtcUndelegation.UnbondingTx)
	if err != nil {
		return nil, fmt.Errorf("failed to convert a Anon unbonding tx to wire.MsgTx: %w", err)
	}
...
// IsSignedByCovMember checks whether the given covenant PK has signed the delegation
func (s *StakeExpansion) IsSignedByCovMember(covPk *anc.BIP340PubKey) bool {
