...
func (tx *BTCSlashingTx) ToMsgTx() (*wire.MsgTx, error) {
	return anc.NewBTCTxFromBytes(*tx)
}
...
func (tx *BTCSlashingTx) Sign(
	...
	return anc.NewBIP340SignatureFromBTCSig(schnorrSig), nil
}
...
// is stack based
func findFPIdxInWitness(fpSK *btcec.PrivateKey, fpBTCPKs []anc.BIP340PubKey) (int, error) {
	fpBTCPK := anc.NewBIP340PubKeyFromBTCPK(fpSK.PubKey())
	sortedFPBTCPKList := anc.SortBIP340PKs(fpBTCPKs)
