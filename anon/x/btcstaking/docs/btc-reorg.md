...
Anon relies in a number of confirmation blocks in BTC to accept BTC
...
By example, if BTC is in block height 150 and Anon chain has a
...
After having collected the anon block height corresponded to the BTC block
...
but it was only later included in the Anon chain, all good and there is
...
which were reorg, the upgrade height would be defined as the Anon block
