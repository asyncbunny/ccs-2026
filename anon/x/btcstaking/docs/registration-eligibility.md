...
This document outlines the various stages of the Anon Genesis chain launch
...
* **Stage 1: Anon Genesis Chain Launch**: At this stage, only
...
Bitcoin stakes to the Anon Genesis chain in order for the stake
...
* **Pre-staking registration**: The process in which a staker registers their
    stake on Anon **before** staking on Bitcoin, without providing a proof
    of inclusion.
* **Post-staking registration**: The process in which a staker **first** stakes on
    Bitcoin and then registers the stake on Anon.
...
This delay is necessary because Anon Chain enables the staking of
...
can begin casting votes to finalize blocks on the Anon chain.
The time of the finality voting activation
is determined by a block height included in the Anon chain
...
### 3.1. Anon Genesis Chain Launch

The Anon Genesis launch procedure involves the chain
...
* **CometBFT Validators**: CometBFT validators can permissionlessly
  ...
  for producing Anon blocks. More details on the CometBFT
  validator registration procedure can be found [here](../../../x/epoching).
* **Finality Providers**: Finality Providers can permissionlessly
  register to Anon Genesis. Note that Finality Providers
  that have operated and received delegations in a Phase-1 Anon
...
* **BTC Stake Registration** Bitcoin stakes for which their hash
  ...
  can now register to Anon Genesis.
  More details on how to register your Bitcoin stakes
  [here](../../../docs/register-bitcoin-stake.md).

> **⚡ Important** Bitcoin stakes cannot register to Anon Genesis
...
> to ever register on the Anon Genesis chain.
...
```shell
anond query btcstaking params --node <rpcnode>
```
