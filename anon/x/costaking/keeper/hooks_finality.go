...
// because the AfterAncFpRemovedFromActiveSet hook gets the updated active sats amount)
...
// AfterAncFpEntersActiveSet handles finality provider activation events.
...
func (h HookFinality) AfterAncFpEntersActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error {
...
// AfterAncFpRemovedFromActiveSet handles finality provider deactivation events.
...
// - For each costaker delegated to this FP:
//   - ActiveSatoshis -= delegated_amount
func (h HookFinality) AfterAncFpRemovedFromActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error {
	return h.k.ictvK.IterateBTCDelegationSatsUpdated(ctx, fpAddr, func(del sdk.AccAddress, activeSats math.Int) error {
		return h.k.costakerModified(ctx, del, func(rwdTracker *types.CostakerRewardsTracker) {
