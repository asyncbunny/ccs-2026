...
func FuzzInitExportGenesis(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		numCostakerTrackers := int(datagen.RandomInt(r, 10))
		...
		costakerTrackers := []types.CostakerRewardsTrackerEntry{}
		addresses := make(map[string]bool) // To avoid duplicates
		for i := 0; i < numCostakerTrackers; i++ {
			addr := datagen.GenRandomAccount().Address
			if !addresses[addr] {
				addresses[addr] = true
				costakerTrackers = append(costakerTrackers, types.CostakerRewardsTrackerEntry{
					CostakerAddress: addr,
...
func TestInitGenesisWithCurrentRewards(t *testing.T) {
	...
	currentRewards := types.CurrentRewards{
		Rewards:    sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(1000))),
		Period:     5,
	...
	historicalRewards := []types.HistoricalRewardsEntry{
		{
			Period: 1,
			Rewards: &types.HistoricalRewards{
				CumulativeRewardsPerScore: sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(500))),
			},
		},
		{
			Period: 2,
			Rewards: &types.HistoricalRewards{
				CumulativeRewardsPerScore: sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(1000))),
			},
		},
	}
...
func TestInitGenesisWithCostakerTrackers(t *testing.T) {
	k, ctx := NewKeeperWithCtx(t)
	...
	// Verify costaker trackers were set
	for _, entry := range costakerTrackers {
		addr, err := sdk.AccAddressFromBech32(entry.CostakerAddress)
		require.NoError(t, err)

		retrievedTracker, err := k.GetCostakerRewards(ctx, addr)
		require.NoError(t, err)
	...
	require.Len(t, exportedGenState.CostakersRewardsTracker, len(costakerTrackers))
}
