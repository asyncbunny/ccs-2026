...
import (
	...
	"github.com/anon-org/anon/v4/testutil/mocks"
...
func makeBtcCkptBytes(r *rand.Rand, epoch uint64, appHash []byte, bitmap []byte, blsSig []byte, t *testing.T) *btctxformatter.RawBtcCheckpoint {
	...
	anonTag := btctxformatter.AnonTag(tag[:btctxformatter.TagLength])
	...
	require.NoError(t, err)
	decodedFirst, err := btctxformatter.IsAnonCheckpointData(anonTag, btctxformatter.CurrentVersion, firstHalf)
	require.NoError(t, err)
	decodedSecond, err := btctxformatter.IsAnonCheckpointData(anonTag, btctxformatter.CurrentVersion, secondHalf)
	require.NoError(t, err)
