...
func (m *MsgAddFinalitySig) ValidateBasic() error {
	...
	if m.FpBtcPk.Size() != anc.BIP340PubKeyLen {
		return ErrInvalidFinalitySig.Wrapf("invalid finality provider BTC public key length: got %d, want %d", m.FpBtcPk.Size(), anc.BIP340PubKeyLen)
	}
	...
	if m.PubRand.Size() != anc.SchnorrPubRandLen {
		return ErrInvalidFinalitySig.Wrapf("invalind public randomness length: got %d, want %d", m.PubRand.Size(), anc.SchnorrPubRandLen)
	}
	...
	if m.FinalitySig.Size() != anc.SchnorrEOTSSigLen {
		return ErrInvalidFinalitySig.Wrapf("invalid finality signature length: got %d, want %d", m.FinalitySig.Size(), anc.BIP340SignatureLen)
	}
...
func (m *MsgResumeFinalityProposal) ValidateBasic() error {
	...
	for _, fpPkHex := range m.FpPksHex {
		_, err := anctypes.NewBIP340PubKeyFromHex(fpPkHex)
		if err != nil {
