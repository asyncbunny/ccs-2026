...
type FinalityHooks interface {
	...
	AfterAncFpEntersActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error
	AfterAncFpRemovedFromActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error
}
...
func (h MultiFinalityHooks) AfterAncFpEntersActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error {
	for i := range h {
		if err := h[i].AfterAncFpEntersActiveSet(ctx, fpAddr); err != nil {
			return err
		}
	}
	return nil
...
func (h MultiFinalityHooks) AfterAncFpRemovedFromActiveSet(ctx context.Context, fpAddr sdk.AccAddress) error {
	for i := range h {
		if err := h[i].AfterAncFpRemovedFromActiveSet(ctx, fpAddr); err != nil {
			return err
		}
	}
	return nil
