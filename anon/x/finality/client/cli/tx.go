...
func NewCommitPubRandListCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			fpBTCPK, err := anc.NewBIP340PubKeyFromHex(args[0])
			if err != nil {
				return err
			}
			...
			// get signature
			sig, err := anc.NewBIP340SignatureFromHex(args[4])
			if err != nil {
				return err
			}
...
func NewAddFinalitySigCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			pubRand, err := anc.NewSchnorrPubRandFromHex(args[2])
			if err != nil {
				return err
			}
			...
			finalitySig, err := anc.NewSchnorrEOTSSigFromHex(args[5])
			if err != nil {
				return err
			}
