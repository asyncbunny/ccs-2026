...
func (k Keeper) SetVotingPower(ctx context.Context, fpBTCPK []byte, height uint64, power uint64) {
	store := k.votingPowerAncBlockHeightStore(ctx, height)
	store.Set(fpBTCPK, sdk.Uint64ToBigEndian(power))
}

// GetVotingPower gets the voting power of a given finality provider at a given Anon height
...
// votingPowerAncBlockHeightStore returns the KVStore of the finality providers' voting power
// prefix: (VotingPowerKey || Anon block height)
// key: Bitcoin secp256k1 PK
// value: voting power quantified in Satoshi
func (k Keeper) votingPowerAncBlockHeightStore(ctx context.Context, height uint64) prefix.Store {
...
// key: Anon block height || Bitcoin secp256k1 PK
// value: voting power quantified in Satoshi
