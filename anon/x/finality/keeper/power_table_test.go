...
func FuzzVotingPowerTable(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		*/
		...
		for i := uint64(0); i < numFpsWithVotingPower; i++ {
			power := h.FinalityKeeper.GetVotingPower(h.Ctx, *fps[i].BtcPk, anonHeight)
		...
		*/
		...
		for _, fp := range fps {
			power := h.FinalityKeeper.GetVotingPower(h.Ctx, *fp.BtcPk, anonHeight)
			require.Zero(t, power)
		}
...
func FuzzRecordVotingPowerDistCache(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		anonHeight := datagen.RandomInt(r, 10) + 1
		h.Ctx = datagen.WithCtxHeight(h.Ctx, anonHeight)
		h.BTCLightClientKeeper.EXPECT().GetTipInfo(gomock.Eq(h.Ctx)).Return(&btclctypes.BTCHeaderInfo{Height: 30}).AnyTimes()
		h.BeginBlocker()
...
func FuzzVotingPowerTable_ActiveFinalityProviders(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		for _, fp := range fpsWithMeta {
			power := h.FinalityKeeper.GetVotingPower(h.Ctx, fp.BtcPk.MustMarshal(), anonHeight)
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		*/
		...
		for i := 0; i < numActiveFPs; i++ {
			fHooks.EXPECT().AfterAncFpEntersActiveSet(
				gomock.Any(),
				fpsWithMeta[i].Addr,
			).Times(1)
		}
		...
		// for i := 0; i < numActiveFPs; i++ {
		// 	votingPower := h.FinalityKeeper.GetVotingPower(h.Ctx, *fpsWithMeta[i].BtcPk, anonHeight)
...
func TestVotingPowerTable_ActiveFinalityProviderRotation_Seed0(t *testing.T) {
	...
	fHooks.EXPECT().AfterAncFpRemovedFromActiveSet(gomock.Any(), gomock.Any()).Times(0)
	...
	t.Logf("About to call BeginBlocker at height %d", anonHeight)
	h.BeginBlocker()
