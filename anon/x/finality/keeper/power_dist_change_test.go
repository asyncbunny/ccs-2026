...
import (
	...
	"github.com/anon-org/anon/v4/test/replay"

	testutil "github.com/anon-org/anon/v4/testutil/btcstaking-helper"
	"github.com/anon-org/anon/v4/testutil/datagen"
	testutilkeeper "github.com/anon-org/anon/v4/testutil/keeper"
	anc "github.com/anon-org/anon/v4/types"
	btcctypes "github.com/anon-org/anon/v4/x/btccheckpoint/types"
	btclckeeper "github.com/anon-org/anon/v4/x/btclightclient/keeper"
	btclightclientkeeper "github.com/anon-org/anon/v4/x/btclightclient/keeper"
	btclctypes "github.com/anon-org/anon/v4/x/btclightclient/types"
	btcstakingkeeper "github.com/anon-org/anon/v4/x/btcstaking/keeper"
	bstypes "github.com/anon-org/anon/v4/x/btcstaking/types"
	btcstktypes "github.com/anon-org/anon/v4/x/btcstaking/types"
	finalitykeeper "github.com/anon-org/anon/v4/x/finality/keeper"
...
func FuzzDistributionCache_BtcUndelegateSameBlockAsExpiration(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		fpMsg, err := datagen.GenRandomCreateFinalityProviderMsgWithBTCAnonSKs(r, fpBtcSK, datagen.GenRandomAddress())
		require.NoError(t, err)
		...
		fpBtcPk := []anc.BIP340PubKey{*fpMsg.BtcPk}
...
func FuzzDistributionCacheVpCheck_FpSlashedBeforeInclusionProof(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		tip := btcLightK.GetTipInfo(ctx)
		lastBTCTipHeight := btcStkK.GetBTCHeightAtAnonHeight(ctx, height-1)
...
func FuzzProcessAllPowerDistUpdateEvents_Determinism(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		fHooks.EXPECT().AfterAncFpEntersActiveSet(gomock.Any(), gomock.Any()).AnyTimes()
		fHooks.EXPECT().AfterAncFpRemovedFromActiveSet(gomock.Any(), gomock.Any()).AnyTimes()
...
func FuzzSlashFinalityProviderEvent(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		*/
		...
		// execute BeginBlock
		btcTip := &btclctypes.BTCHeaderInfo{Height: 30}
		anonHeight := datagen.RandomInt(r, 10) + 1
		h.SetCtxHeight(anonHeight)
		...
		require.Equal(t, uint64(stakingValue), h.FinalityKeeper.GetVotingPower(h.Ctx, *fp.BtcPk, anonHeight))
		...
		*/
		...
		require.Zero(t, h.FinalityKeeper.GetVotingPower(h.Ctx, *fp.BtcPk, anonHeight))
	})
}
...
func FuzzJailFinalityProviderEvents(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		*/
		...
		h.NoError(err)
		require.True(t, fpAfterJailing.IsJailed())
		require.Equal(t, uint64(0), h.FinalityKeeper.GetVotingPower(h.Ctx, *fp.BtcPk, anonHeight))
	})
}
...
func FuzzUnjailFinalityProviderEvents(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		*/
		...
		require.NoError(t, err)
		require.Equal(t, anonHeight, uint64(signInfoAfterUnjail.StartHeight))
...
func AddBtcBlockWithDelegations(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context, delInfos ...*datagen.CreateDelegationInfo) (*datagen.BlockWithProofs, []*wire.MsgTx) {
	stkTxs := datagen.DelegationInfosToBTCTx(delInfos)
	return AddBtcBlockWithTxs(t, r, app, ctx, stkTxs...), stkTxs
}

func AddBtcBlockWithTxs(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context, txs ...*wire.MsgTx) *datagen.BlockWithProofs {
...
func MaybeProduceBlock(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context) sdk.Context {
	if r.Int31n(10) > 5 {
		return ctx
	}
...
func ProduceBlock(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context) sdk.Context {
...
func AddBtcBlock(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context, prevBlockHeader *wire.BlockHeader) *wire.BlockHeader {
	btcLightK := app.BTCLightClientKeeper
	msgSrvrBtcLight := btclightclientkeeper.NewMsgServerImpl(btcLightK)
	...
	generalHeaders := []anc.BTCHeaderBytes{dummyGeneralHeader}
...
func AddNBtcBlock(t *testing.T, r *rand.Rand, app *anonApp.AnonApp, ctx sdk.Context, number uint) {
...
func TestHandleLivenessPanic(t *testing.T) {
	...
	err = btcStakingKeeper.SetParams(ctx, btcstktypes.Params{
		CovenantPks:               anc.NewBIP340PKsFromBTCPKs(covenantPKs),
		CovenantQuorum:            1,
...
func createDelegationWithFinalityProvider(
	...
	covPKs, err := anc.NewBTCPKsFromBIP340PKs(bsParams.CovenantPks)
	require.NoError(t, err)
	...
	serializedStakingTx, err := anc.SerializeBTCTx(stakingMsgTx)
	require.NoError(t, err)
	...
	headers := []anc.BTCHeaderBytes{btcHeader, dummy1Header}
	insertHeaderMsg := &btclctypes.MsgInsertHeaders{
	...
	unbondingTx, err := anc.SerializeBTCTx(testUnbondingInfo.UnbondingTx)
	require.NoError(t, err)
	...
	msgCreateBTCDel := &btcstktypes.MsgCreateBTCDelegation{
		...
		FpBtcPkList:                   []anc.BIP340PubKey{*finalityFP.BtcPk},
		BtcPk:                         anc.NewBIP340PubKeyFromBTCPK(delSK.PubKey()),
		Pop:                           pop,
	...
	vPKs, err := anc.NewBTCPKsFromBIP340PKs(msgCreateBTCDel.FpBtcPkList)
	require.NoError(t, err)
	...
	msgAddCovenantSig := &btcstktypes.MsgAddCovenantSigs{
		...
		UnbondingTxSig:          anc.NewBIP340SignatureFromBTCSig(covUnbondingSigs[0]),
...
func TestTwoBtcActivationEvents(t *testing.T) {
	...
	_, anonFpPk, anonFp := h.CreateFinalityProvider(r)
	...
	// verify anon FP is included in power distribution with correct total voting power
	require.Len(t, newDc.FinalityProviders, 1)
	fpDist := newDc.FinalityProviders[0]
	require.Equal(t, anonFp.BtcPk.MustMarshal(), fpDist.BtcPk.MustMarshal())
...
func TestGovernanceJailingAfterUnjailInSameBlock(t *testing.T) {
	...
	btcTip := &btclctypes.BTCHeaderInfo{Height: 30}
	anonHeight := uint64(10) // Start from a reasonable height
	h.SetCtxHeight(anonHeight)
	...
	for i := anonHeight - 5; i <= anonHeight; i++ {
		dc := ftypes.NewVotingPowerDistCache()
	...
	haltingHeight := uint32(anonHeight - 3)
	...
	anonHeight += 1
	h.SetCtxHeight(anonHeight)
	// fHooks.EXPECT().AfterAncFpEntersActiveSet(gomock.Any(), gomock.Any())
	// fHooks.EXPECT().AfterAncFpRemovedFromActiveSet(gomock.Any(), gomock.Any())
	h.BeginBlocker()

	// Step 6: Check the results
	dc := h.FinalityKeeper.GetVotingPowerDistCache(h.Ctx, anonHeight)
...
func TestProcessAllPowerDistUpdateEvents_SlashedFP(t *testing.T) {
	...
	for _, evt := range h.Ctx.EventManager().Events() {
		if evt.Type == "anon.btcstaking.v1.EventFinalityProviderStatusChange" {
...
func TestHandleFPStateUpdatesWithSlashedFp(t *testing.T) {
	...
	fHooks.EXPECT().AfterAncFpEntersActiveSet(gomock.Any(), fpAddr).Times(1)
	...
	fHooks.EXPECT().AfterAncFpRemovedFromActiveSet(gomock.Any(), fpAddr).Times(1)
	ctx = ctx.WithEventManager(sdk.NewEventManager())
	k.HandleFPStateUpdates(ctx, state, prevDc, newDc, true)
...
func checkHasEventFpStatusChange(t *testing.T, ctx sdk.Context, expFpBtcPk *anc.BIP340PubKey, expFpStatus btcstktypes.FinalityProviderStatus) {
