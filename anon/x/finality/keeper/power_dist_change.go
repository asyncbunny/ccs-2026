...
func (k Keeper) UpdatePowerDist(ctx context.Context) {
	...
	lastBTCTipHeight := k.BTCStakingKeeper.GetBTCHeightAtAnonHeight(ctx, height-1)
...
func (k Keeper) RecordVotingPowerAndCache(ctx context.Context, newDc *ftypes.VotingPowerDistCache) {
	...
	anonTipHeight := uint64(sdk.UnwrapSDKContext(ctx).HeaderInfo().Height)
	...
	for _, fpDistInfo := range newDc.FinalityProviders {
		...
		fpDistInfo.IsTimestamped = k.HasTimestampedPubRand(ctx, fpDistInfo.BtcPk, anonTipHeight)
	}
	...
	for i := uint32(0); i < newDc.NumActiveFps; i++ {
		...
		k.SetVotingPower(ctx, fp.BtcPk.MustMarshal(), anonTipHeight, fp.TotalBondedSat)
	}

	// set the voting power distribution cache of the current height
	k.SetVotingPowerDistCache(ctx, anonTipHeight, newDc)
}
...
func (k Keeper) processActiveFp(
	...
	err := k.hooks.AfterAncFpEntersActiveSet(ctx, fp.GetAddress())
	if err != nil {
...
func (k Keeper) processInactiveFp(
	...
	if shouldCallHooks && state.IsFpInPrevActiveSet(fp.BtcPk) {
		...
		err := k.hooks.AfterAncFpRemovedFromActiveSet(ctx, fpAddr)
		if err != nil {
...
func (k Keeper) HandleActivatedFinalityProvider(ctx context.Context, fpPk *anc.BIP340PubKey) error {
...
func processSlashedEvents(ctx sdk.Context, state *ftypes.ProcessingState) {
	for _, event := range state.SlashedEvents {
		...
		// anon block height that is being processed, because
...
func (k Keeper) loadFP(
	...
	if !found {
		fpBTCPK, err := anc.NewBIP340PubKeyFromHex(fpBTCPKHex)
		if err != nil {
			return nil, err
		}
