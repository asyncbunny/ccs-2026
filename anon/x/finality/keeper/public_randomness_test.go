...
func TestGetPubRandCommitForHeight(t *testing.T) {
	tests := []struct {
		...
		mutateStore        func(context.Context, corestore.KVStoreService, *keeper.Keeper, *anc.BIP340PubKey)
		errMsg             string
	}{
		{
		...
		},
		{
			...
			height: 5,
			valid:  false,
			mutateStore: func(ctx context.Context, ss corestore.KVStoreService, k *keeper.Keeper, fpBtcPK *anc.BIP340PubKey) {
...
func setupTest(t *testing.T, fpBtcPK *anc.BIP340PubKey) (context.Context, corestore.KVStoreService, *keeper.Keeper) {
	var (
		db           = dbm.NewMemDB()
...
func deleteIndex(store corestore.KVStore, fpBtcPK *anc.BIP340PubKey) error {
