...
func FuzzActiveFinalityProvidersAtHeight(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		for i := uint64(0); i < numFpsWithVotingPower; i++ {
			...
			h.FinalityKeeper.SetVotingPower(h.Ctx, fpBTCPK.MustMarshal(), anonHeight, 1)
		}
		...
		req := types.QueryActiveFinalityProvidersAtHeightRequest{Height: anonHeight, Pagination: pagination}
...
func FuzzVotesAtHeight(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		for i := uint64(0); i < numVotedFps; i++ {
			...
			require.NoError(t, err)
			votedSig, err := anc.NewSchnorrEOTSSig(datagen.GenRandomByteArray(r, 32))
			require.NoError(t, err)
			keeper.SetSig(ctx, anonHeight, votedFpPK, votedSig)
...
func FuzzListPubRandCommit(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		bip340PK := anc.NewBIP340PubKeyFromBTCPK(sk.PubKey())
		require.NoError(t, err)
...
func convertToEvidence(er *types.EvidenceResponse) *types.Evidence {
	fpBtcPk, err := anc.NewBIP340PubKeyFromHex(er.FpBtcPkHex)
	if err != nil {
		return nil
	}
