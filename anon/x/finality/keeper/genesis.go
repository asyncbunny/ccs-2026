...
import (
	...
	btcstk "github.com/anon-org/anon/v4/btcstaking"
...
func (k Keeper) voteSigs(ctx context.Context) ([]*types.VoteSig, error) {
	...
	for ; iter.Valid(); iter.Next() {
		...
		finalitySig, err := anc.NewSchnorrEOTSSig(iter.Value())
		if err != nil {
			return nil, err
		}
...
func (k Keeper) signingInfosAndMissedBlock(ctx context.Context) ([]types.SigningInfo, []types.FinalityProviderMissedBlocks, error) {
	...
	err := k.FinalityProviderSigningTracker.Walk(ctx, nil, func(fpPkBytes []byte, info types.FinalityProviderSigningInfo) (stop bool, err error) {
		fpPk, err := anc.NewBIP340PubKey(fpPkBytes)
		if err != nil {
			return true, err
		}
...
// fpVotingPowers gets the voting power of a given finality provider at a given Anon height.
...
func parsePubKeyAndBlkHeightFromStoreKey(key []byte) (fpBTCPK *anc.BIP340PubKey, blkHeight uint64, err error) {
	sizeBigEndian := 8
	keyLen := len(key)
	...
	fpBTCPK, err = anc.NewBIP340PubKey(key[:startKeyHeight])
	if err != nil {
		return nil, 0, fmt.Errorf("failed to parse pub key from key %w: %w", anc.ErrUnmarshal, err)
	}
