...
func (ms msgServer) AddFinalitySig(goCtx context.Context, req *types.MsgAddFinalitySig) (*types.MsgAddFinalitySigResponse, error) {
	...
	// - Anon should reject any new signature from this finality provider, since it's known to be adversarial
	// - Anon should set its voting power since height h'+1 to be zero, due to the same reason
	// - Anon should NOT set its voting power between [h, h'] to be zero, since
	//   - Anon BTC staking ensures safety upon 2f+1 votes, *even if* f of them are adversarial. This is
	...
	//     This ensures slashable safety together with EOTS, thus does not undermine Anon's security guarantee.
	//   - Due to this reason, when tallying a block, Anon finalises this block upon 2f+1 votes. If we
	...
	//   - To fix the above issue, Anon has to allow finalise and unfinalise blocks. However, this means
	//     Anon will lose safety under an adaptive adversary corrupting even 1 finality provider. It can simply
...
// and emit an event
func (k Keeper) slashFinalityProvider(ctx context.Context, fpBtcPk *anc.BIP340PubKey, evidence *types.Evidence) {
