...
func TestHandleResumeFinalityProposal(t *testing.T) {
	...
	for h := haltingHeight; h <= currentHeight; h++ {
		...
		votedSig, err := anctypes.NewSchnorrEOTSSig(datagen.GenRandomByteArray(r, 32))
...
func TestHandleResumeFinalityProposalWithJailedAndSlashedFp(t *testing.T) {
	...
	slashedFp := &bstypes.FinalityProvider{
		...
		BtcPk:                &slashedFpPk,
		Jailed:               false,
		SlashedAnonHeight: currentHeight,
	}
	...
	for i := 0; i < activeFpNum; i++ {
		...
		bsKeeper.EXPECT().GetFinalityProvider(gomock.Any(), fpPk.MustMarshal()).DoAndReturn(
			func(_ interface{}, _ []byte) (*bstypes.FinalityProvider, error) {
				...
				fp.BtcPk = new(anctypes.BIP340PubKey)
				*fp.BtcPk = fpPk
				return fp, nil
			},
		).AnyTimes()
...
func generateNFpPks(t *testing.T, r *rand.Rand, n int) []anctypes.BIP340PubKey {
	fpPks := make([]anctypes.BIP340PubKey, 0, n)
	for i := 0; i < n; i++ {
...
func publicKeysToHex(pks []anctypes.BIP340PubKey) []string {
...
func setupActiveFps(t *testing.T, fpPks []anctypes.BIP340PubKey, height uint64, fKeeper *keeper.Keeper, ctx sdk.Context) {
