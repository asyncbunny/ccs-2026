...
Anon's BTC Staking protocol introduces an additional consensus round on
...
**Anon Bitcoin Staking.** Anon's Bitcoin Staking protocol allows bitcoin
...
<!-- Anon BTC staking security guarantee, i.e., slashable safety -->
...
paper](https://arxiv.org/pdf/2305.07830.pdf). In Anon's Bitcoin Staking
...
needs to interact with Anon as follows:
...
[object](../../proto/anon/finality/v1/params.proto) defined as follows:
...
the voting power table of all finality providers at each height of the Anon
...
EOTS public randomness commit that each finality provider commits to Anon.
...
also stores the epoch number at which Anon receives the commit.
...
[defined](../../proto/anon/finality/v1/finality.proto) as follows.
...
[object](../../proto/anon/finality/v1/finality.proto) representing the
...
// signatures with correct public randomness on two conflicting Anon headers
message Evidence {
    ...
    bytes canonical_finality_sig = 6 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.SchnorrEOTSSig" ];
    // fork_finality_sig is the finality signature to the fork block
    // where finality signature is an EOTS signature
    bytes fork_finality_sig = 7 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.SchnorrEOTSSig" ];
}
```
...
[proto/anon/finality/v1/tx.proto](../../proto/anon/finality/v1/tx.proto).
...
message MsgCommitPubRandList {
  ...
  bytes sig = 6 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.BIP340Signature" ];
}
```

Upon `MsgCommitPubRandList`, a Anon node will execute as follows:
...
2. Ensure the finality provider has been registered in Anon.
...
message MsgAddFinalitySig {
    ...
    // the `s` in a Schnorr signature `(r, s)`
    // `r` is the public randomness that is already committed by the finality provider
    bytes finality_sig = 5 [ (gogoproto.customtype) = "github.com/anon-org/anon/v4/types.SchnorrEOTSSig" ];
}
```

Upon `MsgAddFinalitySig`, a Anon node will execute as follows:

1. Ensure the finality provider has been registered in Anon and is not
   slashed.
...
7. If the voted block's `AppHash` is different from the canonical block at the
   same height known by the Anon node, then this means the finality provider
   has voted for a fork. Anon node buffers this finality vote to the evidence
...
8. If the voted block's `AppHash` is same as that of the canonical block at the
   ...
   canonical block, and the Anon node will store this finality vote to the
...
Upon `BeginBlocker`, the Finality module of each Anon node will [execute the
following](./abci.go):
...
Upon `EndBlocker`, the Finality module of each Anon node will [execute the
...
2. Tally all non-finalized blocks as follows:
   1. Find the starting height that the Anon node should start to finalize.
   ...
   2. For each `IndexedBlock` between the starting height and the current
      ...
      2. If the finality provider set is empty, then this block is not
         finalizable and the Anon node will skip this block.
