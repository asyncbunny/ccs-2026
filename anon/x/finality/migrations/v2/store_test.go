...
func TestMigrateStore(t *testing.T) {
	...
	fpBtcPKs := make([]*anc.BIP340PubKey, 0, fpCount)
	for range fpCount {
...
func populateCommits(t *testing.T, ctx context.Context, s corestore.KVStore, fKeeper *keeper.Keeper, fpBtcPK *anc.BIP340PubKey, n int) {
	for i := 0; i < n; i++ {
...
func getIndex(store corestore.KVStore, cdc codec.BinaryCodec, fpBtcPK *anc.BIP340PubKey) (types.PubRandCommitIndexValue, error) {
	bytesKey, err := collections.EncodeKeyWithPrefix(types.PubRandCommitIndexKeyPrefix.Bytes(), collections.BytesKey, fpBtcPK.MustMarshal())
	if err != nil {
