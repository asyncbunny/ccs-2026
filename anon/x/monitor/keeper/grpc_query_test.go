...
import (
	...
	types2 "github.com/anon-org/anon/v4/x/epoching/types"
...
func FuzzQueryEndedEpochBtcHeight(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		ctx := anonApp.NewContext(false)
		lck := anonApp.BTCLightClientKeeper
		mk := anonApp.MonitorKeeper

		queryHelper := baseapp.NewQueryServerTestHelper(ctx, anonApp.InterfaceRegistry())
...
func FuzzQueryReportedCheckpointBtcHeight(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		lck := anonApp.BTCLightClientKeeper
		mk := anonApp.MonitorKeeper
		ck := anonApp.CheckpointingKeeper
