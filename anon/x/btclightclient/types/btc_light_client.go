...
type BtcChainReadStore interface {
	...
	GetHeaderByHash(hash *anc.BTCHeaderHashBytes) (*BTCHeaderInfo, error)
	// Contract: in case of non-existent header, return error
...
func (i *localHeaderInfo) toBTCHeaderInfo() *BTCHeaderInfo {
	...
	headerBytes := anc.NewBTCHeaderBytesFromBlockHeader(i.header)
	headerHash := anc.NewBTCHeaderHashBytesFromChainhash(&blockHash)
...
func (l *BtcLightClient) InsertHeaders(readStore BtcChainReadStore, headers []*wire.BlockHeader) (*InsertResult, error) {
	...
	if firstHeaderOfExtensionChain.PrevBlock.IsEqual(&currentTipHash) {
		...
		firstExtHeaderHash := anc.NewBTCHeaderHashBytesFromChainhash(&firstExtHeader)
		...
		parentHash := anc.NewBTCHeaderHashBytesFromChainhash(&firstHeaderOfExtensionChain.PrevBlock)
