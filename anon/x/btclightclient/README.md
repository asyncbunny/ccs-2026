...
Anon chain needs to know about different events happening on Bitcoin chain.
To make it possible in a secure way, Anon needs to know the current
...
- [Concepts](#concepts)
  ...
  - [Anon's BTC light client design](#anons-btc-light-client-design)
- [States](#states)
...
The Anon chain needs to learn and validate a number of events that had
happened on the Bitcoin chain. Those events are:

1. `New checkpoint event` - Bitcoin Timestamping protocol requires checkpoints
on Bitcoin to be reported back to Anon. To do it securely, each checkpoint
...
To properly validate those inclusion proofs, the Anon chain needs to know the
current state of BTC chain i.e., what is current canonical chain recognized by BTC.

### Anon's BTC light client design

Anon maintains a BTC light client so that it can verify the inclusion
of various Bitcoin transactions.

In a high-level overview, the Anon BTC light client starts from some base BTC
...
The base BTC header is defined in the [genesis](../../proto/anon/btclightclient/v1/genesis.proto) module.

The Anon BTC light client module stores only BTC headers from the canonical
...
[object](../../proto/anon/btclightclient/v1/params.proto) defined as follows:
...
[object](../../proto/anon/btclightclient/v1/btclightclient.proto)
...
message BTCHeaderInfo {
  bytes header = 1
      [ (gogoproto.customtype) =
            "github.com/anon-org/anon/v4/types.BTCHeaderBytes" ];
  bytes hash = 2
      [ (gogoproto.customtype) =
...
Its purpose is to update the state of the BTC chain as viewed by Anon chain.
...
Upon receiving a `MsgInsertHeaders` message, a Anon node applies the following
