...
func FuzzKeeperInsertValidChainExtension(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		for _, header := range chainToInsert {
			...
			hash := anc.NewBTCHeaderHashBytesFromChainhash(&headerHash)
...
func FuzzKeeperInsertInvalidChain(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		errEmpty := blcKeeper.InsertHeadersWithHookAndEvents(ctx, []anc.BTCHeaderBytes{})
...
func FuzzKeeperValdateHeaderAtDifficultyAdjustmentBoundaries(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		genesisHeader := anc.NewBTCHeaderBytesFromBlockHeader(&chaincfg.SimNetParams.GenesisBlock.Header)
		genesisHash := anc.NewBTCHeaderHashBytesFromChainhash(chaincfg.SimNetParams.GenesisHash)
		...
		err = blcKeeper.InsertHeadersWithHookAndEvents(ctx, []anc.BTCHeaderBytes{anc.NewBTCHeaderBytesFromBlockHeader(invalidAdjustedHeader)})
		require.Error(t, err)
		...
		validAdjustedHeaderBytes := anc.NewBTCHeaderBytesFromBlockHeader(validAdjustedHeader)

		err = blcKeeper.InsertHeadersWithHookAndEvents(ctx, []anc.BTCHeaderBytes{anc.NewBTCHeaderBytesFromBlockHeader(validAdjustedHeader)})
		require.NoError(t, err)
