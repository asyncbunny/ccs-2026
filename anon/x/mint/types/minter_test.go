...
func TestCalculateBlockProvision(t *testing.T) {
	...
	totalSupply := math.LegacyNewDec(1_000_000_000_000)                    // 1 trillion uanc
	annualProvisions := totalSupply.Mul(types.InitialInflationRateAsDec()) // 80 billion uanc
	...
	testCases := []testCase{
		{
			...
			// 55 billion uanc (annual provisions) * 15 (seconds) / 31,556,952 (seconds per year) = 26143.209268119430545763735356950823387505865585498 which truncates to 26143 uanc
		...
		},
		{
			...
			// 55 billion uanc (annual provisions) * 30 (seconds) / 31,556,952 (seconds per year) = 52286.418536238861091527470713901646775011731170995  which truncates to 52.286 uanc
