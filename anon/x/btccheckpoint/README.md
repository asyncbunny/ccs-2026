...
The BTC Checkpoint module is responsible for receiving and managing the checkpoints of Anon's state on the Bitcoin blockchain. This includes:
...
message Params {
  ...
  // - being reported back to ANC
  // If a checkpoint has not been reported back within w BTC blocks, then ANC
...
message MsgInsertBTCSpvProof {
  ...
  string submitter = 1;
  repeated anon.btccheckpoint.v1.BTCSpvProof proofs = 2;
}
```

Upon receiving a `MsgInsertBTCSpvProof`, a Anon node will execute as follows:
