...
import (
	...
	dg "github.com/anon-org/anon/v4/testutil/datagen"
...
func TestBtcCheckpointInfo(t *testing.T) {
	...
	blck1AnonOpReturnIdx, blck2AnonOpReturnIdx := uint32(7), uint32(3)
	blck1 := dg.CreateBlock(r, 1, 7, blck1AnonOpReturnIdx, raw.FirstPart)
	blck2 := dg.CreateBlock(r, 2, 14, blck2AnonOpReturnIdx, raw.SecondPart)
	...
	require.Equal(t, tx0.Index, blck1AnonOpReturnIdx)
	require.Equal(t, tx0.Hash, blk1.MarshalHex())
	require.Equal(t, tx0.Transaction, blck1.Transactions[blck1AnonOpReturnIdx])
	require.Equal(t, tx0.Proof, hex.EncodeToString(proofs[0].MerkleNodes))

	tx1 := btcInfo.BestSubmissionTransactions[1]
	require.Equal(t, tx1.Index, blck2AnonOpReturnIdx)
	require.Equal(t, tx1.Hash, blk2.MarshalHex())
	require.Equal(t, tx1.Transaction, blck2.Transactions[blck2AnonOpReturnIdx])
