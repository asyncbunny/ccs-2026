...
func (mc *MockBTCLightClientKeeper) SetDepth(header *anc.BTCHeaderHashBytes, dd uint32) {
	mc.headers[header.String()] = dd
}

func (mc *MockBTCLightClientKeeper) DeleteHeader(header *anc.BTCHeaderHashBytes) {
	delete(mc.headers, header.String())
}

func (mb MockBTCLightClientKeeper) BlockHeight(ctx context.Context, header *anc.BTCHeaderHashBytes) (uint32, error) {
	// todo not used
	return uint32(10), nil
}

func (ck MockBTCLightClientKeeper) MainChainDepth(ctx context.Context, headerBytes *anc.BTCHeaderHashBytes) (uint32, error) {
