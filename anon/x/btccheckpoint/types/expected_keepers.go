...
type BTCLightClientKeeper interface {
	...
	BlockHeight(ctx context.Context, headerHash *anc.BTCHeaderHashBytes) (uint32, error)

	// MainChainDepth returns the depth of the header in the main chain or error if the header does not exist
	MainChainDepth(ctx context.Context, headerBytes *anc.BTCHeaderHashBytes) (uint32, error)
}
