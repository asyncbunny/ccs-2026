...
func loadFilePV(homeDir string) (*privval.FilePV, error) {
	...
	if !cmtos.FileExists(pvKeyFile) {
		return nil, fmt.Errorf("validator key file does not exist. create file using `anond init`: %s", pvKeyFile)
	}
...
func loadBls(homeDir string) (*Bls, error) {
	...
	if !cmtos.FileExists(blsKeyFile) {
		return nil, fmt.Errorf("BLS key file does not exist. create file using `anond init` or `anond create-bls-key`: %s", blsKeyFile)
	}
