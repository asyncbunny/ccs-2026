...
import (
	...
	"github.com/anon-org/anon/v4/crypto/erc2335"
...
const (
	...
	BlsPasswordEnvVar      = "ANON_BLS_PASSWORD" // Environment variable name for BLS password
...
// Password sources:
// 1. Environment variable (ANON_BLS_PASSWORD)
...
// 2. Environment variable ANON_BLS_PASSWORD
...
func CreateBlsKey(homeDir string, password string, passwordFilePath string, cmd *cobra.Command) error {
	...
	if password == "" {
		...
		if passwordFilePath != "" {
			...
			cmd.Printf("anond start --bls-password-file=<path_to_password_file>\n")
		} else {
			...
			cmd.Printf("anond start --bls-password-file=<path_to_file>\n")
		}
