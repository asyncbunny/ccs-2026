...
func setup(t *testing.T, blsSigner checkpointingtypes.BlsSigner, withGenesis bool, invCheckPeriod uint, btcConf anc.SupportedBtcNetwork) (*AnonApp, GenesisState) {
	db := dbm.NewMemDB()
	nodeHome := t.TempDir()
...
// NewAnonAppWithCustomOptions initializes a new AnonApp with custom options.
// Created Anon application will have one validator with hardcoed amount of tokens.
...
func NewAnonAppWithCustomOptions(t *testing.T, isCheckTx bool, blsSigner checkpointingtypes.BlsSigner, options SetupOptions) *AnonApp {
	t.Helper()
...
func genesisStateWithValSet(t *testing.T,
	app *AnonApp, genesisState GenesisState,
...
// Setup initializes a new AnonApp. A Nop logger is set in AnonApp.
// Created Anon application will have one validator with hardoced amount of tokens.
// This is necessary as from cosmos-sdk 0.46 it is required that there is at least
// one validator in validator set during InitGenesis abci call - https://github.com/cosmos/cosmos-sdk/pull/9697
func Setup(t *testing.T, isCheckTx bool) *AnonApp {
	t.Helper()
	return SetupWithBitcoinConf(t, isCheckTx, anc.BtcSimnet)
}

// SetupWithBitcoinConf initializes a new AnonApp with a specific btc network config.
// A Nop logger is set in AnonApp. Created Anon application will have one validator
...
func SetupWithBitcoinConf(t *testing.T, isCheckTx bool, btcConf anc.SupportedBtcNetwork) *AnonApp {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new AnonApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit (10^6) in the default token of the anon app from first genesis
// account. A Nop logger is set in AnonApp.
// Note that the privSigner should be the 0th item of valSet
func SetupWithGenesisValSet(t *testing.T, btcConf anc.SupportedBtcNetwork, valSet []*checkpointingtypes.GenesisKey, blsSigner checkpointingtypes.BlsSigner, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *AnonApp {
	t.Helper()
...
func AddTestAddrs(app *AnonApp, ctx sdk.Context, accNum int, accAmt math.Int) ([]sdk.AccAddress, error) {
...
func initAccountWithCoins(app *AnonApp, ctx sdk.Context, addr sdk.AccAddress, coins sdk.Coins) {
