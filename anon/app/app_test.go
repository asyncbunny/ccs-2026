...
import (
	...
	testsigner "github.com/anon-org/anon/v4/testutil/signer"
...
func TestAnonCheckRemovedStakingEndBlocker(t *testing.T) {
	db := dbm.NewMemDB()
	...
	appOpts, cleanup := anonApp.TmpAppOptions()
	defer cleanup()

	app := anonApp.NewAnonAppWithCustomOptions(t, false, blsSigner, anonApp.SetupOptions{
		Logger:             logger,
		DB:                 db,
		InvCheckPeriod:     0,
...
func TestAnonBlockedAddrs(t *testing.T) {
	db := dbm.NewMemDB()
	...
	for acc := range anonApp.BlockedAddresses() {
...
func TestStakingRouterDisabled(t *testing.T) {
	...
	app := anonApp.NewAnonAppWithCustomOptions(t, false, tbs, anonApp.SetupOptions{
		Logger:             logger,
		DB:                 db,
		InvCheckPeriod:     0,
		SkipUpgradeHeights: map[int64]bool{},
