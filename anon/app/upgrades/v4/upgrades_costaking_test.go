...
import (
	...
	btcstkkeeper "github.com/anon-org/anon/v4/x/btcstaking/keeper"
...
func convertBTCDelegationResponseToBTCDelegation(resp *btcstktypes.BTCDelegationResponse) (*btcstktypes.BTCDelegation, error) {
	...
	delegatorSig, err := anc.NewBIP340SignatureFromHex(resp.DelegatorSlashSigHex)
	if err != nil {
...
func loadAndSeedBTCDelegations(t *testing.T, ctx sdk.Context, env string, btcStkStoreKey *storetypes.KVStoreKey) (int, []anc.BIP340PubKey, error) {
	...
	fpPubKeyMap := make(map[string]anc.BIP340PubKey)
	count := 0
	for decoder.More() {
	...
	fpPubKeys := make([]anc.BIP340PubKey, 0, len(fpPubKeyMap))
...
func downloadBTCDelegationsFile(filePath string) error {
	...
	req.Header.Set("User-Agent", "Mozilla/5.0 (compatible; anon-test)")
