...
func (s *UpgradeTestSuite) SetupTest() {
	...
	app.Upgrades = []upgrades.Upgrade{v4.Upgrade}

	// Set up app
	s.app = app.SetupWithBitcoinConf(s.T(), false, anc.BtcSignet)
	s.ctx = s.app.BaseApp.NewContextLegacy(false, tmproto.Header{Height: 1, ChainID: "anon-1", Time: time.Now().UTC()})
...
func (s *UpgradeTestSuite) TestUpgradeWarningScenarios() {
	testCases := []struct {
	...
	}{
		{
			...
			setupCondition: func() {
				coins := sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(500000)))
	...
	testCases := []struct {
		...
		},
		{
			...
			setupFailureCondition: func() {
				...
				coins := sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(100)))
				s.Require().NoError(s.app.BankKeeper.MintCoins(s.ctx, minttypes.ModuleName, coins))
...
func (s *UpgradeTestSuite) executeUpgrade(upgradeHeight int64) error {
	...
	plan := upgradetypes.Plan{Name: v4.UpgradeName, Height: upgradeHeight}
...
func (s *UpgradeTestSuite) verifyPostUpgradeDelegation() {
	...
	delegatorCoins := sdk.NewCoins(sdk.NewCoin("uanc", math.NewInt(1000000))) // 1M uanc
	...
	delegationAmount := sdk.NewCoin("uanc", math.NewInt(100000)) // 100k uanc
	...
	balance := s.app.BankKeeper.GetBalance(s.ctx, moduleAddr, "uanc")
	...
	delegatorBalance := s.app.BankKeeper.GetBalance(s.ctx, delegatorAddr, "uanc")
