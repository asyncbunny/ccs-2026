...
type (
	...
	BtcStkKeeper interface {
		...
		SetFpAncAddr(ctx context.Context, fpAddr sdk.AccAddress) error
		SoftDeleteFinalityProvider(ctx context.Context, fpBtcPk *anc.BIP340PubKey) error
		FpTotalSatsStaked(ctx context.Context, fpBTCPK *anc.BIP340PubKey) (uint64, error)
	}
)
...
// FpSoftDeleteDupAddr Iterates over all the finality providers and sets their fp anc address
...
func FpSoftDeleteDupAddr(ctx context.Context, btcStkK BtcStkKeeper) error {
	...
	err := btcStkK.IterateFinalityProvider(ctx, func(fp btcstktypes.FinalityProvider) error {
		...
		return btcStkK.SetFpAncAddr(ctx, fp.Address())
	})
	if err != nil {
		return err
	}
	...
	fpsToDelete := make([]*anc.BIP340PubKey, 0)
