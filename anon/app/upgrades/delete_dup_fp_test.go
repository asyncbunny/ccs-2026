...
type FpFromTestData struct {
	...
	BtcPk              *anc.BIP340PubKey `json:"btc_pk,omitempty"`
...
type MockBtcStkKeeper struct {
	...
	setAncAddrsCount uint64
	deletedFps       []*anc.BIP340PubKey
...
func (m *MockBtcStkKeeper) SetFpAncAddr(ctx context.Context, fpAddr sdk.AccAddress) error {
	m.setAncAddrsCount++
	return nil
}

func (m *MockBtcStkKeeper) SoftDeleteFinalityProvider(ctx context.Context, fpBtcPk *anc.BIP340PubKey) error {
	m.deletedFps = append(m.deletedFps, fpBtcPk)
	return nil
}

func (m *MockBtcStkKeeper) FpTotalSatsStaked(ctx context.Context, fpBTCPK *anc.BIP340PubKey) (uint64, error) {
...
func TestFpSoftDeleteDupAddr(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// Verify that SetFpAncAddr was called for each FP
			require.EqualValues(t, mockBtcStkKeeper.setAncAddrsCount, len(fps), "SetFpAncAddr should be called for each FP")
