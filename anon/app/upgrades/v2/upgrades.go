...
const (
	// UpgradeName defines the on-chain upgrade name for the Anon v2 upgrade
	UpgradeName = "v2"
...
func CreateUpgradeHandler(whitelistedChannelsByID map[string]struct{}) upgrades.UpgradeHandlerCreator {
	return func(mm *module.Manager, configurator module.Configurator, keepers *keepers.AppKeepers) upgradetypes.UpgradeHandler {
		return func(ctx context.Context, plan upgradetypes.Plan, fromVM module.VersionMap) (module.VersionMap, error) {
			...
			// Set the denom creation fee to uanc
...
// addRateLimits adds rate limit to the native denom ('uanc') to all channels registered on the channel keeper
