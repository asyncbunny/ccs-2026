# BLS Key Management in Anon

This document describes how BLS keys are managed in Anon, including password
management options and command-line flags for secure key generation and usage.

## Overview

BLS keys in Anon are used for validator signatures in the checkpointing
...
1. **Environment Variable (Recommended)**: Set the `ANON_BLS_PASSWORD` 
...
The following command-line flags can be used with `anond init`, 
`anond create-bls-key`, and `anond start`:
...
anond create-bls-key --no-bls-password

# Create a BLS key with a specified password
anond create-bls-key --insecure-bls-password="your-secure-password"

# Start a node using a password from a custom location
anond start --bls-password-file="/path/to/custom/password.txt"
```
...
export ANON_BLS_PASSWORD="your-secure-password"
...
anond start --bls-password-file="/path/to/bls_password.txt"
```

## Priority Order

When loading the BLS key, Anon checks for the password in the following 
order:

1. Direct password provided via `--insecure-bls-password` flag
2. `ANON_BLS_PASSWORD` environment variable
