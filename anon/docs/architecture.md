...
The Anon system is composed of a Anon node
...
![Anon Architecture](./static/arch.png)

## Anon Node Modules

### [Epoching](../x/epoching)

The Anon blockchain is divided into epochs
that consist of a parameterized number of blocks.
Within each epoch, the validator set does not change.
This way, Anon needs a checkpoint per epoch rather than per block,
...
The BTC Checkpoint module verifies Anon’s BTC checkpoints
reported by the Vigilante Reporter  and
...
The checkpointing module is responsible for creating Anon checkpoints
...
Anon transactions that carry them.
...
of the rewards intended for Anon stakers and
...
relayer of data between Anon and Bitcoin.
Anon's secure operation requires
...
and reports them back to Anon using Anon transactions.
...
- The consistency between the Bitcoin canonical chain and
  ...
  Anon's BTC Light client module.
...
- The execution of BTC Staking on-demand unbonding transactions
  on the Bitcoin ledger to inform Anon about them.
...
then informing Anon about their staking.
...
between Anon and other Consumer Zones (CZs).
It is responsible for updating the CZ's light client
inside the Anon ledger to enable checkpointing and
propagating checkpoint information to the Anon smart contract
