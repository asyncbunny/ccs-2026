...
3. [Bitcoin Stake Extension Registration](#3-bitcoin-stake-extension-registration)
   ...
   4. [The `MsgBtcStakeExpand` Anon Message](#34-the-msgbtcstakeextend-anon-message)
...
the Anon Genesis chain. Bitcoin Stake Extension allows existing active delegations to
...
- Atomic activation process (unbond and activation in the same Anon Genesis block)
...
- **Stake Extension Submission**:
  The submission of the Stake Extension to Anon Genesis.
...
1. **Stake Extension Request**: The staker submits
   a stake extension request on Anon Genesis. The request
   ...
   script path. The Anon Genesis chain records the request.
...
4. **Monitoring for Confirmation**: A vigilante tracker (or the staker themself)
   ...
   Once this happens, the inclusion proof is submitted to Anon Genesis.
...
> staker first submits to Anon Genesis before broadcasting to
> Bitcoin.

```mermaid
sequenceDiagram
actor btcstaker as BTC Staker
participant anon as Anon Genesis
...
Note over btcstaker,anon: ...Active BTC delegation
btcstaker->>anon: MsgBtcStakeExpand
activate anon
anon-->>btcstaker: New pending BTC Del
deactivate anon

Note over covd,anon: Watch for pending BTC Dels

covd->>anon: Send missing signatures
activate anon
anon-->>covd: Stake Extension Verified
deactivate anon
...
vigilante->>anon: Reports BTC stake extension is k deep<br>MsgBTCUndelegate
activate anon
anon-->>vigilante: Stake extension is confirmed
deactivate anon
Note over anon: Creates events of unbonding for old staking<br>and activation of stake extension to process<br>at the same anon block height
```
...
* **Any number of outputs** with at least *one* corresponding to the
  extended staking transaction staking output. The Anon chain requires that the new staking output value must be at least the same as the old staking value.
...
staking transaction and its state on Anon Genesis.
...
- **Funding UTXO**: The funding UTXO should not correspond
  to any BTC staking output known to Anon Genesis.
...
- **Original Staking Status**: The original staking transaction must have an `ACTIVE` status on Anon Genesis.
...
- **BTC Staking Params**: The staking extension should use the current Anon Genesis staking parameters.
  Documentation on how to select the appropriate parameters can be found
  in the [staking registration document](./register-bitcoin-stake.md#32-anon-genesis-chain-btc-staking-parameters).
  ...
  > where the transaction is first submitted to Anon Genesis for confirmation
  > before being broadcast to the Bitcoin network.
  > As a result, the stake extension transaction must use the current Bitcoin
  > staking parameters as defined by Anon Genesis at the time of submission.
...
- **Submitter Address**: The Anon Genesis address used to submit the stake extension transaction to Anon Genesis
...
> Anon Genesis for confirmation before submitting to Bitcoin.
...
to the Anon Genesis chain before the corresponding stake
...
the Anon Genesis chain and matching with the
**current Anon Genesis Bitcoin light client tip height**
...
which must follow the specific structures required by the Anon Genesis chain.
...
>   * Must use current Anon staking parameters
>
...
### 3.4. The `MsgBtcStakeExpand` Anon Message
...
* `staker_addr`:
  ...
  *This must be the same address that signed the original delegation and
  must also sign the extension registration transaction*. The Anon signer
  address must be the same from the old stake (staker_addr).

  Example: `"anc1abc123def456ghi789jkl012mno345pqr678stu901vwx234yz"`
...
* `fp_btc_pk_list`:
  ...
  > The Anon Genesis chain will reject extensions that attempt to add or remove
...
* `staking_time`:
  ...
  Anon staking parameters by being higher than or equal to
...
* `unbonding_time`:
  ...
  Anon Genesis BTC staking parameters.
...
After the submission of the `MsgBtcStakeExpand` to Anon Genesis,
...
1. **Retrieve covenant signatures from the Anon Genesis chain**
   ...
   anond query btcstaking btc-delegation [previous-staking-tx-hash]
   ```
...
4. **Finalize and Broadcast**:
  ...
  network
  - Submit the transaction hash and inclusion proof to Anon after confirmation
...
> **⚡ Note**: Once you submit `MsgBtcStakeExpand` to Anon, it creates a
...
anond query btcstaking btc-delegation [extension-delegation-hash]
...
anond query btcstaking delegations-by-staker [staker-address]
