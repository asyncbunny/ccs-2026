...
* **Covenant Committee**:
  ...
  The co-signatures are published on the Anon Genesis chain and
...
* **Staking Transaction**:
  ...
  the Anon-recognized staking script.
...
* **Unbonding Transaction**:
  ...
  * The Bitcoin fee of the unbonding transaction must be equal to the fee
    ...
    [Anon Genesis parameters](./register-bitcoin-stake.md#32-anon-chain-btc-staking-parameters).
* **Slashing Transaction**:
  ...
  * It must have exactly two outputs,
    * the first sending the slashed fraction
      of the funds to a burn address specified in the Anon Genesis chain's parameters, and
...
2. **Unbonding Path**:
   ...
   * `CovenantPk1..CovenantPkN` are the lexicographically sorted public keys of the
     ...
     [Anon Genesis parameters](./register-bitcoin-stake.md#32-anon-chain-btc-staking-parameters).
   * `CovenantThreshold` is a Anon parameter specifying the number of how many
...
>   It becomes active only when the Anon Genesis chain receives
