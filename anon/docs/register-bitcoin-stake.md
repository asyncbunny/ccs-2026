...
3. [Bitcoin Stake Registration](#3-bitcoin-stake-registration)
   ...
   2. [Anon Chain BTC Staking Parameters](#32-anon-chain-btc-staking-parameters)
   3. [Creating the Bitcoin transactions](#33-creating-the-bitcoin-transactions)
   4. [The `MsgCreateBTCDelegation` Anon message](#34-the-msgcreatebtcdelegation-anon-message)
...
with the Anon chain in order to register Bitcoin stakes.
The document is structured as follows:
- [Section 2](#2-bitcoin-stake-registration-methods) provides an overview
    of the two flows for registering stakes on the Anon chain.
- [Section 3](#3-bitcoin-stake-registration) describes the data required for stake registration,
    ...
    to communicate staking transactions to the Anon chain.
...
Bitcoin stakes must be registered on the Anon blockchain
...
network components including the Anon chain CometBFT validators
and the covenant committee.

There are two main methods for registering Bitcoin stakes on the Anon blockchain:
* **Post-staking registration**: This applies to stakers who have already submitted
  their BTC Staking transaction on the Bitcoin network and then register it on Anon
  (e.g., phase-1 stakers).
* **Pre-staking registration**: This is for stakers who seek validation from the Anon
  chain before submitting their staking transaction on Bitcoin and locking their funds,
  ensuring acceptance guarantees (e.g., newly created stakes after the Anon chain launch).
...
> we frequently refer to `k`-depth. The Anon chain
> only activates staking transactions if they reach `k`-depth,
> where `k` is a Anon chain parameter defined in the
...
and is subsequently registered on the Anon chain.
For example, this includes participants from a Anon phase-1 network.
...
2. **Fill the `MsgCreateBTCDelegation` message** with the  unbonding transaction,
  ...
  the Anon blockchain (details on how to do this will be provided in
  [Section 3.4.](#34-the-msgcreatebtcdelegation-anon-message))
...
> **Failing to provide this proof will make the Anon node expect**
...
verification from the Anon chain before submitting their
...
to the Anon chain. Subsequently, the covenant committee provides
...
1. **Fill the `MsgCreateBTCDelegation` message** with necessary metadata and
   broadcast it to the Anon blockchain.
   (details on how to do this
   will be provided in [Section 3.4.](#34-the-msgcreatebtcdelegation-anon-message))
...
> staking parameter of the Anon chain
> (this parameter will be detailed in
> [Section 3.2.](#32-anon-chain-btc-staking-parameters)).

## 3. Bitcoin Stake Registration

### 3.1. Overview of Registration Data

Registering a Bitcoin stake on the Anon chain requires submitting the
...
* **Unbonding Transaction**: An *unsigned* unbonding transaction allowing
  the staker to on-demand unbond their stake. It is submitted to the Anon chain
...
* **Slashing Transactions**: Two staker pre-signed slashing transactions
  ...
  double-signs. They are submitted to the Anon chain and co-signed
  by the covenants upon verification.
* **Proof of Possession**: Confirms ownership of the Bitcoin key
  by the Anon account used for stake registration.
...
Once assembled, this data is packaged into a Anon chain transaction and
...
* Building the Anon chain transaction with required staking data.
* Submitting the registration transaction to the Anon chain.

### 3.2. Anon Chain BTC Staking Parameters

BTC Staking transactions must adhere to parameters defined by the Anon chain,
...
* `delegation_creation_base_gas_fee`: Defines the minimum
  ...
  registration flow.
* `allow_list_expiration_height`: The Anon block height
...
> module parameters can be queried via a Anon node using
...
>   **Anon on-chain Bitcoin light client** at the time of pre-staking
>   registration. This ensures that the transaction commits to be validated
>   against the current staking parameters version (as observed by the Anon chain).
...
a Bitcoin stake on the Anon and Bitcoin ledgers.
...
> **⚡ Note**: Ensure you use the valid Anon parameters when creating
> the Bitcoin transactions.

### 3.4. The `MsgCreateBTCDelegation` Anon message

Cosmos SDK transactions are used for registering
BTC delegations on the Anon chain.
The `MsgCreateBTCDelegation` message
bundles the necessary staking data and registers
the Bitcoin stake on the Anon blockchain.
...
* `staker_addr`:
  A Bech32-encoded Anon address (`anc...`) representing the
  staker's Anon account where staking rewards will be accumulated.
...
* `pop` (Proof of Possession):
  ...
  * **ECDSA**: Bytes of the bech32 encoded address should be signed.
  ...
  message ProofOfPossessionBTC {
      ...
      // btc_sig is the signature generated via sign(sk_btc, anon_staker_address)
...
* `fp_btc_pk_list`:
  ...
  **For phase-2,
  ...
  since Anon is the only system secured by the Bitcoin stake**.
...
* `staking_time`:
  ...
  and must comply with the Anon staking parameters.
* `staking_value`:
...
* `staking_tx`:
  ...
  > Since the Anon chain tracks staking transactions based on
  ...
  > transaction to Anon, as this field directly influences
...
* `staking_tx_inclusion_proof` (Optional):
  ...
  message TransactionKey {
    ...
    bytes hash = 2
      ...
      "github.com/anon-org/anon/v4/types.BTCHeaderHashBytes" ];
  }

  // in x/btcstaking module types
  message InclusionProof {
      // key is the position (txIdx, blockHash) of this tx on BTC blockchain
      anon.btccheckpoint.v1.TransactionKey key = 1;
...
message to the Anon network:
* **Command line interface (CLI)**:
  Use the `anond tx btcstaking create-btc-delegation` command.
...
in the Anon parameters (see [Section 3.2.](#32-anon-chain-btc-staking-parameters)).
...
1. **Retrieve the unbonding transaction and covenant signatures**
   by querying the Anon ledger (e.g., through the RPC/LCD or CLI).
...
that transfers staked BTC from Anon protocol output (either staking or
...
(as defined in the staking params in [Section 3.2.](#32-anon-chain-btc-staking-parameters)),
...
parameter in the [Anon Chain BTC Staking Parameters](32-anon-chain-btc-staking-parameters). Anon ensures that the timelock on the change output of a slashing
...
* **via an RPC/LCD query**
  on the URL `/anon/incentive/address/{address}/reward_gauge`,
