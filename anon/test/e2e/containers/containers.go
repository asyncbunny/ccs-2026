...
const (
	...
	AnonHomePath        = "/home/anon/anondata"
	FlagHome               = "--home=" + AnonHomePath
)
...
func (m *Manager) ExecTxCmdWithSuccessString(t *testing.T, chainId string, containerName string, command []string, successStr string) (bytes.Buffer, bytes.Buffer, error) {
	additionalArgs := []string{fmt.Sprintf("--chain-id=%s", chainId), "--gas-prices=1uanc", "-b=sync", "--yes", "--keyring-backend=test", "--log_format=json", "--home=/home/anon/anondata"}
...
func (m *Manager) RunHermesResource(chainAID, osmoARelayerNodeName, osmoAValMnemonic, chainBID, osmoBRelayerNodeName, osmoBValMnemonic string, hermesCfgPath string) (*dockertest.Resource, error) {
	hermesResource, err := m.pool.RunWithOptions(
		&dockertest.RunOptions{
			...
			Env: []string{
				fmt.Sprintf("ANC_A_E2E_CHAIN_ID=%s", chainAID),
				fmt.Sprintf("ANC_B_E2E_CHAIN_ID=%s", chainBID),
				fmt.Sprintf("ANC_A_E2E_VAL_MNEMONIC=%s", osmoAValMnemonic),
				fmt.Sprintf("ANC_B_E2E_VAL_MNEMONIC=%s", osmoBValMnemonic),
				fmt.Sprintf("ANC_A_E2E_VAL_HOST=%s", osmoARelayerNodeName),
				fmt.Sprintf("ANC_B_E2E_VAL_HOST=%s", osmoBRelayerNodeName),
			},
			Entrypoint: []string{
				"sh",
				"-c",
...
func (m *Manager) RunNodeResource(chainId string, containerName, valCondifDir string) (*dockertest.Resource, error) {
	...
	runOpts := &dockertest.RunOptions{
		...
		Entrypoint: []string{
			...
			// "anond start " + FlagHome + " --log_level trace --trace",
			...
			// "anond start " + FlagHome + " --no-bls-password",
		},
		ExposedPorts: []string{"26656", "26657", "1317", "9090"},
		Mounts: []string{
			fmt.Sprintf("%s/:%s", valCondifDir, AnonHomePath),
...
func (m *Manager) ClearResources() (e error) {
	...
	// unlinkat /tmp/anc-e2e-testnet-2820217771/anc-test-a/anc-test-a-node-anon-default-a-2/ibc_08-wasm/state/wasm: permission denied
	err := g.Wait()
	if err != nil {
...
func (m *Manager) RunChainInitResource(
	...
	// from that anon node, probably a new tag will need to be pushed in
...
func (m *Manager) NetworkName() string {
	return fmt.Sprintf("anc-testnet-%s", m.identifier)
}
...
// WithCurrentTag sets the current tag of the anon image in the manager
// This function is useful when we want to test an upgrade from a specific tag
// overriding the default AnonContainerTagBeforeUpgrade value
