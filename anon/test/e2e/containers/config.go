...
const (
	...
	AnonContainerTagBeforeUpgrade  = "v1.0.1"

	// name of the image produced by running `make e2e-init-chain` in contrib/images
	InitChainContainerE2E = "anon-org/anond-e2e-init-chain"
...
func NewImageConfig(isCosmosRelayer, isUpgrade bool) (ic ImageConfig) {
	ic = ImageConfig{
		CurrentRepository: AnonContainerName,
		CurrentTag:        "latest",
	}

	if isUpgrade {
		// starts at the older version and later upgrades it to current branch... AnonContainerName
		ic.CurrentRepository = AnonContainerNameBeforeUpgrade
		ic.CurrentTag = AnonContainerTagBeforeUpgrade
	}
