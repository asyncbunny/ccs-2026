...
func (s *BTCStakingPreApprovalTestSuite) Test1CreateFinalityProviderAndDelegation() {
	...
	*/
	...
	delBtcPK := anc.NewBIP340PubKeyFromBTCPK(s.delBTCSK.PubKey())
...
func (s *BTCStakingPreApprovalTestSuite) Test3SendStakingTransctionInclusionProof() {
	...
	// staking tx hash
	stakingMsgTx, err := anc.NewBTCTxFromBytes(btcDel.StakingTx)
	s.NoError(err)
...
func (s *BTCStakingPreApprovalTestSuite) Test4CommitPublicRandomnessAndSubmitFinalitySignature() {
	...
	*/
	...
	_, err = n.QueryRewardGauge(fpAnonAddr)
	...
	*/
	...
	n.SubmitRefundableTxWithAssertion(func() {
		...
		s.NoError(err)
		btcPK := anc.NewBIP340PubKeyFromBTCPK(pk)
	...
	fpRewardGauges, err := n.QueryRewardGauge(fpAnonAddr)
	s.NoError(err)
	...
	btcDelRewardGauges, err := n.QueryRewardGauge(delAnonAddr)
	s.NoError(err)
