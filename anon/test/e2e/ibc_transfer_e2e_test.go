...
func (s *IBCTransferTestSuite) IBCTransfer() {
	...
	ancChainA := s.configurer.GetChainConfig(0)
	ancChainA.WaitUntilHeight(2)
	ancChainB := s.configurer.GetChainConfig(1)
	ancChainB.WaitUntilHeight(2)

	nA, err := ancChainA.GetNodeAtIndex(2)
	s.NoError(err)
	nB, err := ancChainB.GetNodeAtIndex(2)
	s.NoError(err)

	// Check balance of val in chain-A (Node 3)
	s.addrA = nA.KeysAdd("addr-A")
	nA.BankSendFromNode(s.addrA, "10000000uanc")

	s.addrB = nB.KeysAdd("addr-B")
	nB.BankSendFromNode(s.addrB, "30000000uanc")
...
func (s *IBCTransferTestSuite) IBCTransferBack() {
	...
	nA, err := ancChainA.GetNodeAtIndex(0)
	s.NoError(err)
	nB, err := ancChainB.GetNodeAtIndex(2)
	s.NoError(err)
...
func (s *IBCTransferTestSuite) MultiCoinFee() {
	...
	cmd := []string{
		...
		"--yes",
		"--keyring-backend=test", "--log_format=json", "--home=/home/anon/anondata",
	}
	...
	feeCollectorAddr := "anc17xpfvakm2amg962yls6f84z3kell8c5l88j35y"
