...
func (n *NodeConfig) QueryBTCStakingParams() *bstypes.Params {
	bz, err := n.QueryGRPCGateway("/anon/btcstaking/v1/params", url.Values{})
...
func (n *NodeConfig) QueryBTCStakingParamsByVersion(version uint32) *bstypes.Params {
	path := fmt.Sprintf("/anon/btcstaking/v1/params/%d", version)
...
func (n *NodeConfig) QueryFinalityParams() *ftypes.Params {
	bz, err := n.QueryGRPCGateway("/anon/finality/v1/params", url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryFinalityProviders() []*bstypes.FinalityProviderResponse {
	bz, err := n.QueryGRPCGateway("/anon/btcstaking/v1/finality_providers", url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryFinalityProvider(btcPkHex string) *bstypes.FinalityProviderResponse {
	path := fmt.Sprintf("/anon/btcstaking/v1/finality_providers/%s/finality_provider", btcPkHex)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryFinalityProviderSigningInfo(btcPkHex string) ftypes.SigningInfoResponse {
	path := fmt.Sprintf("/anon/finality/v1/signing_infos/%s", btcPkHex)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryActiveFinalityProvidersAtHeight(height uint64) []*ftypes.ActiveFinalityProvidersAtHeightResponse {
	path := fmt.Sprintf("/anon/finality/v1/finality_providers/%d", height)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryFinalityProviderDelegations(fpBTCPK string) []*bstypes.BTCDelegatorDelegationsResponse {
	path := fmt.Sprintf("/anon/btcstaking/v1/finality_providers/%s/delegations", fpBTCPK)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryBtcDelegation(stakingTxHash string) *bstypes.QueryBTCDelegationResponse {
	path := fmt.Sprintf("/anon/btcstaking/v1/btc_delegation/%s", stakingTxHash)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryBtcDelegations(status bstypes.BTCDelegationStatus) *bstypes.QueryBTCDelegationsResponse {
	path := fmt.Sprintf("/anon/btcstaking/v1/btc_delegations/%d", status)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryActivatedHeight() (uint64, error) {
	bz, err := n.QueryGRPCGateway("/anon/finality/v1/activated_height", url.Values{})
	if err != nil {
		return 0, err
	}
...
func (n *NodeConfig) QueryListPublicRandomness(fpBTCPK *anc.BIP340PubKey) map[uint64]*anc.SchnorrPubRand {
	path := fmt.Sprintf("/anon/finality/v1/finality_providers/%s/public_randomness_list", fpBTCPK.MarshalHex())
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
// TODO: pagination support
func (n *NodeConfig) QueryListPubRandCommit(fpBTCPK *anc.BIP340PubKey) map[uint64]*ftypes.PubRandCommitResponse {
	path := fmt.Sprintf("/anon/finality/v1/finality_providers/%s/pub_rand_commit_list", fpBTCPK.MarshalHex())
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryVotesAtHeight(height uint64) []anc.BIP340PubKey {
	path := fmt.Sprintf("/anon/finality/v1/votes/%d", height)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryListBlocks(status ftypes.QueriedBlockStatus) []*ftypes.IndexedBlock {
	...
	bz, err := n.QueryGRPCGateway("/anon/finality/v1/blocks", values)
	require.NoError(n.t, err)
...
func (n *NodeConfig) QueryIndexedBlock(height uint64) *ftypes.IndexedBlock {
	path := fmt.Sprintf("/anon/finality/v1/blocks/%d", height)
	bz, err := n.QueryGRPCGateway(path, url.Values{})
	require.NoError(n.t, err)
