...
import (
	...
	"github.com/anon-org/anon/v4/test/e2e/containers"
	"github.com/anon-org/anon/v4/test/e2e/initialization"
	"github.com/anon-org/anon/v4/testutil/datagen"
	tkeeper "github.com/anon-org/anon/v4/testutil/keeper"
...
func (n *NodeConfig) CreateFinalityProvider(walletAddrOrName string, btcPK *anc.BIP340PubKey, pop *bstypes.ProofOfPossessionBTC, moniker, identity, website, securityContract, details string, commission *sdkmath.LegacyDec, commissionMaxRate, commissionMaxRateChange sdkmath.LegacyDec) {
	...
	cmd := []string{
		"anond", "tx", "btcstaking", "create-finality-provider", btcPKHex, popHex,
...
func (n *NodeConfig) CreateBTCDelegation(
	...
	unbondingTxBytes, err := anc.SerializeBTCTx(unbondingTx)
	require.NoError(n.t, err)
	...
	cmd := []string{
		"anond", "tx", "btcstaking", "create-btc-delegation",
	...
	// gas price
	cmd = append(cmd, "--gas-prices=0.1uanc")
...
func (n *NodeConfig) CreateBTCStakeExpansionDelegation(
	...
	var inclusionProofHex string
	cmd := []string{
		"anond", "tx", "btcstaking", "btc-stake-expand",
...
func (n *NodeConfig) AddCovenantSigsFromVal(covPK *anc.BIP340PubKey, stakingTxHash string, slashingSigs [][]byte, unbondingSig *anc.BIP340Signature, unbondingSlashingSigs [][]byte, stakeExpTxSig *anc.BIP340Signature) {
	n.AddCovenantSigs("val", covPK, stakingTxHash, slashingSigs, unbondingSig, unbondingSlashingSigs, stakeExpTxSig)
}

func (n *NodeConfig) AddCovenantSigsFromValForStakeExp(covPK *anc.BIP340PubKey, stakingTxHash string, slashingSigs [][]byte, unbondingSig *anc.BIP340Signature, unbondingSlashingSigs [][]byte, stkExpSig *anc.BIP340Signature) string {
...
func (n *NodeConfig) AddCovenantSigs(
	fromWalletName string,
	covPK *anc.BIP340PubKey,
	stakingTxHash string,
	slashingSigs [][]byte,
	unbondingSig *anc.BIP340Signature,
	unbondingSlashingSigs [][]byte,
	stakeExpTxSig *anc.BIP340Signature,
) string {
	n.LogActionF("adding covenant signature from nodeName: %s", n.Name)

	covPKHex := covPK.MarshalHex()

	cmd := []string{"anond", "tx", "btcstaking", "add-covenant-sigs", covPKHex, stakingTxHash}
...
func (n *NodeConfig) AddFinalitySignatureToBlockWithContext(
	...
	finalitySig := anc.NewSchnorrEOTSSigFromModNScalar(fp1Sig)
...
func (n *NodeConfig) CommitPubRandListFromNode(fpBTCPK *anc.BIP340PubKey, startHeight uint64, numPubrand uint64, commitment []byte, sig *anc.BIP340Signature) {
	n.CommitPubRandList("val", fpBTCPK, startHeight, numPubrand, commitment, sig)
}

func (n *NodeConfig) CommitPubRandList(fromWallet string, fpBTCPK *anc.BIP340PubKey, startHeight uint64, numPubrand uint64, commitment []byte, sig *anc.BIP340Signature) {
	n.LogActionF("committing public randomness list")

	cmd := []string{"anond", "tx", "finality", "commit-pubrand-list"}
...
func (n *NodeConfig) AddFinalitySig(
	...
	proof cmtcrypto.Proof,
	appHash []byte,
	finalitySig *anc.SchnorrEOTSSig,
	overallFlags ...string,
) {
	...
	cmd := []string{
		"anond", "tx", "finality", "add-finality-sig",
		fpBTCPKHex, blockHeightStr, pubRandHex, proofHex, appHashHex, finalitySigHex, "--gas=500000",
	}
	additionalArgs := []string{fmt.Sprintf("--chain-id=%s", n.chainId), "--gas-prices=0.1uanc", "-b=sync", "--yes", "--keyring-backend=test", "--log_format=json", "--home=/home/anon/anondata"}
...
func (n *NodeConfig) AddCovenantUnbondingSigs(
	covPK *anc.BIP340PubKey,
	stakingTxHash string,
	unbondingTxSig *anc.BIP340Signature,
	...
	cmd := []string{"anond", "tx", "btcstaking", "add-covenant-unbonding-sigs", covPKHex, stakingTxHash, unbondingTxSigHex}
...
func (n *NodeConfig) BTCUndelegate(
	...
	spendStakeTxBytes, err := anc.SerializeBTCTx(spendStakeTx)
	require.NoError(n.t, err)
	...
	for i, tx := range fundingTxs {
		fundingTxBytes, err := anc.SerializeBTCTx(tx)
		require.NoError(n.t, err)
		fundingTxsHex[i] = hex.EncodeToString(fundingTxBytes)
	}
	fundingTxsHexStr := strings.Join(fundingTxsHex, ",")

	cmd := []string{"anond", "tx", "btcstaking", "btc-undelegate", stakingTxHash.String(), spendStakeTxHex, inclusionProofHex, fundingTxsHexStr, "--from=val", "--gas=500000"}
...
func (n *NodeConfig) AddBTCDelegationInclusionProof(
	...
	cmd := []string{"anond", "tx", "btcstaking", "add-btc-inclusion-proof", stakingTxHash.String(), inclusionProofHex, "--from=val"}
	_, _, err = n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) BTCStakingUnbondSlashInfo(
	...
	// make block k-deep
	for i := 0; i < initialization.AnonBtcConfirmationPeriod; i++ {
...
func (n *NodeConfig) CreateBTCDel(
	...
	n.CreateBTCDelegation(
		anc.NewBIP340PubKeyFromBTCPK(btcStakerSK.PubKey()),
		popDel1,
		stakingTx,
		inclusionProof,
		fp.BtcPk,
...
func (n *NodeConfig) CreateBTCStakeExpDelegationAndCheck(
	...
	fundingTx, err := anc.NewBTCTxFromBytes(msg.FundingTx)
	require.NoError(t, err)
...
func (n *NodeConfig) createBtcStakeExpandMessage(
	...
	var fpBtcPkList []anc.BIP340PubKey
	for _, fp := range fps {
		fpBtcPkList = append(fpBtcPkList, *fp.BtcPk)
	}
	fpPKs, err := anc.NewBTCPKsFromBIP340PKs(fpBtcPkList)
	require.NoError(t, err)
	...
	serializedStakingTx, err := anc.SerializeBTCTx(stakingSlashingInfo.StakingTx)
	require.NoError(t, err)
	...
	fundingTxBz, err := anc.SerializeBTCTx(fundingTx)
	require.NoError(t, err)
...
func (n *NodeConfig) SendCovenantSigs(
	...
	stakingMsgTx, err := anc.NewBTCTxFromBytes(stakingTx)
	require.NoError(t, err)
	stakingTxHash := stakingMsgTx.TxHash().String()

	fpBTCPKs, err := anc.NewBTCPKsFromBIP340PKs(pendingDel.FpBtcPkList)
	require.NoError(t, err)
	...
	*/
	...
	require.NoError(t, err)
	unbondingTx, err := anc.NewBTCTxFromBytes(pendingDel.BtcUndelegation.UnbondingTx)
	require.NoError(t, err)
	...
	covStkExpSigs := []*anc.BIP340Signature{}
	...
	for i := 0; i < int(params.CovenantQuorum); i++ {
		// add covenant sigs
		var stkExpSig *anc.BIP340Signature
		if pendingDel.IsStakeExpansion() {
		...
		txHashes[i] = n.AddCovenantSigs(
			...
			anc.NewBIP340SignatureFromBTCSig(covUnbondingSigs[i]),
