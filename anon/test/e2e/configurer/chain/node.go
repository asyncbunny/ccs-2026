...
func (n *NodeConfig) Run() error {
	...
	require.Eventually(
		...
		3*time.Minute,
		2*time.Second,
		"anc node failed to produce blocks",
	)
...
func (n *NodeConfig) extractOperatorAddressIfValidator() error {
	...
	cmd := []string{"anond", "debug", "addr", hex.EncodeToString(n.PublicKey)}
	...
	// Match the full bech32 validator operator address (ancvaloper1... with ~59 chars total)
	re := regexp.MustCompile(`ancvaloper1[a-z0-9]{38,59}`)
...
func (n *NodeConfig) Status() (*coretypes.ResultStatus, error) {
	cmd := []string{"anond", "status", "--output=json"}
