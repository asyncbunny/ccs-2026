...
import (
	...
	"github.com/anon-org/anon/v4/test/e2e/util"
	...
	blc "github.com/anon-org/anon/v4/x/btclightclient/types"
	cttypes "github.com/anon-org/anon/v4/x/checkpointing/types"
...
func (n *NodeConfig) GetWallet(walletName string) string {
	...
	cmd := []string{"anond", "keys", "show", walletName, flagKeyringTest, containers.FlagHome}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "")
	require.NoError(n.t, err)
	re := regexp.MustCompile("anc(.{39})")
...
func (n *NodeConfig) KeysAdd(walletName string, overallFlags ...string) string {
	...
	cmd := []string{"anond", "keys", "add", walletName, flagKeyringTest, containers.FlagHome}
...
func (n *NodeConfig) QueryParams(module string, result any) {
	cmd := []string{"anond", "query", module, "params", "--output=json"}
...
func (n *NodeConfig) SendIBCTransfer(from, recipient, memo string, token sdk.Coin) (txHash string) {
	...
	cmd := []string{"anond", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, token.String(), fmt.Sprintf("--from=%s", from), "--memo", memo}
...
func (n *NodeConfig) FailIBCTransfer(from, recipient, amount string) {
	...
	cmd := []string{"anond", "tx", "ibc-transfer", "transfer", "transfer", "channel-0", recipient, amount, fmt.Sprintf("--from=%s", from)}
...
func (n *NodeConfig) BankSend(fromWallet, to, amount string, overallFlags ...string) {
	...
	cmd := []string{"anond", "tx", "bank", "send", fromAddr, to, amount, fmt.Sprintf("--from=%s", fromWallet)}
...
func (n *NodeConfig) BankMultiSend(fromWallet string, receivers []string, amount string, overallFlags ...string) {
	...
	cmd := []string{"anond", "tx", "bank", "multi-send", fromAddr} // starts the initial flags
...
func (n *NodeConfig) BankSendOutput(fromWallet, to, amount string, overallFlags ...string) (out bytes.Buffer, errBuff bytes.Buffer, err error) {
	...
	n.LogActionF("bank sending %s from wallet %s to %s", amount, fromWallet, to)
	cmd := []string{
		"anond", "tx", "bank", "send", fromAddr, to, amount, fmt.Sprintf("--from=%s", fromWallet),
		n.FlagChainID(), "-b=sync", "--yes", "--keyring-backend=test", "--log_format=json", "--home=/home/anon/anondata",
	}
...
func (n *NodeConfig) SendHeaderHex(headerHex string) {
	...
	cmd := []string{"anond", "tx", "btclightclient", "insert-headers", headerHex, "--from=val", "--gas=500000"}
...
func (n *NodeConfig) InsertHeader(h *anc.BTCHeaderBytes) {
	tip, err := n.QueryTip()
	require.NoError(n.t, err)
...
func (n *NodeConfig) InsertProofs(p1 *btccheckpointtypes.BTCSpvProof, p2 *btccheckpointtypes.BTCSpvProof) {
	...
	cmd := []string{"anond", "tx", "btccheckpoint", "insert-proofs", p1HexBytes, p2HexBytes, "--from=val"}
	_, _, err = n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) FinalizeSealedEpochs(startEpoch uint64, lastEpoch uint64) {
	...
	for _, checkpoint := range resp.RawCheckpoints {
		...
		anonTagBytes, err := hex.DecodeString(initialization.AnonOpReturnTag)
		require.NoError(n.t, err)
	...
	if madeProgress {
		...
		for i := 0; i < initialization.AnonBtcFinalizationPeriod; i++ {
...
func (n *NodeConfig) StoreWasmCode(wasmFile, from string) {
	...
	cmd := []string{"anond", "tx", "wasm", "store", wasmFile, fmt.Sprintf("--from=%s", from), "--gas=auto", "--gas-adjustment=1.3"}
...
func (n *NodeConfig) InstantiateWasmContract(codeId, initMsg, from string) {
	...
	cmd := []string{"anond", "tx", "wasm", "instantiate", codeId, initMsg, fmt.Sprintf("--from=%s", from), "--no-admin", "--label=contract", "--gas-adjustment=1.3"}
	n.LogActionF("Executing command: %s", strings.Join(cmd, " "))
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) WasmExecute(contract, execMsg, from string) {
	...
	cmd := []string{"anond", "tx", "wasm", "execute", contract, execMsg, fmt.Sprintf("--from=%s", from)}
	n.LogActionF("Executing command: %s", strings.Join(cmd, " "))
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) WithdrawReward(sType, from string) (txHash string) {
	...
	cmd := []string{"anond", "tx", "incentive", "withdraw-reward", sType, fmt.Sprintf("--from=%s", from)}
	n.LogActionF("Executing command: %s", strings.Join(cmd, " "))
	outBuf, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, cmd)
	require.NoError(n.t, err)
...
func (n *NodeConfig) TxMultisign(walletNameMultisig, txFileFullPath, outputFileName string, signedFiles []string, overallFlags ...string) (signedTxFilePath string) {
	...
	cmd := []string{
		"anond", "tx", "multisign", txFileFullPath, walletNameMultisig,
		n.FlagChainID(),
...
func (n *NodeConfig) TxBroadcast(txSignedFileFullPath string, overallFlags ...string) {
	...
	cmd := []string{
		"anond", "tx", "broadcast", txSignedFileFullPath,
		n.FlagChainID(),
	}
...
func (n *NodeConfig) TxFeeGrant(granter, grantee string, overallFlags ...string) {
	...
	cmd := []string{
		"anond", "tx", "feegrant", "grant", granter, grantee,
		n.FlagChainID(),
	}
	_, _, err := n.containerManager.ExecTxCmd(n.t, n.chainId, n.Name, append(cmd, overallFlags...))
	require.NoError(n.t, err)
...
// anon home inside the container and returns the full file path inside the container.
func (n *NodeConfig) WriteFile(fileName, content string) (fullFilePathInContainer string) {
	...
	return filepath.Join(containers.AnonHomePath, fileName)
}
...
func (n *NodeConfig) TxGovPropSubmitProposal(proposalJsonFilePath, from string, overallFlags ...string) int {
	...
	cmd := []string{
		"anond", "tx", "gov", "submit-proposal", proposalJsonFilePath,
...
func (n *NodeConfig) TxGovVote(from string, propID int, option govv1.VoteOption, overallFlags ...string) {
	...
	cmd := []string{
		"anond", "tx", "gov", "vote", fmt.Sprintf("%d", propID), option.String(),
		fmt.Sprintf("--from=%s", from),
	}
...
func (n *NodeConfig) Delegate(fromWallet, validator string, amount string, overallFlags ...string) (txHash string) {
	...
	cmd := []string{"anond", "tx", "epoching", "delegate", validator, amount, fmt.Sprintf("--from=%s", fromWallet)}
...
func (n *NodeConfig) ValidatorConsPubKey() cryptotypes.PubKey {
	cmd := []string{"anond", "comet", "show-validator", "--home=/home/anon/anondata"}
	outBuf, _, err := n.containerManager.ExecCmd(n.t, n.Name, cmd, "")
	require.NoError(n.t, err)
