...
import (
	...
	types2 "github.com/anon-org/anon/v4/x/btccheckpoint/types"
...
func (bc *baseConfigurer) InstantiateAnonContract() error {
	// Store the contract on the second chain (B)
	chainConfig := bc.chainConfigs[1]
	contractPath := "/bytecode/anon_contract.wasm"
	...
	initMsg := fmt.Sprintf(`{ "network": %q, "anon_tag": %q, "btc_confirmation_depth": %d, "checkpoint_finalization_timeout": %d, "notify_cosmos_zone": %s }`,
		types.BtcRegtest,
...
func (bc *baseConfigurer) RunCosmosRelayerIBC() error {
	...
	// Launches a relayer between chain A (anond) and chain B (wasmd)
	return nil
}
...
func (bc *baseConfigurer) runHermesIBCRelayer(chainConfigA *chain.Config, chainConfigB *chain.Config) error {
	...
	tmpDir, err := os.MkdirTemp("", "anc-e2e-testnet-hermes-")
	if err != nil {
		return err
	}
...
func (bc *baseConfigurer) runCosmosIBCRelayer(chainConfigA *chain.Config, chainConfigB *chain.Config) error {
	...
	tmpDir, err := os.MkdirTemp("", "anc-e2e-testnet-cosmos-")
	if err != nil {
		return err
	}
