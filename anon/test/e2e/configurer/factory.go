...
const (
	btcNetworkStr = string(anc.BtcSimnet)
)
...
// NewAnonConfigurer returns a new Configurer for BTC staking service
func NewAnonConfigurer(t *testing.T, isDebugLogEnabled bool) (Configurer, error) {
	identifier := identifierName(t)
	containerManager, err := containers.NewManager(identifier, isDebugLogEnabled, false, false)
	if err != nil {
		return nil, err
