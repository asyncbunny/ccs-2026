...
import (
	...
	"github.com/anon-org/anon/v4/test/e2e/configurer"
	"github.com/anon-org/anon/v4/test/e2e/configurer/chain"
	"github.com/anon-org/anon/v4/test/e2e/util"
	"github.com/anon-org/anon/v4/testutil/datagen"
	etypes "github.com/anon-org/anon/v4/x/epoching/types"
...
func (s *EpochingSpamPreventionTestSuite) SetupSuite() {
	...
	s.configurer, err = configurer.NewAnonConfigurer(s.T(), true)
	s.NoError(err)
...
func (s *EpochingSpamPreventionTestSuite) TestNormalDelegationCase() {
	...
	// Use anond query staking validators to get actual validator addresses
	validatorAddr := ""
	...
	initialDelegatorBalance, err := nonValidatorNode.QueryBalance(delegatorAddr, "uanc")
	s.NoError(err)
	...
	initialModuleBalance, err := nonValidatorNode.QueryBalance(epochingModuleAddrStr, "uanc")
	s.NoError(err)
	s.T().Logf("Initial epoching module balance: %s", initialModuleBalance.String())

	// Delegation amount
	delegationAmount := sdkmath.NewInt(1000000) // 1 ANC

	// Step 1: Send WrappedMsgDelegate and verify funds are locked
	s.T().Logf("Step 1: Sending WrappedMsgDelegate for %s uanc", delegationAmount.String())

	// Send anond tx epoching delegate command using the existing Delegate method
	delegationAmountCoin := delegationAmount.String() + "uanc"
	s.T().Logf("Executing: anond tx epoching delegate %s %s --from=%s",
	...
	delegatorBalanceAfterLock, err := nonValidatorNode.QueryBalance(delegatorAddr, "uanc")
	s.NoError(err)
	...
	moduleBalanceAfterLock, err := nonValidatorNode.QueryBalance(epochingModuleAddrStr, "uanc")
	s.NoError(err)
	...
	finalDelegatorBalance, err := nonValidatorNode.QueryBalance(delegatorAddr, "uanc")
	s.NoError(err)

	finalModuleBalance, err := nonValidatorNode.QueryBalance(epochingModuleAddrStr, "uanc")
	s.NoError(err)
	...
	delegationCmd := []string{
		"anond", "query", "staking", "delegation", delegatorAddr, validatorAddr,
		"--output=json", "--home=/home/anon/anondata",
	}
...
func (s *EpochingSpamPreventionTestSuite) TestNormalCreateValidatorCase() {
	...
	fundingAmount := "100000000uanc"
	nonValidatorNode.BankSend(nonValidatorNode.WalletName, newValidatorAddr, fundingAmount)
	chainA.WaitForNumHeights(2)

	initialBalance, err := nonValidatorNode.QueryBalance(newValidatorAddr, "uanc")
...
EOF`, string(blsPopJSON))})
		...
		createValCmd := []string{
			"anond", "tx", "checkpointing", "create-validator", "/tmp/validator.json",
			"--bls-pop", "/tmp/bls_pop.json",
			fmt.Sprintf("--from=%s", walletName),
			"--keyring-backend=test",
			"--home=/home/anon/anondata",
	...
	epochResponse, err := func() (*etypes.QueryCurrentEpochResponse, error) {
		bz, err := nonValidatorNode.QueryGRPCGateway("/anon/epoching/v1/current_epoch", url.Values{})
		if err != nil {
			return nil, err
		}
	...
	stakingAmountInt := sdkmath.NewInt(50000000) // 50 ANC
	...
	afterBalance, err := nonValidatorNode.QueryBalance(newValidatorAddr, "uanc")
	s.NoError(err)
	...
	expectedSelfDelegation := sdkmath.NewInt(50000000) // 50 ANC
