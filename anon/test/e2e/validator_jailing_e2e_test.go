...
func (s *ValidatorJailingTestSuite) TestValidatorJailingWithExtraDelegation() {
	...
	s.T().Logf("Need to delegate %s uanc to validator 1", additionalDelegation.String())

	// Delegate from non-validator node to validator 1
	delegationAmount := additionalDelegation.String() + "uanc"
	...
	// delegate 1 uanc to validator 2 as well
	txHash = nonValidatorNode.Delegate(nonValidatorNode.WalletName, val2.OperatorAddress, "1uanc", "--gas=500000")

	chainA.WaitForNumHeights(2)
	res, _ = nonValidatorNode.QueryTx(txHash)
	s.Equal(res.Code, uint32(0), res.RawLog)

	// Wait for delegation to take effect - need to wait for epoch end in Anon
