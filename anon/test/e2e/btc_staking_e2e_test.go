...
import (
	...
	itypes "github.com/anon-org/anon/v4/x/incentive/types"
)
...
func (s *BTCStakingTestSuite) Test3CommitPublicRandomnessAndSubmitFinalitySignature() {
	...
	*/
	...
	fpAnonAddr, err := sdk.AccAddressFromBech32(s.cacheFP.Addr)
	s.NoError(err)

	_, err = nonValidatorNode.QueryRewardGauge(fpAnonAddr)
	s.ErrorContains(err, itypes.ErrRewardGaugeNotFound.Error())
	delAnonAddr := fpAnonAddr
	...
	*/
	...
	fpRewardGauges, err := nonValidatorNode.QueryRewardGauge(fpAnonAddr)
	s.NoError(err)
	...
	btcDelRewardGauges, err := nonValidatorNode.QueryRewardGauge(delAnonAddr)
	s.NoError(err)
...
func (s *BTCStakingTestSuite) Test5SubmitStakerUnbonding() {
	...
	// staking tx hash
	stakingMsgTx, err := anc.NewBTCTxFromBytes(activeDel.StakingTx)
	s.NoError(err)
...
func (s *BTCStakingTestSuite) Test6MultisigBTCDelegation() {
	...
	nonValidatorNode.BankSendFromNode(multisigAddr, "10000000uanc")
	...
	jsonTx := nonValidatorNode.CreateBTCDelegation(
		anc.NewBIP340PubKeyFromBTCPK(s.delBTCSK.PubKey()),
		pop,
		stakingTx,
		inclusionProof,
		s.cacheFP.BtcPk,
...
func (s *BTCStakingTestSuite) Test8BTCDelegationFeeGrantTyped() {
	...
	// TODO: Uncomment the next lines when issue: https://github.com/anon-org/anon/v4/issues/693
	...
	// 	anc.NewBIP340PubKeyFromBTCPK(delBTCPK),
	// 	pop,
	// 	stakingTxInfo,
	// 	cacheFP.BtcPk,
	// 	stakingTimeBlocks,
...
func (s *BTCStakingTestSuite) Test9BlockBankSendAndBTCDelegate() {
	...
	// submit the message to create BTC delegation using the fee grant at the max of spend limit
	btcPK := anc.NewBIP340PubKeyFromBTCPK(s.delBTCSK.PubKey())
...
func CreateNodeFP(
	...
	newFP, err = datagen.GenRandomFinalityProviderWithBTCAnonSKs(r, fpSk, nodeAddr)
	require.NoError(t, err)
