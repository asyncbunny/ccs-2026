...
import (
	...
	"github.com/anon-org/anon/v4/testutil/coins"
...
func (s *BtcRewardsDistribution) CreateFinalityProviders() {
	...
	n2.BankMultiSendFromNode([]string{s.fp1Addr, s.fp2Addr}, "1000000uanc")
...
func (s *BtcRewardsDistribution) CreateFirstBtcDelegations() {
	...
	n2.BankMultiSendFromNode([]string{s.del1Addr, s.del2Addr}, "10000000uanc")
...
func (s *BtcRewardsDistribution) CommitPublicRandomnessAndSealed() {
	...
	for _, fp := range fps {
		...
		s.Require().Zero(fp.SlashedAnonHeight, "fp is slashed")
...
func (s *BtcRewardsDistribution) CheckRewardsFirstDelegations() {
	...
	for _, fp := range fps {
		s.Equal(fp.SlashedAnonHeight, uint64(0))
	...
	coins.RequireCoinsDiffInPointOnePercentMargin(
		s.T(),
		fp2DiffRewards.Coins.MulInt(sdkmath.NewIntFromUint64(3)).Add(sdk.NewCoin("uanc", sdkmath.NewInt(2))), // truncation rounding
...
func (s *BtcRewardsDistribution) SlashFp() {
	...
	for _, fp := range fps {
		if strings.EqualFold(fp.Addr, s.fp1Addr) {
			require.Zero(s.T(), fp.SlashedAnonHeight)
			continue
		}
		require.NotZero(s.T(), fp.SlashedAnonHeight)
	}
...
func (s *BaseBtcRewardsDistribution) CreateCovenantsAndSubmitSignaturesToPendDels(
	...
	n.BankMultiSendFromNode(covAddrs, "200000000uanc")

	fpBtcPks := make([]string, len(fps))
	for i, fp := range fps {
		fpBtcPks[i] = fp.BtcPk.MarshalHex()
	}
