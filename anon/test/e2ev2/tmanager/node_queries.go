...
func (n *Node) QueryCostkRwdTrckCli(addr sdk.AccAddress) *costktypes.QueryCostakerRewardsTrackerResponse {
	cmd := []string{"anond", "query", "costaking", "costaker-rewards-tracker", addr.String(), "--output=json", "--node", n.GetRpcEndpoint()}
...
func (n *Node) QueryCostkCurrRwdCli() *costktypes.QueryCurrentRewardsResponse {
	cmd := []string{"anond", "query", "costaking", "current-rewards", "--output=json", "--node", n.GetRpcEndpoint()}
	outBuf, _, err := n.Tm.ContainerManager.ExecCmd(n.T(), n.Container.Name, cmd, "")
	require.NoError(n.T(), err)
...
func (n *Node) QueryLatestEpochHeaderCLI(consumerID string) string {
	cmd := []string{"anond", "query", "zc", "latest-epoch-header", consumerID, "--output=json", "--node", n.GetRpcEndpoint()}
	outBuf, _, err := n.Tm.ContainerManager.ExecCmd(n.T(), n.Container.Name, cmd, "")
	require.NoError(n.T(), err)
...
func (n *Node) QueryGetSealedEpochProofCLI(epochNum uint64) string {
	cmd := []string{"anond", "query", "zc", "get-sealed-epoch-proof", fmt.Sprintf("%d", epochNum), "--output=json", "--node", n.GetRpcEndpoint()}
	outBuf, _, err := n.Tm.ContainerManager.ExecCmd(n.T(), n.Container.Name, cmd, "")
	require.NoError(n.T(), err)
	return outBuf.String()
}
