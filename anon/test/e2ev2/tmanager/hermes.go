...
func (hr *HermesRelayer) RunResource(cA, cB *Chain) *dockertest.Resource {
	...
	runOpts := &dockertest.RunOptions{
		...
		Env: []string{
			...
			fmt.Sprintf("ANC_A_E2E_CHAIN_ID=%s", cA.ChainID()),
			fmt.Sprintf("ANC_B_E2E_CHAIN_ID=%s", cB.ChainID()),
			fmt.Sprintf("ANC_A_E2E_VAL_MNEMONIC=%s", rnA.DefaultWallet().Mnemonic),
			fmt.Sprintf("ANC_B_E2E_VAL_MNEMONIC=%s", rnB.DefaultWallet().Mnemonic),
			fmt.Sprintf("ANC_A_E2E_VAL_HOST_GRPC=%s:%d", rnA.Container.Name, rnA.Ports.GRPC),
			fmt.Sprintf("ANC_A_E2E_VAL_HOST_RPC=%s:%d", rnA.Container.Name, rnA.Ports.RPC),
			fmt.Sprintf("ANC_B_E2E_VAL_HOST_GRPC=%s:%d", rnB.Container.Name, rnB.Ports.GRPC),
			fmt.Sprintf("ANC_B_E2E_VAL_HOST_RPC=%s:%d", rnB.Container.Name, rnB.Ports.RPC),
		},
		Entrypoint: []string{
			"sh",
			"-c",
