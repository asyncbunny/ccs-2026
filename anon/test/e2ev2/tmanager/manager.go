...
func NewTestManager(t *testing.T) *TestManager {
	...
	network, err := pool.CreateNetwork(fmt.Sprintf("anc-e2e-%s", GenerateNetworkID(t)))
	if err != nil {
...
func NewTmWithUpgrade(
	...
	tm := NewTestManager(t)
	ancCfg := NewChainConfig(tm.TempDir, CHAIN_ID_ANON)
	ancCfg.IsUpgrade = true
	// if tag is empty string, use default tag v4.2.2
	if tag == "" {
		tag = AnonContainerTagBeforeUpgrade
	}
	ancCfg.Tag = tag
	ancCfg.BootstrapRepository = AnonContainerNameBeforeUpgrade
	tm.Chains[CHAIN_ID_ANON] = NewChain(tm, ancCfg)
...
func (tm *TestManager) ChainValidator() *ValidatorNode {
	return tm.Chains[CHAIN_ID_ANON].Validators[0]
}
...
func (tm *TestManagerIbc) ChainANC() *Chain {
