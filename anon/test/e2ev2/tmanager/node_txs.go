...
func (n *Node) FinalizeSealedEpochs(startEpoch uint64, lastEpoch uint64) {
	...
	for _, checkpoint := range resp.RawCheckpoints {
		...
		anonTagBytes, err := hex.DecodeString(AnonOpReturnTag)
		require.NoError(n.T(), err)
	...
	if madeProgress {
		...
		for i := 0; i < AnonBtcFinalizationPeriod; i++ {
...
func (n *Node) BuildSingleSigDelegationMsg(
	...
	serializedStakingTx, err := anc.SerializeBTCTx(stakingInfo.StakingTx)
	require.NoError(n.T(), err)
	serializedUnbondingTx, err := anc.SerializeBTCTx(unbondingInfo.UnbondingTx)
	require.NoError(n.T(), err)

	return &bstypes.MsgCreateBTCDelegation{
		StakerAddr:                    wallet.Address.String(),
		BtcPk:                         anc.NewBIP340PubKeyFromBTCPK(stakerSK.PubKey()),
...
func (n *Node) CreateBtcDelegation(wallet *WalletSender, fpPK *btcec.PublicKey) *bstypes.BTCDelegationResponse {
	...
	*/
	...
	stakingMsgTx, err := anc.NewBTCTxFromBytes(pendingDel.StakingTx)
	require.NoError(n.T(), err)
	...
	*/
	...
	for i := 0; i < AnonBtcConfirmationPeriod; i++ {
		n.InsertNewEmptyBtcHeader(n.Tm.R)
	}
