...
const (
	AnonBtcConfirmationPeriod = 2
	AnonBtcFinalizationPeriod = 4
	AnonOpReturnTag           = "01020304"
)

var (
	SelfDelegationAmt     = sdkmath.NewInt(10_000_000000) // 10k uanc
...
type StartingBtcStakingParams struct {
	CovenantCommittee []anc.BIP340PubKey
...
func UpdateGenesisBtcLightClient(btcHeaders []*btclighttypes.BTCHeaderInfo) func(blcGenState *btclighttypes.GenesisState) {
	return func(blcGenState *btclighttypes.GenesisState) {
		...
		baseBtcHeader, err := anc.NewBTCHeaderBytesFromHex(btcSimnetGenesisHex)
		if err != nil {
			panic(err)
		}
...
func UpdateGenesisBtccheckpoint(btccheckpointGenState *btccheckpointtypes.GenesisState) {
	...
	btccheckpointGenState.Params.BtcConfirmationDepth = AnonBtcConfirmationPeriod
	btccheckpointGenState.Params.CheckpointFinalizationTimeout = AnonBtcFinalizationPeriod
	btccheckpointGenState.Params.CheckpointTag = AnonOpReturnTag
}
