...
func (s *Staker) BTCPublicKey() *anc.BIP340PubKey {
	pk := anc.NewBIP340PubKeyFromBTCPK(s.BTCPrivateKey.PubKey())
	return pk
}
...
func (s *Staker) CreateBtcExpandMessage(
	...
	fpBtcPkList := make([]anc.BIP340PubKey, len(fpKeys))
...
func (s *Staker) UnbondDelegation(
	...
	stakingTxBz, err := anc.SerializeBTCTx(stakingTx)
	require.NoError(s.t, err)

	unbondingTxBytes, err := anc.SerializeBTCTx(unbondingTxMsg)
	require.NoError(s.t, err)
