...
func TestExpandBTCDelegation(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			fundingSetup: func(s *testSetup) (*wire.MsgTx, uint32) {
				fundingTx, _, err := anc.NewBTCTxFromHex(s.ActiveDelegations[1].StakingTxHex)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			prevStkTx, prevStkTxBz, err := anc.NewBTCTxFromHex(s.ActiveDelegations[0].StakingTxHex)
			require.NoError(t, err)
			...
			btcExpMsg := s.Staker.CreateBtcExpandMessage(
				[]*anc.BIP340PubKey{s.Fp.BTCPublicKey()},
				stakingTime,
				stakingValue,
			...
			spendingTx, err := anc.NewBTCTxFromBytes(btcExpMsg.StakingTx)
			require.NoError(t, err)
...
func TestInvalidStakeExpansion(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			testCase: func(s *testSetup) {
				prevStkTx, _, err := anc.NewBTCTxFromHex(s.ActiveDelegations[0].StakingTxHex)
				require.NoError(t, err)
		...
		},
		{
			...
			testCase: func(s *testSetup) {
				...
				stkExpStakingTx, err := anc.NewBTCTxFromBytes(stakeExpandMsg.StakingTx)
				require.NoError(t, err)
		...
		},
		{
			...
			testCase: func(s *testSetup) {
				...
				params := s.Driver.GetBTCStakingParams(t)
				unbondingTx, _, err := anc.NewBTCTxFromHex(s.ActiveDelegations[0].UndelegationResponse.UnbondingTxHex)
				require.NoError(t, err)
...
func setupTest(t *testing.T) *testSetup {
	...
	driver := NewAnonAppDriver(r, t, driverTempDir, replayerTempDir)
