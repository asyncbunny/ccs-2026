...
import (
	...
	anonApp "github.com/anon-org/anon/v4/app"
	appparams "github.com/anon-org/anon/v4/app/params"
	appsigner "github.com/anon-org/anon/v4/app/signer"
	"github.com/anon-org/anon/v4/test/e2e/initialization"
	"github.com/anon-org/anon/v4/testutil/datagen"
	btclighttypes "github.com/anon-org/anon/v4/x/btclightclient/types"
...
var (
	...
	defaultFeeCoin                   = sdk.NewCoin("uanc", math.NewInt(defaultFee))
...
// NewAnonAppDriverTmpDir initializes Anon driver for block creation with
// temporary directories
func NewAnonAppDriverTmpDir(r *rand.Rand, t *testing.T) *AnonAppDriver {
	driverTempDir := t.TempDir()
	replayerTempDir := t.TempDir()
	return NewAnonAppDriver(r, t, driverTempDir, replayerTempDir)
}

// Inititializes Anon driver for block creation
func NewAnonAppDriver(
	r *rand.Rand,
	t *testing.T,
	dir string,
	copyDir string,
	...
	chain, err := initialization.InitChain(
		...
		&initialization.StartingBtcStakingParams{
			CovenantCommittee: anc.NewBIP340PKsFromBTCPKs(pks),
...
func (d *AnonAppDriver) Ctx() sdk.Context {
	return d.GetContextForLastFinalizedBlock()
}

func (d *AnonAppDriver) GetLastFinalizedBlock() *FinalizedBlock {
...
func (d *AnonAppDriver) GetContextForLastFinalizedBlock() sdk.Context {
...
func (d *AnonAppDriver) GenerateNewBlockReturnResults() []*abci.ExecTxResult {
...
func (d *AnonAppDriver) SendTxWithMessagesSuccess(
	t *testing.T,
	senderInfo *SenderInfo,
	gas uint64,
	fee sdk.Coin,
...
func (d *AnonAppDriver) GenerateNewBlock() *abci.ResponseFinalizeBlock {
...
func (d *AnonAppDriver) GetFinalizedBlocks() []FinalizedBlock {
	lastState, err := d.StateStore.Load()
	require.NoError(d.t, err)
	require.NotNil(d.t, lastState)
...
func (d *AnonAppDriver) GetLastState() sm.State {
	lastState, err := d.StateStore.Load()
	require.NoError(d.t, err)
	require.NotNil(d.t, lastState)
	return lastState
...
func (d *AnonAppDriver) GenerateBlocksUntilHeight(untilBlock uint64) {
...
func (d *AnonAppDriver) GenerateNewBlockAssertExecutionSuccess() {
	d.GenerateNewBlockAssertExecutionSuccessWithResults()
}

func (d *AnonAppDriver) GenerateNewBlockAssertExecutionFailure() []*abci.ExecTxResult {
	response := d.GenerateNewBlock()
...
func (d *AnonAppDriver) GetDriverAccountAddress() sdk.AccAddress {
	return sdk.AccAddress(d.SenderInfo.privKey.PubKey().Address())
}

func BlocksWithProofsToHeaderBytes(blocks []*datagen.BlockWithProofs) []anc.BTCHeaderBytes {
	headerBytes := []anc.BTCHeaderBytes{}
	for _, block := range blocks {
		headerBytes = append(headerBytes, anc.NewBTCHeaderBytesFromBlockHeader(&block.Block.Header))
	}
	return headerBytes
}

func (d *AnonAppDriver) ExtendBTCLcWithNEmptyBlocks(
	r *rand.Rand,
	t *testing.T,
	n uint32,
...
func (d *AnonAppDriver) GenBlockWithTransactions(
	r *rand.Rand,
	t *testing.T,
	txs []*wire.MsgTx,
...
func (d *AnonAppDriver) ActivateVerifiedDelegations(expectedVerifiedDelegations int) []*abci.ResponseFinalizeBlock {
	btcBlock, ancBlock := d.IncludeVerifiedStakingTxInBTC(expectedVerifiedDelegations)
...
func (d *AnonAppDriver) IncludeVerifiedStakingTxInBTC(expectedVerifiedDelegations int) (btcBlock *datagen.BlockWithProofs, ancBlock *abci.ResponseFinalizeBlock) {
	...
	for _, del := range verifiedDelegations {
		stakingTx, _, err := anc.NewBTCTxFromHex(del.StakingTxHex)
		require.NoError(d.t, err)
...
func (d *AnonAppDriver) IncludeTxsInBTCAndConfirm(
	txs []*wire.MsgTx,
) (btcBlock *datagen.BlockWithProofs, ancBlock *abci.ResponseFinalizeBlock) {
	...
	ancBlock = d.SendTxWithMsgsFromDriverAccount(d.t, &btclighttypes.MsgInsertHeaders{
		Signer:  d.GetDriverAccountAddress().String(),
		Headers: headers,
	})

	return block, ancBlock
}

// BatchUnbondDelegations processes multiple unbondings in a single Anon block.
// All unbonding BTC txs are included in the same BTC block, and all MsgBTCUndelegate
// messages are submitted before generating the Anon block.
func (d *AnonAppDriver) BatchUnbondDelegations(unbondings []*UnbondingInfo) {
	if len(unbondings) == 0 {
		return
	}
	...
	// Insert BTC headers (this creates a Anon block)
	...
	for i, ub := range unbondings {
		stakingTxBz, err := anc.SerializeBTCTx(ub.StakingTx)
		require.NoError(d.t, err)

		unbondingTxBytes, err := anc.SerializeBTCTx(ub.UnbondingTx)
		require.NoError(d.t, err)
	...
	// Generate the Anon block that includes all unbonding messages
	d.GenerateNewBlockAssertExecutionSuccess()
}

func (d *AnonAppDriver) IncludeTxsInBTC(txs []*wire.MsgTx) *datagen.BlockWithProofs {
	tip, _ := d.GetBTCLCTip()
...
func (d *AnonAppDriver) GenerateNewBlockAssertExecutionSuccessWithResults() []*abci.ExecTxResult {
	response := d.GenerateNewBlock()
...
// to the mempool.
func (d *AnonAppDriver) ConfirmStakingTransactionOnBTC(
	...
	var transactions []*wire.MsgTx
	for _, m := range msg {
		stakingTx, err := anc.NewBTCTxFromBytes(m.StakingTx)
		require.NoError(d.t, err)
		transactions = append(transactions, stakingTx)
	}
...
func (d *AnonAppDriver) GenCkptForEpoch(r *rand.Rand, t *testing.T, epochNumber uint64) {
	...
	tagBytes, err := hex.DecodeString(initialization.AnonOpReturnTag)
	require.NoError(t, err)

	data1, data2 := btctxformatter.MustEncodeCheckpointData(
		btctxformatter.AnonTag(tagBytes),
...
func (d *AnonAppDriver) FinalizeCkptForEpoch(epochNumber uint64) {
...
func (d *AnonAppDriver) ProgressTillFirstBlockTheNextEpoch() {
...
func (d *AnonAppDriver) WaitTillAllFpsJailed(t *testing.T) {
	for {
...
func (d *AnonAppDriver) SendTxWithMsgsFromDriverAccount(
	t *testing.T,
	msgs ...sdk.Msg,
...
func (d *AnonAppDriver) CreateSendingAccountMessage() *NewAccountInfo {
	...
	msgBankSend := banktypes.MsgSend{
		...
		// 100 ANC, should be enough for most tests
		Amount: sdk.NewCoins(sdk.NewCoin("uanc", sdkmath.NewInt(100000000))),
	}
...
func (d *AnonAppDriver) getAccountInfo(accAddress string) sdk.AccountI {
	add, err := sdk.AccAddressFromBech32(accAddress)
	require.NoError(d.t, err)
	return d.App.AccountKeeper.GetAccount(d.GetContextForLastFinalizedBlock(), add)
}

func (d *AnonAppDriver) CreateNStakerAccounts(n int) []*Staker {
	// pre-condition
...
func (d *AnonAppDriver) CreateNFinalityProviderAccounts(n int) []*FinalityProvider {
...
func (d *AnonAppDriver) CreateCovenantSender() *CovenantSender {
...
func (d *AnonAppDriver) GovPropAndVote(msgInGovProp sdk.Msg) (lastPropId uint64) {
...
func (d *AnonAppDriver) GovPropWaitPass(msgInGovProp sdk.Msg) {
...
func (d *AnonAppDriver) BankSendNative(
	t *testing.T,
	from *SenderInfo,
	to sdk.AccAddress,
	amt int64,
...
func (d *AnonAppDriver) BankSendNativeFromSender(
	to sdk.AccAddress,
	amt int64,
) {
...
func (d *AnonAppDriver) MintNativeTo(
