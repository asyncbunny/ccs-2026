...
import (
	...
	bapp "github.com/anon-org/anon/v4/app"
	...
	btclightclientt "github.com/anon-org/anon/v4/x/btclightclient/types"
)
...
func NewBTCHeaderBytesList(chain []*wire.BlockHeader) []anc.BTCHeaderBytes {
	chainBytes := make([]anc.BTCHeaderBytes, len(chain))
	for i, header := range chain {
		chainBytes[i] = anc.NewBTCHeaderBytesFromBlockHeader(header)
	}
	return chainBytes
}
...
func BTCLightClientKeeperWithCustomParams(
	...
	defer cleanup()
	testCfg := anc.ParseBtcOptionsFromConfig(appOpts)
...
func ParseBTCHeaderInfoResponseToInfo(r *btclightclientt.BTCHeaderInfoResponse) (*btclightclientt.BTCHeaderInfo, error) {
	header, err := anc.NewBTCHeaderBytesFromHex(r.HeaderHex)
	if err != nil {
		return nil, err
	}

	hash, err := anc.NewBTCHeaderHashBytesFromHex(r.HashHex)
	if err != nil {
		return nil, err
	}
