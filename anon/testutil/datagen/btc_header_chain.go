...
func NewBTCHeaderChainFromParentInfoResponse(
	...
	headerBytes, err := anc.NewBTCHeaderBytesFromHex(parent.HeaderHex)
	if err != nil {
		panic(err)
	}
	return NewBTCHeaderChainFromParent(
...
func (c *BTCHeaderPartialChain) ChainToBytes() []anc.BTCHeaderBytes {
	chainBytes := make([]anc.BTCHeaderBytes, 0)
	for _, header := range c.Headers {
		h := header
		bytes := anc.NewBTCHeaderBytesFromBlockHeader(h)
