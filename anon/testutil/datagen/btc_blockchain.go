...
// GenRandomBtcdBlock generates a random BTC block, which can include Anon txs.
// Specifically,
// - when numAnonTxs == 0 or numAnonTxs > 2, it generates a BTC block with 3 random txs.
// - when numAnonTxs == 1, it generates a BTC block with 2 random txs and a Anon tx.
// - when numAnonTxs == 2, it generates a BTC block with 1 random tx and 2 Anon txs that constitute a raw BTC checkpoint.
// When numAnonTxs == 2, the function will return the BTC raw checkpoint as well.
func GenRandomBtcdBlock(r *rand.Rand, numAnonTxs int, prevHash *chainhash.Hash) (*wire.MsgBlock, *btctxformatter.RawBtcCheckpoint) {
	var (
	...
	} else if numAnonTxs == 1 {
		ancTxs, _ := GenRandomAnonTxPair(r)
		idx := r.Intn(2)
		randomTxs[idx] = ancTxs[idx]
	}
...
func GenRandomBtcdBlockWithTransactions(
	...
	for i := range msgTxs {
		headerBytes := anc.NewBTCHeaderBytesFromBlockHeader(header)
...
// GenRandomBtcdBlockchainWithAnonTx generates a blockchain of `n` blocks, in which each block has some probability of including some Anon txs
// Specifically, each block
// - has `oneTxThreshold` probability of including 1 Anon tx that does not has any match
// - has `twoTxThreshold - oneTxThreshold` probability of including 2 Anon txs that constitute a checkpoint
// - has `1 - twoTxThreshold` probability of including no Anon tx
func GenRandomBtcdBlockchainWithAnonTx(r *rand.Rand, n uint64, oneTxThreshold float32, twoTxThreshold float32) ([]*wire.MsgBlock, int, []*btctxformatter.RawBtcCheckpoint) {
