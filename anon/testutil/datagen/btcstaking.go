...
import (
	...
	stk "github.com/anon-org/anon/v4/btcstaking"
...
func GenRandomMsgCreateFinalityProvider(r *rand.Rand) (*bstypes.MsgCreateFinalityProvider, error) {
	...
	return GenRandomCreateFinalityProviderMsgWithBTCAnonSKs(r, btcSK, GenRandomAccount().GetAddress())
}
...
func GenRandomFinalityProviderWithBTCSK(r *rand.Rand, btcSK *btcec.PrivateKey) (*bstypes.FinalityProvider, error) {
	return GenRandomFinalityProviderWithBTCAnonSKs(r, btcSK, GenRandomAccount().GetAddress())
}
...
func GenRandomFinalityProviderWithBTCAnonSKs(
	r *rand.Rand,
...
func GenRandomCreateFinalityProviderMsgWithBTCAnonSKs(
	r *rand.Rand,
	btcSK *btcec.PrivateKey,
	fpAddr sdk.AccAddress,
) (*bstypes.MsgCreateFinalityProvider, error) {
	fp, err := GenRandomFinalityProviderWithBTCAnonSKs(r, btcSK, fpAddr)
	if err != nil {
		return nil, err
	}
...
func GenRandomBTCDelegation(
	...
	delPK := delSK.PubKey()
	delBTCPK := anc.NewBIP340PubKeyFromBTCPK(delPK)

	// list of finality provider PKs
	fpPKs, err := anc.NewBTCPKsFromBIP340PKs(fpBTCPKs)
	if err != nil {
		return nil, err
	}
...
func (info *TestUnbondingSlashingInfo) GenDelSlashingTxSig(sk *btcec.PrivateKey) (*anc.BIP340Signature, error) {
...
func (info *TestUnbondingSlashingInfo) GenCovenantSigs(
	...
	for i := range covUnbondingSigs {
		covUnbondingSigList = append(covUnbondingSigList, &bstypes.SignatureInfo{
			Pk:  anc.NewBIP340PubKeyFromBTCPK(covSKs[i].PubKey()),
			Sig: anc.NewBIP340SignatureFromBTCSig(covUnbondingSigs[i]),
		})
	}
...
func AddWitnessToUnbondingTx(
	...
	serializedUnbondingTxWithWitness, err := anc.SerializeBTCTx(unbondingTx)
	require.NoError(t, err)
...
func AddWitnessToStakeExpTx(
	...
	serializedUnbondingTxWithWitness, err := anc.SerializeBTCTx(spendingTx)
	require.NoError(t, err)
