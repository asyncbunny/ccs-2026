...
func GenRandomEvidence(r *rand.Rand, sk *btcec.PrivateKey, height uint64) (*ftypes.Evidence, error) {
	pk := sk.PubKey()
	bip340PK := anc.NewBIP340PubKeyFromBTCPK(pk)
	...
	evidence := &ftypes.Evidence{
		FpBtcPk:              bip340PK,
		BlockHeight:          height,
		PubRand:              anc.NewSchnorrPubRandFromFieldVal(pr),
		CanonicalAppHash:     cAppHash,
		ForkAppHash:          fAppHash,
		CanonicalFinalitySig: anc.NewSchnorrEOTSSigFromModNScalar(cSig),
		ForkFinalitySig:      anc.NewSchnorrEOTSSigFromModNScalar(fSig),
	}
	return evidence, nil
}
...
func GenRandomFinalitySig(r *rand.Rand) (*anc.SchnorrEOTSSig, error) {
	...
	return anc.NewSchnorrEOTSSigFromModNScalar(&modNScalar), nil
}
