...
func CreatOpReturnTransaction(r *rand.Rand, anonData []byte) *wire.MsgTx {
	out := makeSpendableOutWithRandOutPoint(r, 1000)
	tx := createSpendOpReturnTx(&out, lowFee, anonData)
	return tx
}
...
func CreateBlock(
	...
	height uint32,
	numTx uint32,
	anonOpReturnIdx uint32,
	anonData []byte,
) *BlockCreationResult {
	if anonOpReturnIdx > numTx {
		panic("anon tx index should be less than number of transasactions and greater than 0")
	}
	...
	res := BlockCreationResult{
		HeaderBytes:  anc.NewBTCHeaderBytesFromBlockHeader(btcHeader),
		Transactions: hexTx,
		AncTxIndex:   anonOpReturnIdx,
	}
...
func GenRandomAnonTxPair(r *rand.Rand) ([]*wire.MsgTx, *txformat.RawBtcCheckpoint) {
...
func BlockCreationResultToProofs(inputs []*BlockCreationResult) []*btcctypes.BTCSpvProof {
	...
	for _, input := range inputs {
		headerBytes, err := anc.NewBTCHeaderBytesFromBytes(input.HeaderBytes)
		...
		spv, err := btcctypes.SpvProofFromHeaderAndTransactions(&headerBytes, txBytes, uint(input.AncTxIndex))
...
func getExpectedOpReturn(tag txformat.AnonTag, f []byte, s []byte) []byte {
...
func EncodeRawCkptToTestData(rawBTCCkpt *txformat.RawBtcCheckpoint) *TestRawCheckpointData {
	...
	anonTag := txformat.AnonTag(tagAsBytes)
	data1, data2 := txformat.MustEncodeCheckpointData(
		anonTag,
		txformat.CurrentVersion,
		rawBTCCkpt,
	)
	opReturn := getExpectedOpReturn(anonTag, data1, data2)
