...
func GenCovenantAdaptorSigs(
	...
	for _, covenantSK := range covenantSKs {
		covMemberSigs := &bstypes.CovenantAdaptorSignatures{
			CovPk:       anc.NewBIP340PubKeyFromBTCPK(covenantSK.PubKey()),
			AdaptorSigs: [][]byte{},
		}
...
func GenCovenantStakeExpSig(covenantSKs []*btcec.PrivateKey, del *bstypes.BTCDelegation, prevDelStakingInfo *btcstaking.StakingInfo) ([]*anc.BIP340Signature, error) {
	if del.StkExp == nil {
	...
	for i := range covenantSKs {
		...
		sigs = append(sigs, anc.NewBIP340SignatureFromBTCSig(sig))
	}
