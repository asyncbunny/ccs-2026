...
import (
	...
	btclightclientk "github.com/anon-org/anon/v4/x/btclightclient/keeper"
...
func GenRandomBTCHeaderPrevBlock(r *rand.Rand) *anc.BTCHeaderHashBytes {
	hex := GenRandomHexStr(r, anc.BTCHeaderHashLen)
	hashBytes, _ := anc.NewBTCHeaderHashBytesFromHex(hex)
	return &hashBytes
}
...
func GenRandomBTCHeaderBytes(r *rand.Rand, parent *btclightclienttypes.BTCHeaderInfo, bitsBig *sdkmath.Uint) anc.BTCHeaderBytes {
	...
	return anc.NewBTCHeaderBytesFromBlockHeader(btcdHeader)
}
...
func GenRandomValidBTCHeaderInfoWithParent(r *rand.Rand, parent btclightclienttypes.BTCHeaderInfo) *btclightclienttypes.BTCHeaderInfo {
	...
	headerBytes := anc.NewBTCHeaderBytesFromBlockHeader(randHeader)
...
func ChainToInfoChain(
	...
	for i, header := range chain {
		...
		headerHash := anc.NewBTCHeaderHashBytesFromChainhash(&hash)
...
func HeaderToHeaderBytes(headers []*wire.BlockHeader) []anc.BTCHeaderBytes {
	headerBytes := make([]anc.BTCHeaderBytes, len(headers))
	for i, header := range headers {
		headerBytes[i] = anc.NewBTCHeaderBytesFromBlockHeader(header)
	}
	return headerBytes
}
...
func GenRandomBTCHeaderInfoWithInvalidHeader(r *rand.Rand, powLimit *big.Int) *btclightclienttypes.BTCHeaderInfo {
	...
	for {
		...
		err := anc.ValidateBTCHeader(info.Header.ToBlockHeader(), powLimit)
...
func MutateHash(r *rand.Rand, hash *anc.BTCHeaderHashBytes) *anc.BTCHeaderHashBytes {
	mutatedBytes := make([]byte, anc.BTCHeaderHashLen)
	// Retrieve a random byte index
	idx := RandomInt(r, anc.BTCHeaderHashLen)
	copy(mutatedBytes, hash.MustMarshal())
	// Add one to the index
	mutatedBytes[idx] += 1
	mutated, _ := anc.NewBTCHeaderHashBytesFromBytes(mutatedBytes)
	return &mutated
}
