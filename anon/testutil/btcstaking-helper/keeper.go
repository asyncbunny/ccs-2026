...
import (
	...
	"github.com/anon-org/anon/v4/x/btcstaking/keeper"
	...
	fkeeper "github.com/anon-org/anon/v4/x/finality/keeper"
...
func (h *Helper) CreateDelegationWithBtcBlockHeight(
	...
	*/
	...
	msgCreateBTCDel := &types.MsgCreateBTCDelegation{
		...
		BtcPk:                         stPk,
		FpBtcPkList:                   []anc.BIP340PubKey{*fpBTCPK},
		Pop:                           pop,
	...
	stakingMsgTx, err := anc.NewBTCTxFromBytes(msgCreateBTCDel.StakingTx)
	h.NoError(err)
...
func (h *Helper) GenerateCovenantSignaturesMessages(
	...
	stakingTx, err := anc.NewBTCTxFromBytes(del.StakingTx)
	h.NoError(err)
	stakingTxHash := stakingTx.TxHash().String()

	bsParams := h.BTCStakingKeeper.GetParams(h.Ctx)

	vPKs, err := anc.NewBTCPKsFromBIP340PKs(del.FpBtcPkList)
	h.NoError(err)
	...
	*/
	...
	unbondingTx, err := anc.NewBTCTxFromBytes(del.BtcUndelegation.UnbondingTx)
	h.NoError(err)
	...
	for i := 0; i < len(bsParams.CovenantPks); i++ {
		msgAddCovenantSig := &types.MsgAddCovenantSigs{
			...
			UnbondingTxSig:          anc.NewBIP340SignatureFromBTCSig(covUnbondingSigs[i]),
...
func (h *Helper) CreateBtcStakeExpansionWithBtcTipHeight(
	...
	spendingTx, err := anc.NewBTCTxFromBytes(expandMsg.StakingTx)
	if err != nil {
		return nil, nil, err
	}

	fundingTx, err := anc.NewBTCTxFromBytes(expandMsg.FundingTx)
	if err != nil {
		return nil, nil, err
	}
