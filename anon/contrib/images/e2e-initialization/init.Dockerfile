...
# Create a user
RUN addgroup --gid 1137 --system anon && adduser --uid 1137 --gid 1137 --system --home /home/anon anon
...
COPY --from=build-env /go/src/github.com/anon-org/anon/go.mod /tmp
...
COPY --from=build-env /go/src/github.com/anon-org/anon/build/${E2E_SCRIPT_NAME} /bin/${E2E_SCRIPT_NAME}
