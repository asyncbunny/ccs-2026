...
func ToProviderMsgs(msgs []sdk.Msg) []anonclient.RelayerMessage {
	relayerMsgs := make([]anonclient.RelayerMessage, 0, len(msgs))
	for _, m := range msgs {
		relayerMsgs = append(relayerMsgs, anonclient.NewCosmosMessage(m, func(signer string) {}))
	}
	return relayerMsgs
}
...
func (c *Client) SendMsgsToMempool(ctx context.Context, msgs []sdk.Msg) error {
	...
	if err := retry.Do(func() error {
		...
		krErr := c.accessKeyWithLock(func() {
			sendMsgErr = c.provider.SendMessagesToMempool(ctx, relayerMsgs, "", ctx, []func(*anonclient.RelayerTxResponse, error){})
		})
		if krErr != nil {
...
// TODO: needs tests
func (c *Client) ReliablySendMsg(ctx context.Context, msg sdk.Msg, expectedErrors []*errors.Error, unrecoverableErrors []*errors.Error) (*anonclient.RelayerTxResponse, error) {
...
// It utilizes a file lock as well as a keyring lock to ensure atomic access.
// TODO: needs tests
func (c *Client) ReliablySendMsgs(ctx context.Context, msgs []sdk.Msg, expectedErrors []*errors.Error, unrecoverableErrors []*errors.Error) (*anonclient.RelayerTxResponse, error) {
	var (
		rlyResp     *anonclient.RelayerTxResponse
		callbackErr error
		wg          sync.WaitGroup
	)

	callback := func(rtr *anonclient.RelayerTxResponse, err error) {
		rlyResp = rtr
		callbackErr = err
		wg.Done()
	}
	...
	// TODO: consider using Anon's retry package
	if err := retry.Do(func() error {
		var sendMsgErr error
		krErr := c.accessKeyWithLock(func() {
			sendMsgErr = c.provider.SendMessagesToMempool(ctx, relayerMsgs, "", ctx, []func(*anonclient.RelayerTxResponse, error){callback})
		})
		if krErr != nil {
			c.logger.Error("unrecoverable err when submitting the tx, skip retrying", zap.Error(krErr))
			return retry.Unrecoverable(krErr)
...
func (c *Client) ReliablySendMsgsWithSigner(ctx context.Context, signerAddr sdk.AccAddress, signerPvKey *secp256k1.PrivKey, msgs []sdk.Msg, expectedErrors []*errors.Error, unrecoverableErrors []*errors.Error) (*anonclient.RelayerTxResponse, error) {
	var (
		rlyResp     *anonclient.RelayerTxResponse
		callbackErr error
		wg          sync.WaitGroup
...
func (c *Client) SendMessageWithSigner(
	...
	relayerMsgs []anonclient.RelayerMessage,
) (result *coretypes.ResultBroadcastTx, err error) {
	cMsgs := anonclient.CosmosMsgs(relayerMsgs...)
	var (
		num, seq uint64
	)
...
func (c *Client) InsertBTCSpvProof(ctx context.Context, msg *btcctypes.MsgInsertBTCSpvProof) (*anonclient.RelayerTxResponse, error) {
	return c.ReliablySendMsg(ctx, msg, []*errors.Error{}, []*errors.Error{})
}

func (c *Client) InsertHeaders(ctx context.Context, msg *btclctypes.MsgInsertHeaders) (*anonclient.RelayerTxResponse, error) {
	return c.ReliablySendMsg(ctx, msg, []*errors.Error{}, []*errors.Error{})
}
