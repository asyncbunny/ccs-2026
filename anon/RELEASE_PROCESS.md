...
This document outlines the release process for the Anon node (anond)

Anon follows [semantic versioning](https://semver.org), but with the
...
`lightweight` tag, so it's possible that `anond version` returns a commit
hash, instead of a tag. This is important because most operators build from
source, and having incorrect information when you run `make install && anond
...
The intention of the Release Policy is to ensure that the latest Anond release is maintained with the following
categories of fixes:
