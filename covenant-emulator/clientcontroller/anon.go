...
var (
	_                  ClientController = &AnonController{}
...
func NewAnonController(
	...
	logger *zap.Logger,
	maxRetiresBatchRemovingMsgs uint64,
) (*AnonController, error) {
	ancConfig := config.ANCConfigToAnonConfig(cfg)
...
func (bc *AnonController) mustGetTxSigner() string {
...
func (bc *AnonController) GetKeyAddress() sdk.AccAddress {
	...
	// and we should panic.
	// This is checked at the start of AnonController, so if it fails something is really wrong
...
func (bc *AnonController) QueryStakingParamsByVersion(version uint32) (*types.StakingParams, error) {
	...
	stakingParamRes, err := bc.ancClient.BTCStakingParamsByVersion(version)
	if err != nil {
	...
	if stakingParamRes.Params.MinStakingTimeBlocks > math.MaxUint16 {
		return nil, fmt.Errorf("anon min staking time blocks (%d) is larger than the maximum uint16", stakingParamRes.Params.MinStakingTimeBlocks)
	}
	...
	if stakingParamRes.Params.MaxStakingTimeBlocks > math.MaxUint16 {
		return nil, fmt.Errorf("anon max staking time blocks (%d) is larger than the maximum uint16", stakingParamRes.Params.MaxStakingTimeBlocks)
	}
	// #nosec G115 -- performed the conversion check above
...
func (bc *AnonController) reliablySendMsg(msg sdk.Msg) (*anonclient.RelayerTxResponse, error) {
	return bc.reliablySendMsgs([]sdk.Msg{msg})
}

func (bc *AnonController) reliablySendMsgs(msgs []sdk.Msg) (*anonclient.RelayerTxResponse, error) {
	return bc.ancClient.ReliablySendMsgs(
		context.Background(),
		msgs,
		nil,
		unrecoverableErrors,
...
// SubmitCovenantSigs submits the Covenant signature via a MsgAddCovenantSig to Anon if the daemon runs in Covenant mode
// it returns tx hash and error
func (bc *AnonController) SubmitCovenantSigs(covSigs []*types.CovenantSigs) (*types.TxResponse, error) {
	msgs := make([]sdk.Msg, 0, len(covSigs))
	for _, covSig := range covSigs {
		bip340UnbondingSig := anctypes.NewBIP340SignatureFromBTCSig(covSig.UnbondingSig)
		msg := &btcstakingtypes.MsgAddCovenantSigs{
			Signer:                  bc.mustGetTxSigner(),
			Pk:                      anctypes.NewBIP340PubKeyFromBTCPK(covSig.PublicKey),
		...
		if covSig.StkExpSig != nil {
			stkExpSig := anctypes.NewBIP340SignatureFromBTCSig(covSig.StkExpSig)
...
func (bc *AnonController) reliablySendMsgsResendingOnMsgErr(msgs []sdk.Msg) (*types.TxResponse, error) {
	var err error
...
func (bc *AnonController) QueryPendingDelegations(limit uint64, filter FilterFn) ([]*types.Delegation, error) {
	return bc.queryDelegationsWithStatus(btcstakingtypes.BTCDelegationStatus_PENDING, limit, filter)
}

func (bc *AnonController) QueryActiveDelegations(limit uint64) ([]*types.Delegation, error) {
	return bc.queryDelegationsWithStatus(btcstakingtypes.BTCDelegationStatus_ACTIVE, limit, nil)
}

func (bc *AnonController) QueryVerifiedDelegations(limit uint64) ([]*types.Delegation, error) {
...
func (bc *AnonController) QueryBTCDelegation(stakingTxHashHex string) (*types.Delegation, error) {
	resp, err := bc.ancClient.BTCDelegation(stakingTxHashHex)
	if err != nil {
...
func (bc *AnonController) queryDelegationsWithStatus(status btcstakingtypes.BTCDelegationStatus, delsLimit uint64, filter FilterFn) ([]*types.Delegation, error) {
...
func (bc *AnonController) Close() error {
	if !bc.ancClient.IsRunning() {
		return nil
	}

	return bc.ancClient.Stop()
}
...
func (bc *AnonController) CreateBTCDelegation(
...
// Currently this is only used for e2e tests, probably does not need to add it into the interface
func (bc *AnonController) CreateStakeExpansionDelegation(
	delBtcPk *anctypes.BIP340PubKey,
	fpPks []*btcec.PublicKey,
	pop *btcstakingtypes.ProofOfPossessionBTC,
	stakingTime uint32,
...
// Register a finality provider to Anon
// Currently this is only used for e2e tests, probably does not need to add it into the interface
func (bc *AnonController) RegisterFinalityProvider(
	btcPubKey *anctypes.BIP340PubKey, commission *sdkmath.LegacyDec,
	description *stakingtypes.Description, pop *btcstakingtypes.ProofOfPossessionBTC) (*anonclient.RelayerTxResponse, error) {
...
// Currently this is only used for e2e tests, probably does not need to add it into the interface
func (bc *AnonController) InsertBtcBlockHeaders(headers []anctypes.BTCHeaderBytes) (*anonclient.RelayerTxResponse, error) {
...
func (bc *AnonController) QueryFinalityProviders() ([]*btcstakingtypes.FinalityProviderResponse, error) {
	...
	clientCtx := sdkclient.Context{Client: bc.ancClient.RPCClient}
...
// Currently this is only used for e2e tests, probably does not need to add this into the interface
func (bc *AnonController) QueryBtcLightClientTip() (*btclctypes.BTCHeaderInfoResponse, error) {
	ctx, cancel := getContextWithCancel(bc.cfg.Timeout)
	defer cancel()
