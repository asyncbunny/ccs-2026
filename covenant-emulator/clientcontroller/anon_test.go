...
func FuzzDelegationRespToDelegation(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		response := types.BTCDelegationResponse{
			BtcPk:            delPub,
			FpBtcPkList:      []anctypes.BIP340PubKey{*fpPub},
		...
		require.Equal(t, response.BtcPk, anctypes.NewBIP340PubKeyFromBTCPK(del.BtcPk))
		require.Equal(t, response.FpBtcPkList, anctypes.NewBIP340PKsFromBTCPKs(del.FpBtcPks))
	})
}
